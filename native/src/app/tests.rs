...
#[test]
fn test_trade_marker_dedupes_darwin_closed_position_entry_and_recent_deal() {
    // A DARWIN closed position entry and a recent deal can describe the same
    // execution. The chart marker should show the actual executed volume once,
    // not sum both sources into a 2x buy marker.
    use super::chart_ops::{
        add_trade_marker_volume, darwin_marker_signature, trade_marker_price_key,
    };
    use std::collections::{HashMap, HashSet};

    let mut marker_map: HashMap<(usize, bool, i64), (f64, u32, String)> = HashMap::new();
    let mut closed_position_markers = HashSet::new();
    let bar_idx = 42;
    let is_buy = true;
    let price_key = trade_marker_price_key(123.45);
    let expected_actual_volume = 0.10;
    let account = "ACC-1";
    let ticker = "DARWIN-1";

    // Closed-position entry arrow.
    ...
    closed_position_markers.insert(darwin_marker_signature(
        account,
        ticker,
        bar_idx,
        is_buy,
    ...
    // Recent deal row for the same account/symbol/time/bar/side/price/volume.
    ...
    if !closed_position_markers.contains(&signature) {
        add_trade_marker_volume(
            &mut marker_map,
            bar_idx,
            is_buy,
    ...
    let buy = marker_map.get(&(bar_idx, is_buy, price_key)).unwrap();
    assert!(
        (buy.0 - expected_actual_volume).abs() < 0.001,
        "duplicate DARWIN position+deal sources should not double marker volume: got {} expected {}",
        buy.0,
        expected_actual_volume
    );
    assert_eq!(
        buy.1, 1,
        "duplicate source should not increment trade count"
    );
}
