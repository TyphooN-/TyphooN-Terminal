            ...
            && let Ok(msg) = self.broker_rx.try_recv()
        {
            ...
            match msg {
                ...
                BrokerMsg::KrakenTrades(mut trades) => {
                    ...
                    self.log.push_back(LogEntry::info(format!(
                        "Kraken: loaded {} trades",
                        self.kraken_trades.len()
                    )));
                }
                BrokerMsg::KrakenLiveTrade(trade) => {
                    let duplicate = self.kraken_trades.iter().any(|existing| {
                        (!trade.trade_id.is_empty() && existing.trade_id == trade.trade_id)
                            || (!trade.ordertxid.is_empty()
                                && existing.ordertxid == trade.ordertxid
                                && (existing.time - trade.time).abs() < 1e-6)
                    });
                    if !duplicate {
