                ...
                if let Some(conn) = cache.try_connection() {
                    ...
                    if let Ok(Some(est)) = rx::get_earnings_estimates(&conn, &sym_upper) {
                        if !est.is_empty() {
                            ...
                            let _ = writeln!(
                                p,
                                "| Period | EPS Avg | EPS Lo/Hi | Rev Avg | Analysts |"
                            );
                            let _ = writeln!(p, "|---|---|---|---|---|");
                    ...
                    if let Ok(Some(fa)) = rx::get_financials(&conn, &sym_upper) {
                        if !fa.income_annual.is_empty() {
                            ...
                            let _ =
                                writeln!(p, "| FY | Revenue | Gross | Op Inc | Net Inc | EPS |");
                            let _ = writeln!(p, "|---|---|---|---|---|---|");
                    ...
                    if let Ok(Some(holders)) = rx::get_institutional_holders(&conn, &sym_upper) {
                        if !holders.is_empty() {
                    ...
                    if let Ok(Some(sf)) = rx::get_shares_float(&conn, &sym_upper) {
                    ...
                    if let Ok(Some(hp)) = rx::get_historical_price(&conn, &sym_upper) {
                        if !hp.is_empty() {
                    ...
                    if let Ok(Some(eps)) = rx::get_earnings_surprises(&conn, &sym_upper) {
                        if !eps.is_empty() {
                    ...
                    if let Ok(Some(rv)) = rx::get_relative_valuation(&conn, &sym_upper) {
                        if !rv.rows.is_empty() {
                            ...
                            let _ =
                                writeln!(p, "| Metric | Value | Peer Median | Z | Percentile |");
                            let _ = writeln!(p, "|---|---|---|---|---|");
                    ...
                    if let Ok(Some(d)) = rx::get_dcf(&conn, &sym_upper) {
                        if d.implied_price > 0.0 || !d.note.is_empty() {
                            ...
                            if !d.years.is_empty() {
                                let _ = writeln!(
                                    p,
                                    "| Year | Revenue | EBIT | NOPAT | FCFF | PV FCFF |"
                                );
                                let _ = writeln!(p, "|---|---|---|---|---|---|");
                    ...
                    if let Ok(Some(s)) = rx::get_svm(&conn, &sym_upper) {
                        if !s.rows.is_empty() {
                            ...
                            let _ =
                                writeln!(p, "| Model | Implied | Upside | Confidence | Source |");
                            let _ = writeln!(p, "|---|---|---|---|---|");
                    ...
                    if let Ok(Some(o)) = rx::get_options_chain(&conn, &sym_upper) {
                        if !o.expirations.is_empty() {
                            ...
                            if let Some(exp) = o.expirations.first() {
                                ...
                                let total_put_oi: f64 =
                                    exp.puts.iter().map(|c| c.open_interest).sum();
                                ...
                                if !strikes.is_empty() {
                                    ...
                                    for k in &strikes[lo..=hi] {
                                        let c =
                                            exp.calls.iter().find(|c| (c.strike - k).abs() < 1e-6);
                                        let pt =
                    ...
                    if let Ok(Some(sg)) = rx::get_seasonality(&conn, &sym_upper) {
                        if !sg.months.is_empty() || !sg.dow.is_empty() {
                            ...
                            if !sg.months.is_empty() {
                                let _ =
                                    writeln!(p, "| Month | Avg | Median | Stdev | +Years | N |");
                                let _ = writeln!(p, "|---|---|---|---|---|---|");
                    ...
                    if let Ok(Some(ac)) = rx::get_accruals(&conn, &sym_upper) {
                        if !ac.periods.is_empty() {
                            let _ =
                                writeln!(p, "### Earnings Quality / Accruals (as of {})", ac.as_of);
                            let _ = writeln!(
                                p,
                    ...
                    if let Ok(Some(rv)) = rx::get_realized_vol(&conn, &sym_upper) {
                        if !rv.windows.is_empty() {
                            let _ =
                                writeln!(p, "### Realized Volatility Cone (as of {})", rv.as_of);
                    ...
                    if let Ok(Some(si)) = rx::get_short_interest(&conn, &sym_upper) {
                        if si.shares_float > 0.0 || si.short_percent_of_float > 0.0 {
                            let _ = writeln!(
                                p,
                                "### Short Interest & Days-to-Cover (as of {})",
                                si.as_of
                            );
                            let _ = writeln!(
                                p,
                    ...
                    if let Ok(Some(ov)) = rx::get_ohlc_vol(&conn, &sym_upper) {
                        if !ov.estimators.is_empty() {
                            let _ =
                                writeln!(p, "### OHLC Volatility Estimators (as of {})", ov.as_of);
                            let _ = writeln!(
                                p,
                    ...
                    if let Ok(Some(eb)) = rx::get_eps_beat(&conn, &sym_upper) {
                        if eb.total_reports > 0 {
                            let _ =
                                writeln!(p, "### EPS Beat Streak & Surprise (as of {})", eb.as_of);
                            let _ = writeln!(
                                p,
                            ...
                            let _ = writeln!(
                                ...
                                eb.avg_surprise_pct,
                                eb.median_surprise_pct,
                                eb.recent_avg_surprise_pct
                            );
                    ...
                    if let Ok(Some(sr)) = rx::get_sector_rotation(&conn, &sym_upper) {
                        if sr.sectors_total > 0 {
                            let _ =
                                writeln!(p, "### Sector Rotation Strength (as of {})", sr.as_of);
                            let _ = writeln!(
                                p,
                                "- {} · sector: {} · rank {}/{}",
                                sr.strength_label,
                    ...
                    if let Ok(Some(um)) = rx::get_updm(&conn, &sym_upper) {
                        if um.total_actions > 0 {
                            let _ =
                                writeln!(p, "### Upgrade/Downgrade Momentum (as of {})", um.as_of);
                            let _ = writeln!(
                                p,
                                "- {} · trend: {} · {} total actions",
                    ...
                    if let Ok(Some(rv)) = rx::get_relvol(&conn, &sym_upper) {
                        ...
                        {
                            let _ =
                                writeln!(p, "### Relative Volume — RELVOL (as of {})", rv.as_of);
                            let _ = writeln!(
                                p,
