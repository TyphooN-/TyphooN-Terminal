...
const KAMA_COL: egui::Color32 = egui::Color32::from_rgb(220, 220, 230);  // soft white (MT5 KAMA)
...
fn parse_range_f32(s: &str, default_lo: f64, default_hi: f64) -> (f64, f64) {
    let parts: Vec<&str> = s.split('-').collect();
    if parts.len() == 2 {
        let lo = parts[0].trim().parse().unwrap_or(default_lo);
        let hi = parts[1].trim().parse().unwrap_or(default_hi);
...
pub struct TyphooNApp {
    ...
    opt_atr_tp_range: String,
    gpu_opt_results: Vec<gpu_compute::BacktestResult>,
    gpu_opt_combos: Vec<gpu_compute::ParamCombo>,
    gpu_backtester: Option<gpu_compute::GpuBacktester>,
...
impl TyphooNApp {
    pub fn new(_cc: &eframe::CreationContext<'_>, rt_handle: tokio::runtime::Handle) -> Self {
        ...
        if let Some(ref render_state) = _cc.wgpu_render_state {
            ...
            app.gpu_indicators = Some(gpu_compute::GpuCompute::new(device.clone(), queue.clone()));
            app.gpu_backtester = Some(gpu_compute::GpuBacktester::new(device, queue));
            app.log.push_back(LogEntry::info("GPU compute initialized (indicators + DARWIN analytics + backtester)"));
        }
        ...
        if self.show_optimizer {
            egui::Window::new("Optimizer")
                ...
                .show(ctx, |ui| {
                    ...
                    let chart = self.charts.get(self.active_tab);
                    let n_bars = chart.map(|c| c.bars.len()).unwrap_or(0);
                    ui.label(egui::RichText::new(format!("Symbol: {}  |  Bars: {}  |  {}", self.symbol_input, n_bars,
                        chart.map(|c| c.timeframe.label()).unwrap_or("?"))).color(opt_cyan));

                    // Parameter ranges
                    ui.add_space(4.0);
                    ui.label(egui::RichText::new("Parameter Ranges").strong());
                    egui::Grid::new("opt_params").num_columns(4).show(ui, |ui| {
                        ui.label("SMA Fast:"); ui.add(egui::TextEdit::singleline(&mut self.opt_fast_range).desired_width(60.0));
                        ui.label("SMA Slow:"); ui.add(egui::TextEdit::singleline(&mut self.opt_slow_range).desired_width(60.0));
                        ui.end_row();
                        ui.label("RSI Period:"); ui.add(egui::TextEdit::singleline(&mut self.opt_rsi_range).desired_width(60.0));
                        ui.label("ATR SL Mult:"); ui.add(egui::TextEdit::singleline(&mut self.opt_atr_sl_range).desired_width(60.0));
                        ui.end_row();
                        ui.label("ATR TP Mult:"); ui.add(egui::TextEdit::singleline(&mut self.opt_atr_tp_range).desired_width(60.0));
                        ui.label(""); ui.label("");
                        ui.end_row();
                    });
                    ...
                    ui.add_space(4.0);
                    ui.horizontal(|ui| {
                        // GPU optimizer button
                        if gpu_available {
                            if ui.button(egui::RichText::new("Run GPU Optimization").color(opt_green).strong()).clicked() && n_bars > 50 {
                                if let Some(chart) = self.charts.get(self.active_tab) {
                                                    ...
                                                    while sl_m <= atr_sl.1 + 0.001 {
                                                        ...
                                                        while tp_m <= atr_tp.1 + 0.001 {
                                                            combos.push(gpu_compute::ParamCombo {
                                                                sma_fast: f as u32, sma_slow: s as u32,
                                                                rsi_period: r as u32, rsi_overbought: 70.0, rsi_oversold: 30.0,
                                                                atr_period: 14, atr_sl_mult: sl_m as f32, atr_tp_mult: tp_m as f32,
                                                            });
                                                            tp_m += atr_tp_step;
                                                        }
                                                        sl_m += atr_sl_step;
                                    ...
                                    if let Some(ref mut bt) = self.gpu_backtester {
                                        ...
                                        if let Some(results) = bt.evaluate() {
                                            ...
                                            let mut indexed: Vec<(usize, &gpu_compute::BacktestResult)> = self.gpu_opt_results.iter().enumerate().collect();
                                            indexed.sort_by(|a, b| b.1.sharpe.partial_cmp(&a.1.sharpe).unwrap_or(std::cmp::Ordering::Equal));
                                            // Reorder results + combos
                                            let sorted_results: Vec<gpu_compute::BacktestResult> = indexed.iter().map(|(i, _)| self.gpu_opt_results[*i].clone()).collect();
                                            let sorted_combos: Vec<gpu_compute::ParamCombo> = indexed.iter().map(|(i, _)| self.gpu_opt_combos[*i].clone()).collect();
                        ...
                        if ui.button("Run CPU Optimization").clicked() && n_bars > 50 {
                            if let Some(chart) = self.charts.get(self.active_tab) {
                                ...
                                self.log.push_back(LogEntry::info(format!("CPU Optimizer: {} combinations tested", report.total_combinations)));
                            }
                        }
                    });
                    ...
                    if !self.gpu_opt_results.is_empty() {
                        ui.add_space(6.0);
                        ui.horizontal(|ui| {
                            ui.label(egui::RichText::new(format!("GPU Results — Top {} of {}", self.gpu_opt_results.len().min(50), self.gpu_opt_results.len())).strong().color(opt_green));
                            // Export best result to MQL5
                            if ui.small_button(egui::RichText::new("Export #1 to MQL5").color(opt_cyan)).clicked() {
                                if let (Some(best), Some(combo)) = (self.gpu_opt_results.first(), self.gpu_opt_combos.first()) {
                                    let strategy = typhoon_engine::core::mql5_export::OptimizedStrategy {
                                        symbol: self.symbol_input.clone(),
                                        timeframe: self.charts.get(self.active_tab).map(|c| c.timeframe.label().to_string()).unwrap_or_default(),
                                    ...
                                    let mut out = dirs_home();
                                    out.push("export");
                                    match typhoon_engine::core::mql5_export::export_strategy(&strategy, &out.display().to_string()) {
                                        Ok(files) => {
                                            for f in &files { self.log.push_back(LogEntry::info(format!("Exported: {}", f))); }
                                        }
                                        Err(e) => self.log.push_back(LogEntry::err(format!("MQL5 export failed: {}", e))),
                                    }
                                }
                            }
                        });
                        ...
                        {
                            let bars: Vec<PlotBar> = self.gpu_opt_results.iter().take(50).enumerate().map(|(i, r)| {
                                let c = if r.net_pnl >= 0.0 { opt_green } else { opt_red };
                        ...
                        egui::ScrollArea::vertical().max_height(350.0).show(ui, |ui| {
                            egui::Grid::new("gpu_opt_grid").striped(true).num_columns(11).min_col_width(45.0).show(ui, |ui| {
                                ...
                                ui.label(egui::RichText::new("Sharpe").color(opt_dim).small());
                                ui.label(egui::RichText::new("Win%").color(opt_dim).small());
                                ui.label(egui::RichText::new("Trades").color(opt_dim).small());
                                ui.label(egui::RichText::new("Robust").color(opt_dim).small());
                                ui.end_row();

                                for (i, r) in self.gpu_opt_results.iter().take(50).enumerate() {
                                    let combo = &self.gpu_opt_combos[i.min(self.gpu_opt_combos.len().saturating_sub(1))];
                                    ...
                                    let pc = if r.net_pnl >= 0.0 { opt_green } else { opt_red };
                                    ui.label(egui::RichText::new(format!("${:.0}", r.net_pnl)).color(pc));
                        ...
                        egui::ScrollArea::vertical().id_salt("sec_alerts_tab").min_scrolled_height(avail).show(ui, |ui| {
                            if alerts.is_empty() {
                                ...
                                if let Some(id) = dismiss_id {
                                    if let Some(ref cache) = self.cache {
                                        if let Some(conn) = cache.try_connection() { let _ = sec_filing::dismiss_alert(&conn, id, "dismissed"); }
                                    }
                                }
                            }
                        });
