...
pub fn handle_research_compute_command(
    cmd: BrokerCmd,
    ...
    match cmd {
        ...
        | BrokerCmd::ComputeSvmSnapshot { .. }) => {
            valuation::handle_valuation_compute(
                cmd,
        ...
        cmd @ BrokerCmd::ComputeIvolSnapshot { .. } => {
            volatility::handle_volatility_compute(
                cmd,
                broker_msg_tx_clone.clone(),
                shared_cache_broker.clone(),
            );
        ...
        // ── analytics ──
        cmd @ (BrokerCmd::ComputeSeasonalitySnapshot { .. }
        ...
        | BrokerCmd::ComputeVolSkewSnapshot { .. }) => {
            analytics::handle_analytics_compute(
                cmd,
                broker_msg_tx_clone.clone(),
                shared_cache_broker.clone(),
            );
