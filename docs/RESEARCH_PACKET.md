...
### 2. Global Market Context (ADR-079)
...
#### Global.4 World Currency Rates (WCR — ADR-079)
...
New in ADR-079 — pulled from the multi-source news pipeline (ADR-078) via
...
then a table of Name / Position / Since / Compensation. Source: ADR-079 MGMT
window.

#### 2.14 Stock split history (SPLT)

Pulled from `research::get_stock_splits`. Capped at **4 most recent splits**.
Columns: Date / Ratio. Source: ADR-079 SPLT window.
...
Source: ADR-079 ANR window.

#### 2.16 ESG score

Pulled from `research::get_esg`. Shows latest-year environmental, social,
governance, and composite scores. Source: ADR-079 ESG window.
...
QoQ Δ (millions of shares) and report date. Source: ADR-079 HDS window.
...
don't need to open the chart. Source: ADR-079 HP window.
...
estimate EPS, surprise, surprise %. Source: ADR-079 EPS window.

#### 2.22 WACC snapshot (WACC — ADR-079)
...
command to populate). Source: ADR-079 WACC window.

#### 2.23 Rolling beta vs SPY (BETA — ADR-079)
...
all windows in one shot. Source: ADR-079 BETA window.

#### 2.24 Gordon Growth DDM (DDM — ADR-079)
...
Source: ADR-079 DDM window.

#### 2.25 Relative valuation matrix (RV — ADR-079)
...
(ADR-079 PEERS surface), and each metric is only emitted when there are ≥3
non-null peer values. Populated by running the `RV` command after peers
and fundamentals are cached. Source: ADR-079 RV window.

#### 2.26 Instrument identifiers (FIGI — ADR-079)
...
Source: ADR-079 FIGI window.

#### 2.27 Historical return / risk (HRA — ADR-079)
...
Source: ADR-079 HRA window.

#### 2.28 Discounted Cash Flow fair value (DCF — ADR-079)
...
Source: ADR-079 DCF window.

#### 2.29 Stock Valuation Model synthesis (SVM — ADR-079)
...
Source: ADR-079 SVM window.

#### 2.30 Options chain summary (OMON — ADR-079)
...
the `OMON` command. Source: ADR-079 OMON window.

#### 2.31 Implied-vol rank / percentile (IVOL — ADR-079)
...
command after `OMON` has pulled the chain. Source: ADR-079 IVOL window.

#### 2.32 Seasonality (SEAG — ADR-079)
...
Source: ADR-079 SEAG window.

#### 2.33 Correlation matrix (COR — ADR-079)
...
intersection. Peers come from `research::get_peers` (ADR-079 PEERS
surface). Populated by running the `COR` command after `PEERS` + `HP`.
Source: ADR-079 COR window.

#### 2.34 Total return analysis (TRA — ADR-079)
...
command. Source: ADR-079 TRA window.

#### 2.35 Technical indicators (TECH — ADR-079)
...
by running the `TECH` command. Source: ADR-079 TECH window.

#### 2.36 Volatility skew / smile (SKEW — ADR-079)
...
`SKEW` command after `OMON`. Source: ADR-079 SKEW window.

#### 2.37 Leverage & coverage (LEV — ADR-079)
...
fallbacks. Populated by running the `LEV` command after `FA`. Source: ADR-079
LEV window.

#### 2.38 Earnings quality / accruals (ACRL — ADR-079)
...
Populated by running the `ACRL` command after `FA`. Source: ADR-079 ACRL
window.

#### 2.39 Realized volatility cone (RVOL — ADR-079)
...
not been run or returns 0. Source: ADR-079 RVOL window.

#### 2.40 FCF yield & dividend sustainability (FCFY — ADR-079)
...
from Fundamentals. Source: ADR-079 FCFY window.

#### 2.41 Short interest & days-to-cover (SHRT — ADR-079)
...
#### 2.42 Altman Z-Score (ALTZ — ADR-079)
...
market cap from Fundamentals. Source: ADR-079 ALTZ window.

#### 2.43 Piotroski F-Score (PTFS — ADR-079)
...
annual cashflow statement. Source: ADR-079 PTFS window.

#### 2.44 OHLC Volatility Estimators (VOLE — ADR-079)
...
valid HP bars. Source: ADR-079 VOLE window.

#### 2.45 EPS Beat Streak & Surprise (EPSB — ADR-079)

Pulled from `research::get_eps_beat`. Walks the cached
`EarningsSurprise` history (from ADR-079 ERN fetch) sorted oldest-first.
...
`EarningsSurprise` row. Source: ADR-079 EPSB window.

#### 2.46 Price Target Dispersion (PTD — ADR-079)

Pulled from `research::get_price_target_dispersion`. Aggregates cached
`PriceTarget` (from ADR-079 UPDG fetch) against current price to compute:
...
positive current price from Fundamentals. Source: ADR-079 PTD window.

#### 2.47 Insider Activity Bias (MNGR — ADR-079)

Pulled from `research::get_insider_activity`. Windows the cached
`InsiderTrade` rows (from ADR-079 INS) over a user-tunable window
...
neither. Needs cached `INS` rows. Source: ADR-079 MNGR window.

#### 2.48 Dividend Growth Analysis (DIVG — ADR-079)

Pulled from `research::get_divg`. Sorts cached `DividendRecord` rows
(from ADR-079 DVD) into calendar-year buckets, excludes the incomplete
...
STABLE. Source: ADR-079 DIVG window.

#### 2.49 Earnings Momentum Trend (EARM — ADR-079)

Pulled from `research::get_earm`. Fuses cached `FA.income_quarterly`
(from ADR-079 FA) with cached `EarningsSurprise` history (from
ADR-079 ERN) to compute a **0-100 composite momentum score**.
...
cached surprise history. Source: ADR-079 EARM window.

#### 2.50 Sector Rotation Strength (SECTR — ADR-079)

Pulled from `research::get_sector_rotation`. Ranks the symbol's
Fundamentals sector among cached `SectorPerformance` rows (from
ADR-079 INDU) and derives a relative strength label. Header line
...
strength < 0. NEUTRAL otherwise. Source: ADR-079 SECTR window.

#### 2.51 Upgrade/Downgrade Momentum (UPDM — ADR-079)

Pulled from `research::get_updm`. Buckets cached `RatingChange` rows
(from ADR-079 UPDG) into **30d / 90d / 180d** windows and classifies
...
-net_90d / 3, else STABLE. Source: ADR-079 UPDM window.

#### 2.52 Momentum 12-1 (MOM — ADR-079)
...
Source: ADR-079 MOM window.

#### 2.53 Liquidity Profile (LIQ — ADR-079)
...
MODERATE ≥$5M, THIN ≥$500K, ILLIQUID below. Source: ADR-079 LIQ
window.

#### 2.54 Breakout Proximity (BREAK — ADR-079)
...
Source: ADR-079 BREAK window.

#### 2.55 Cash Conversion Cycle (CCRL — ADR-079)
...
DETERIORATING when ≥ +5, STABLE otherwise. Source: ADR-079 CCRL
window.
...
none of ALTZ / PTFS / LEV / ACRL is cached. Source: ADR-079 CREDIT
window.
...
of MOM / EARM / DIVG are cached. Source: ADR-079 GROWM window.

#### 2.58 Smart-Money Flow (FLOW — ADR-079)
...
single-side when only one. Source: ADR-079 FLOW window.
...
Source: ADR-079 REGIME window.

#### 2.60 Relative Volume (RELVOL — ADR-079)
...
bar to prevent self-skew. Source: ADR-079 RELVOL window.

#### 2.61 Margin Trajectory (MARGINS — ADR-079)
...
cache is empty. Source: ADR-079 MARGINS window.

#### 2.62 Value-Factor Composite (VAL — ADR-079)
...
each populated component. Source: ADR-079 QUAL window.
...
(up to **5 rows**) lists each populated component. Source: ADR-079
RISK window.

#### 2.65 Insider Streak Detector (INSSTRK — ADR-079)
...
the composite down. Source: ADR-079 COVG window.
...
BOTTOM_QUARTILE ≥10, BOTTOM_DECILE <10. Source: ADR-079 VRK window.
...
scores, and peers_considered / with_data. Source: ADR-079 QRK window.
...
sector cross-join pattern as QRK. Source: ADR-079 RRK window.

#### 2.70 Relative EPS Growth (RELEPSGR — ADR-079)
...
proxy is used for `symbol_cagr_pct`. Source: ADR-079 RELEPSGR window.

#### 2.71 Post-Earnings Drift (PEAD — ADR-079)
...
±1 % surprise threshold. Source: ADR-079 PEAD window.

#### 2.72 Size Factor (SIZEF — ADR-079)
...
#### 2.74 PEAD Rank (PEADRANK — ADR-079)
...
events_used >= 3`. Source: ADR-079 PEADRANK window.

#### 2.75 Fundamental Quality Meter (FQM — ADR-079)
...
cached. Source: ADR-079 FQM window.

#### 2.76 Relative Revenue Growth (REVRANK — ADR-079)
...
for symmetry with RELEPSGR). Source: ADR-079 REVRANK window.
...
ADR-079 OPERANK window.

#### 2.79 FQM Rank (FQMRANK — ADR-079)
...
Source: ADR-079 LIQRANK window.

#### 2.81 Earnings Surprise Streak (SURPSTK — ADR-079)
...
move out of INSUFFICIENT_DATA. Source: ADR-079 SURPSTK window.
...
peers_with_data. Source: ADR-079 DVDRANK window.
...
`momentum_label != "INSUFFICIENT_DATA"`. Source: ADR-079 EARMRANK
window.
...
sell-side-active names only. Source: ADR-079 UPDGRANK window.

#### 2.85 Gap Yearly (GY — ADR-079)
...
#### 2.86 Daily Event Streak (DES — ADR-079)
...
Source: ADR-079 DES window.

#### 2.87 Dividend Yield Rank (DVDYIELDRANK — ADR-079)
...
Requires ≥3 sector peers paying dividends. Source: ADR-079
DVDYIELDRANK window.

#### 2.88 Short Interest Rank (SHRANK — ADR-079)
...
sector peers with short interest data. Source: ADR-079
SHRANK window.

#### 2.89 Annualized ATR (ATRANN — ADR-079)
...
ATR14 %, and annualized %. Complements IVOL (implied vol, ADR-079)
with a realized-vol regime surface that works without options data.
Requires ≥15 HP bars. Source: ADR-079 ATRANN window.

#### 2.90 Drawdown History (DDHIST — ADR-079)
...
≥10%, and current dd %. Requires ≥20 HP bars. Source: ADR-079 DDHIST
window.

#### 2.91 Price Performance (PRICEPERF — ADR-079)
...
ADR-079 PRICEPERF window.

#### 2.92 Beta Rank vs Sector (BETARANK — ADR-079)
...
≥3 sector peers with a non-None beta. Source: ADR-079 BETARANK
window. Negative-beta subjects correctly land at SAFEST_DECILE
against conventional positive-beta peers.

#### 2.93 PEG Ratio Rank vs Sector (PEGRANK — ADR-079)
...
filtered on both subject and peer sides. Source: ADR-079 PEGRANK
window.

#### 2.94 52-Week High/Low Distance (FHIGHLOW — ADR-079)
...
mid-range (50.0) on degenerate flat-series case. Source: ADR-079
FHIGHLOW window.

#### 2.95 Realized Volatility Cone (RVCONE — ADR-079)
...
its *history*. Source: ADR-079 RVCONE window.

#### 2.96 Calendar Period Breakdowns (CALPB — ADR-079)
...
year. Source: ADR-079 CALPB window.

#### 2.97 Return Distribution Skewness (RETSKEW — ADR-079)
...
up and down moves in the window. Source: ADR-079 RETSKEW window.

#### 2.98 Return Distribution Excess Kurtosis (RETKURT — ADR-079)
...
Source: ADR-079 RETKURT window.

#### 2.99 Tail Ratio (TAILR — ADR-079)
...
ratio. Source: ADR-079 TAILR window.

#### 2.100 Run Length (RUNLEN — ADR-079)
...
counts, and the signed current run. Source: ADR-079 RUNLEN window.

#### 2.101 Daily Range (DAYRANGE — ADR-079)
...
Source: ADR-079 DAYRANGE window.

#### 2.102 Return Autocorrelation (AUTOCOR — ADR-079)
...
for whether "yesterday predicts today." Source: ADR-079 AUTOCOR
window.

#### 2.103 Hurst Exponent (HURST — ADR-079)
...
classic fat-tailed regime). Source: ADR-079 HURST window.

#### 2.104 Multi-Horizon Hit Rate (HITRATE — ADR-079)
...
Source: ADR-079 HITRATE window.

#### 2.105 Gain/Loss Asymmetry (GLASYM — ADR-079)
...
ratio, and the up/down day counts. Source: ADR-079 GLASYM window.

#### 2.106 Up/Down Volume Ratio (VOLRATIO — ADR-079)
...
trailing 253-session window. Upside mirror of DDHIST (ADR-079):
...
peak-trough-peak-trough history view. Source: ADR-079 DRAWUP window.

#### 2.108 Overnight Gap Statistics (GAPSTATS — ADR-079)
...
ADR-079 GAPSTATS window.

#### 2.109 Volatility Clustering (VOLCLUSTER — ADR-079)
...
1 / 5 / 20. Complements AUTOCOR (ADR-079) one-to-one: return ACF
...
follow big moves"). Source: ADR-079 VOLCLUSTER window.

#### 2.110 Close Placement (CLOSEPLC — ADR-079)
...
by-zero. Source: ADR-079 CLOSEPLC window.

#### 2.111 Mean-Reversion Half-Life (MRHL — ADR-079)
...
not occur on stationary log returns). Source: ADR-079 MRHL window.

#### 2.112 Downside Deviation / Sortino (DOWNVOL — ADR-079)
...
ADR-079 DOWNVOL window.

#### 2.113 Sharpe Ratio (SHARPR — ADR-079)
...
ADR-079 SHARPR window.

#### 2.114 Kaufman Efficiency Ratio (EFFRATIO — ADR-079)
...
ADR-079 EFFRATIO window.

#### 2.115 Wick Bias (WICKBIAS — ADR-079)
...
ADR-079 WICKBIAS window.

#### 2.116 Vol-of-Vol (VOLOFVOL — ADR-079)
...
*variability*). Source: ADR-079 VOLOFVOL window.

#### 2.117 Calmar Ratio (CALMAR — ADR-079)
...
rising series (zero drawdown) gets EXCELLENT. Source: ADR-079 CALMAR window.

#### 2.118 Ulcer Index + Martin Ratio (ULCER — ADR-079)
...
HIGH <20 / SEVERE ≥20 / INSUFFICIENT_DATA). Source: ADR-079 ULCER window.

#### 2.119 Lo-MacKinlay Variance Ratio (VARRATIO — ADR-079)
...
Requires ≥40 log returns. Source: ADR-079 VARRATIO window.

#### 2.120 Amihud Illiquidity (AMIHUD — ADR-079)
...
volume). Source: ADR-079 AMIHUD window.

#### 2.121 Jarque-Bera Normality Test (JBNORM — ADR-079)
...
surface in the packet to report an explicit p-value. Source: ADR-079
JBNORM window.

#### 2.122 Omega Ratio at τ=0 (OMEGA — ADR-079)
...
loss_days, win_rate_pct. Source: ADR-079 OMEGA window.

#### 2.123 Detrended Fluctuation Analysis (DFA — ADR-079)
...
enough to invalidate R/S. Source: ADR-079 DFA window.

#### 2.124 Burke Ratio (BURKE — ADR-079)
...
episodes), sum_sq_drawdowns, worst_event_dd_pct. Source: ADR-079
BURKE window.

#### 2.125 Monthly Seasonality (MONTHSEAS — ADR-079)
...
calendar effects vs its peer set. Source: ADR-079 MONTHSEAS window.

#### 2.126 Roll Implicit Bid-Ask Spread (ROLLSPRD — ADR-079)
...
implicit_spread_bps. Source: ADR-079 ROLLSPRD window.

#### 2.127 Parkinson H-L Volatility (PARKINSON — ADR-079)
...
Source: ADR-079 PARKINSON window.

#### 2.128 Garman-Klass OHLC Volatility (GKVOL — ADR-079)
...
two contributions visible separately). Source: ADR-079 GKVOL
window.

#### 2.129 Rogers-Satchell Drift-Free Volatility (RSVOL — ADR-079)
...
daily_vol_pct and annualized_vol_pct. Source: ADR-079 RSVOL
window.

#### 2.130 Conditional VaR / Expected Shortfall (CVAR — ADR-079)
...
#### 2.131 Day-of-Week Seasonality (DOWEFFECT — ADR-079)
...
covered. Source: ADR-079 DOWEFFECT window.

#### 2.132 Sterling Ratio (STERLING — ADR-079)
...
mean_worst_dd_pct, and sterling_ratio. Source: ADR-079 STERLING
window.

#### 2.133 Kelly Fraction (KELLYF — ADR-079)
...
half_kelly. Source: ADR-079 KELLYF window.

#### 2.134 Ljung-Box Joint Autocorrelation (LJUNGB — ADR-079)
...
documented in ADR-079. Source: ADR-079 LJUNGB window.

#### 2.135 Wald-Wolfowitz Runs Test (RUNSTEST — ADR-079)
...
standard normal CDF. Source: ADR-079 RUNSTEST window.

#### 2.136 Zero-Return-Day Fraction (ZERORET — ADR-079)
...
longest_zero_streak, and epsilon. Source: ADR-079 ZERORET window.

#### 2.137 Probabilistic Sharpe Ratio (PSR — ADR-079)
...
Source: ADR-079 PSR window.

#### 2.138 Dickey-Fuller Unit-Root Test (ADF — ADR-079)
...
critical values, and reject_unit_root. Source: ADR-079 ADF
window.

#### 2.139 Mann-Kendall Trend Test (MNKENDALL — ADR-079)
...
reject_no_trend. Source: ADR-079 MNKENDALL window.

#### 2.140 Bipower Variation / Jump Ratio (BIPOWER — ADR-079)
...
ratio, and jump %. Source: ADR-079 BIPOWER window.

#### 2.141 Drawdown Duration Statistics (DDDUR — ADR-079)
...
(if a drawdown is still open at window end). Source: ADR-079
DDDUR window.

#### 2.142 Hill Tail-Index Estimator (HILLTAIL — ADR-079)
...
ADR-079 HILLTAIL window.

#### 2.143 Engle ARCH-LM Test (ARCHLM — ADR-079)
...
critical values, and `reject_homoskedastic`. Source: ADR-079
ARCHLM window.

#### 2.144 Pain Index + Pain Ratio (PAINRATIO — ADR-079)
...
ADR-079 PAINRATIO window.

#### 2.145 Brown-Durbin-Evans CUSUM Break Test (CUSUM — ADR-079)
...
`reject_stability`. Source: ADR-079 CUSUM window.

#### 2.146 Cornish-Fisher Modified VaR (CFVAR — ADR-079)
...
skew_term_5pct, and kurt_term_5pct. Source: ADR-079 CFVAR window.

#### 2.147 Shannon Return Entropy (ENTROPY — ADR-079)
...
normalised_entropy. Source: ADR-079 ENTROPY window.

#### 2.148 Rachev Ratio (RACHEV — ADR-079)
...
rachev_1pct. Source: ADR-079 RACHEV window.

#### 2.149 Gain-to-Pain Ratio (GPR — ADR-079)
...
profit_factor, win_count, and loss_count. Source: ADR-079 GPR window.

#### 2.150 Partial Autocorrelation (PACF — ADR-079)
...
Source: ADR-079 PACF window.

#### 2.151 Approximate Entropy (APEN — ADR-079)
...
phi_m, phi_m1, and apen. Source: ADR-079 APEN window.

#### 2.152 Upside Potential Ratio (UPR — ADR-079)
...
Source: ADR-079 UPR window.

#### 2.153 Leverage Effect (LEVEREFF — ADR-079)
...
Source: ADR-079 LEVEREFF window.

#### 2.154 Drawdown-at-Risk (DRAWDAR — ADR-079)
...
Source: ADR-079 DRAWDAR window.

#### 2.155 Volatility Half-Life (VARHALF — ADR-079)
...
Source: ADR-079 VARHALF window.

#### 2.156 Return Gini Coefficient (GINI — ADR-079)
...
max_abs_ret, min_abs_ret. Source: ADR-079 GINI window.

#### 2.157 Sample Entropy (SAMPEN — ADR-079)
...
INSUFFICIENT_DATA). Body reports bars_used, embed_dim, tolerance,
a_count, b_count, and sampen. Source: ADR-079 SAMPEN window.

#### 2.158 Permutation Entropy (PERMEN — ADR-079)
...
permen_raw, permen_normalised. Source: ADR-079 PERMEN window.

#### 2.159 Recovery Factor (RECFACT — ADR-079)
...
recovery_factor. Source: ADR-079 RECFACT window.

#### 2.160 KPSS Stationarity Test (KPSS — ADR-079)
...
Source: ADR-079 KPSS window.

#### 2.161 Spectral Entropy (SPECENT — ADR-079)
...
Source: ADR-079 SPECENT window.

#### 2.162 Robust Volatility (ROBVOL — ADR-079)
...
iqr_sigma, mad_ratio, iqr_ratio. Source: ADR-079 ROBVOL window.

#### 2.163 Rényi Entropy α=2 (RENYIENT — ADR-079)
...
Source: ADR-079 RENYIENT window.

#### 2.164 Return Quantile Profile (RETQUANT — ADR-079)
...
and tail_asymmetry. Source: ADR-079 RETQUANT window.

#### 2.165 Multiscale Entropy (MSENT — ADR-079)
...
Source: ADR-079 MSENT window.

#### 2.166 EWMA Volatility (EWMAVOL — ADR-079)
...
Source: ADR-079 EWMAVOL window.

#### 2.167 Kolmogorov-Smirnov Normality Test (KSNORM — ADR-079)
...
sigma.
Source: ADR-079 KSNORM window.

#### 2.168 Anderson-Darling Normality Test (ADTEST — ADR-079)
...
Source: ADR-079 ADTEST window.

#### 2.169 L-Moments (LMOM — ADR-079)
...
Source: ADR-079 LMOM window.

#### 2.170 Kyle's Price-Impact λ (KYLELAM — ADR-079)
...
Source: ADR-079 KYLELAM window.

#### 2.171 Peaks-Over-Threshold (PEAKOVER — ADR-079)
...
Source: ADR-079 PEAKOVER window.

#### 2.172 Higuchi Fractal Dimension (HIGUCHI — ADR-079)
...
estimator. Source: ADR-079 HIGUCHI window.

#### 2.173 Pickands Tail-Index (PICKANDS — ADR-079)
...
ADR-079 PICKANDS window.

#### 2.174 Kappa-3 Ratio (KAPPA3 — ADR-079)
...
lpm3, lpm3_root, kappa3, sortino_compare. Source: ADR-079 KAPPA3
window.

#### 2.175 Largest Lyapunov Exponent (LYAPUNOV — ADR-079)
...
Source: ADR-079 LYAPUNOV window.

#### 2.176 Spearman Rank Autocorrelation (RANKAC — ADR-079)
...
heavy-tail-driven spurious Pearson ACF signal. Source: ADR-079
RANKAC window.

#### 2.177 Barndorff-Nielsen-Shephard Jump Test (BNSJUMP — ADR-079)
...
alongside ADF (ADR-079) and KPSS (ADR-079). Phillips & Perron (1988)
...
Source: ADR-079 PPROOT window.

#### 2.179 Multifractal DFA (MFDFA — ADR-079)
...
Source: ADR-079 MFDFA window.

#### 2.180 Hill-Tail KS Goodness-of-Fit (HILLKS — ADR-079)
...
Pareto. Source: ADR-079 HILLKS window.

#### 2.181 True Strength Index (TSI — ADR-079)
...
#### 2.182 GARCH(1,1) Fit (GARCH11 — ADR-079)
...
standard 2-parameter decomposition. Source: ADR-079 GARCH11 window.

#### 2.183 Sup-ADF Bubble Test (SADF — ADR-079)
...
#### 2.184 Correlation Dimension (CORDIM — ADR-079)
...
ADR-079 CORDIM window.

#### 2.185 Rolling Skewness Spectrum (SKSPEC — ADR-079)
...
RETQUANT (ADR-079) which ships full-window skew — SKSPEC says
whether that number is reliable. Source: ADR-079 SKSPEC window.

#### 2.186 Auto Mutual Information (AUTOMI — ADR-079)
...
ADR-079 AUTOMI window.

#### 2.187 Durbin-Watson Autocorrelation (DURBINWATSON — ADR-079)
...
tradition. Source: ADR-079 DURBINWATSON window.

#### 2.188 BDS iid Test (BDSTEST — ADR-079)
...
Variance approximation is a simplified upper bound (see ADR-079); a
...
Source: ADR-079 BDSTEST window.

#### 2.189 Breusch-Pagan Heteroskedasticity (BREUSCHPAGAN — ADR-079)
...
(ADR-079) which tests autoregressive conditional heteroskedasticity;
BREUSCHPAGAN catches monotonic trends in variance that ARCHLM misses.
Source: ADR-079 BREUSCHPAGAN window.

#### 2.190 Bartels Turning-Points Test (TURNPTS — ADR-079)
...
catches different iid failure modes. Source: ADR-079 TURNPTS window.

#### 2.191 Direct-DFT Periodogram (PERIODOGRAM — ADR-079)
...
Source: ADR-079 PERIODOGRAM window.

#### 2.192 McLeod-Li Squared-Returns Portmanteau (MCLEODLI — ADR-079)
...
Source: ADR-079 MCLEODLI window.

#### 2.193 Ornstein-Uhlenbeck Mean-Reversion Fit (OUFIT — ADR-079)
...
Source: ADR-079 OUFIT window.

#### 2.194 Geweke-Porter-Hudak Long-Memory d̂ (GPH — ADR-079)
...
angles. Source: ADR-079 GPH window.

#### 2.195 Burg Maximum-Entropy AR Spectrum (BURGSPEC — ADR-079)
...
Source: ADR-079 BURGSPEC window.

#### 2.196 Kendall's Tau Lag-1 Rank Autocorrelation (KENDALLTAU — ADR-079)
...
and RANKAC's Spearman lag. Source: ADR-079 KENDALLTAU window.

#### 2.197 Composite Short-Squeeze Score (SQUEEZE — ADR-079)
...
and inputs_present (0..5). Source: ADR-079 SQUEEZE window.

#### 2.198 Cross-Symbol Short-Squeeze Rank (SQUEEZERANK — ADR-079)
...
when the sector tape is quiet versus running hot. Source: ADR-079 SQUEEZERANK.

#### 2.199 Bollinger Band Width Squeeze (BBSQUEEZE — ADR-079)
...
ADR-079 BBSQUEEZE window.

#### 2.200 Donchian Channel Breakout (DONCHIAN — ADR-079)
...
ADR-079 DONCHIAN window.

#### 2.201 Kaufman Adaptive Moving Average (KAMA — ADR-079)
...
Source: ADR-079 KAMA window.

#### 2.202 Ichimoku Kinkō Hyō Cloud (ICHIMOKU — ADR-079)
...
Source: ADR-079 ICHIMOKU window.

#### 2.203 SuperTrend ATR-Channel Overlay (SUPERTREND — ADR-079)
...
DONCHIAN's pure N-bar envelope breakout. Source: ADR-079 SUPERTREND.

#### 2.204 Keltner Channel + TTM-Squeeze (KELTNER — ADR-079)
...
pairs KELTNER + BBSQUEEZE (ADR-079) for the canonical John Carter
construct. Source: ADR-079 KELTNER window.

#### 2.205 Ehlers Fisher Transform (FISHER — ADR-079)
...
complementary oscillator to TSI. Source: ADR-079 FISHER window.

#### 2.206 Aroon Oscillator (AROON — ADR-079)
...
trends earlier than strength-based indicators. Source: ADR-079 AROON
window.

#### 2.207 Wilder's Average Directional Index (ADX — ADR-079)
...
Complements AROON (ADR-079) which measures *time-since-extreme* — ADX
measures *strength regardless of time*. Source: ADR-079 ADX window.

#### 2.208 Lambert Commodity Channel Index (CCI — ADR-079)
...
one-sided slow grinds register different extremes. Source: ADR-079 CCI window.

#### 2.209 Chaikin Money Flow (CMF — ADR-079)
...
NaN. Source: ADR-079 CMF window.

#### 2.210 Money Flow Index (MFI — ADR-079)
...
oscillator. Source: ADR-079 MFI window.

#### 2.211 Wilder Parabolic Stop-And-Reverse (PSAR — ADR-079)
...
SUPERTREND (ADR-079): PSAR accelerates (AF grows each new EP),
SuperTrend is ATR-proportional and does not accelerate — so PSAR fires
trailing-stop exits earlier in mature trends. Source: ADR-079 PSAR window.

#### 2.212 Botes & Siepman Vortex Indicator (VORTEX — ADR-079)
...
sum_vm_minus, last_close. Complements ADX (ADR-079): ADX is Wilder-smoothed
and lagged, VORTEX is unsmoothed and catches direction changes earlier.
Source: ADR-079 VORTEX window.

#### 2.213 Bill Dreiss Choppiness Index (CHOP — ADR-079)
...
than NaN. Source: ADR-079 CHOP window.

#### 2.214 Granville On-Balance Volume (OBV — ADR-079)
...
obv_max_20, last_close. Complements CMF (ADR-079): CMF is bounded
...
so OBV can plateau during halts. Source: ADR-079 OBV window.

#### 2.215 Hutson Triple-EMA Rate-of-Change (TRIX — ADR-079)
...
rejection, more lag. Source: ADR-079 TRIX window.

#### 2.216 Hull Moving Average (HMA — ADR-079)
...
hma_vs_close_pct, last_close. Complements SMA/EMA/KAMA (ADR-079): HMA
is the zero-lag-by-construction member of the MA family. We floor √n
(TradingView convention). Source: ADR-079 HMA window.

#### 2.217 Percentage Price Oscillator (PPO — ADR-079)
...
basket directly comparable. Source: ADR-079 PPO window.

#### 2.218 Detrended Price Oscillator (DPO — ADR-079)
...
Source: ADR-079 DPO window.

#### 2.219 Know Sure Thing (KST — ADR-079)
...
oscillator in the packet. Source: ADR-079 KST window.

#### 2.220 Ultimate Oscillator (ULTOSC — ADR-079)
...
RSI/Stochastic. Source: ADR-079 ULTOSC window.

#### 2.221 Williams %R (WILLR — ADR-079)
...
Stochastic's 20/80. Source: ADR-079 WILLR window.

#### 2.222 Mass Index (MASS — ADR-079)
...
oscillators (MACD/PPO/KST). Source: ADR-079 MASS window.

#### 2.223 Chaikin Oscillator (CHAIKOSC — ADR-079)
...
up even when the cumulative line trends slowly. Source: ADR-079
CHAIKOSC window.

#### 2.224 Klinger Volume Oscillator (KLINGER — ADR-079)
...
Source: ADR-079 KLINGER window.

#### 2.225 Stochastic RSI (STOCHRSI — ADR-079)
...
RSI clusters stuck in [40, 60]. Source: ADR-079 STOCHRSI window.

#### 2.226 Awesome Oscillator (AWESOME — ADR-079)
...
oscillators' signals. Source: ADR-079 AWESOME window.

#### 2.227 Force Index (EFI — ADR-079)
...
#### 2.228 Ease of Movement (EMV — ADR-079)
...
volume-efficiency, not direction. Source: ADR-079 EMV window.

#### 2.229 Negative Volume Index (NVI — ADR-079)
...
Source: ADR-079 NVI window.

#### 2.230 Positive Volume Index (PVI — ADR-079)
...
#### 2.231 Coppock Curve (COPPOCK — ADR-079)
...
interpret relative to tape granularity. Source: ADR-079 COPPOCK
window.

#### 2.232 Chande Momentum Oscillator (CMO — ADR-079)
...
raw gain/loss spread surface we ship. Source: ADR-079 CMO window.

#### 2.233 Q-Stick (QSTICK — ADR-079)
...
position). Source: ADR-079 QSTICK window.

#### 2.234 Disparity Index (DISPARITY — ADR-079)
...
we ship. Source: ADR-079 DISPARITY window.

#### 2.235 Balance of Power (BOP — ADR-079)
...
last_close. Source: ADR-079 BOP window.

#### 2.236 Schaff Trend Cycle (SCHAFF — ADR-079)
...
chain. Source: ADR-079 SCHAFF window.

#### 2.237 Stochastic Oscillator (STOCH — ADR-079)
...
directly on price. Source: ADR-079 STOCH window.

#### 2.238 MACD — Appel (MACD — ADR-079)
...
histogram_prev, last_close. Source: ADR-079 MACD window.

#### 2.239 Volume-Weighted Average Price (VWAP — ADR-079)
...
data). Source: ADR-079 VWAP window.

#### 2.240 McGinley Dynamic (MCGD — ADR-079)
...
the repo. Source: ADR-079 MCGD window.

#### 2.241 Random Walk Index (RWI — ADR-079)
...
Source: ADR-079 RWI window.

#### 2.242 Double Exponential Moving Average (DEMA — ADR-079)
...
deviation_pct, last_close. Complements MCGD (ADR-079, adaptive-by-
feedback) and KAMA (ADR-079, adaptive-by-efficiency-ratio) on the
"reduced-lag MA" axis: DEMA reduces lag *algebraically* (subtracting
the lag term) rather than adaptively. Source: ADR-079 DEMA window.

#### 2.243 Triple Exponential Moving Average (TEMA — ADR-079)
...
price shocks. Distinct from TRIX (ADR-079, *rate-of-change* of
triple EMA) — TEMA is a price level MA, TRIX is an oscillator
derived from the same triple-EMA chain. Source: ADR-079 TEMA window.

#### 2.244 Linear Regression Channel (LINREG — ADR-079)
...
channel when R² is low. Complements BBSQUEEZE (ADR-079) and
DONCHIAN (ADR-079) on the channel/envelope axis. Source: ADR-079
LINREG window.

#### 2.245 Floor-Trader Pivot Points (PIVOTS — ADR-079)
...
SUPERTREND (ADR-079, ATR-channel), DONCHIAN (ADR-079, N-bar H/L),
and BBSQUEEZE (ADR-079, σ-envelope): PIVOTS is a *prior-bar-
derived fixed grid* — no moving averages, no volatility scaling,
just the canonical floor-pit arithmetic. Source: ADR-079 PIVOTS
window.

#### 2.246 Heikin-Ashi Candle (HEIKIN — ADR-079)
...
#### 2.247 Arnaud Legoux Moving Average (ALMA — ADR-079)
...
deviation_pct, last_close. Distinct from HMA (ADR-081, sqrt-WMA
lag), DEMA/TEMA (ADR-079, algebraic lag subtraction), KAMA
(ADR-079, efficiency-adaptive), MCGD (ADR-079, feedback-adaptive):
ALMA reduces lag by *peak-biased Gaussian kernel placement*. The
`offset` and `sigma` fields are exposed so the AI can read the
shape rather than guessing from the name. Source: ADR-079 ALMA
window.

#### 2.248 Zero-Lag EMA (ZLEMA — ADR-079)
...
DEMA (ADR-079, **second-order algebraic** lag subtraction on the
...
zlema_prev, deviation_pct, last_close. Source: ADR-079 ZLEMA
window.

#### 2.249 Elder Ray — Bull/Bear Power (ELDERRAY — ADR-079)
...
intensity surface in the packet: unlike BOP (ADR-079, per-bar
close-vs-range conviction) or Williams %R (ADR-079, N-bar close-
...
bear_power, bear_power_prev, last_close. Source: ADR-079 ELDERRAY
window.

#### 2.250 Time Series Forecast (TSF — ADR-079)

Pulled from `research::get_tsf`. TSF extends the existing LINREG
(ADR-079, OLS fit at `t = N−1`) with a **forward projection** to
...
fit. Source: ADR-079 TSF window.

#### 2.251 Relative Vigor Index (RVI — ADR-079)
...
to the range denominator. Distinct from BOP (ADR-079, single-bar
close−open/range with no smoothing), from Stochastic (ADR-079,
...
signal_value, signal_prev, last_close. Source: ADR-079 RVI window.

#### 2.252 Triangular Moving Average (TRIMA — ADR-079)
...
trima_prev, deviation_pct, last_close. Source: ADR-079 TRIMA
window.

#### 2.253 Tillson T3 Moving Average (T3 — ADR-079)
...
last_close. Source: ADR-079 T3 window.

#### 2.254 Variable Index Dynamic Average (VIDYA — ADR-079)
...
deviation_pct, last_close. Source: ADR-079 VIDYA window.

#### 2.255 Stochastic Momentum Index (SMI — ADR-079)
...
smi_prev, signal_value, signal_prev, last_close. Source: ADR-079
SMI window.

#### 2.256 Price Volume Trend (PVT — ADR-079)
...
Unlike OBV (ADR-079, sign-of-Δclose times volume) which attributes
...
from CHAIKOSC (ADR-079, Accumulation/Distribution Line's MACD-
...
pvt_prev, pvt_ema, pvt_slope, last_close. Source: ADR-079 PVT
window.

#### 2.257 Accelerator Oscillator (AC — ADR-079)
...
where `medprice = (high + low) / 2`. Where AO (ADR-079) measures
...
(Alligator ADR-079 + AO ADR-079 + AC). Conceptually analogous to
...
ao_sma5 (SMA₅ of AO), last_close. Source: ADR-079 AC window.

#### 2.258 Chaikin Volatility (CHVOL — ADR-079)
...
pre-breakout). Distinct from ATR (ADR-079, exponential smoothing
of true range — a *level* measure), BBWIDTH (below, stddev-based
bandwidth), and Volatility Regime (ADR-079, realized-vol term
...
ema_range (current EMA of H−L), last_close. Source: ADR-079
CHVOL window.

#### 2.259 Bollinger Bandwidth (BBWIDTH — ADR-079)
...
(ADR-079, which compares Bollinger Bandwidth to Keltner Channel
...
upper, lower, last_close. Source: ADR-079 BBWIDTH window.

#### 2.260 Elder Impulse System (ELDERIMP — ADR-079)
...
Elder Ray (ADR-079, bull-power and bear-power oscillators
...
macd_hist_prev, macd_hist_slope, last_close. Source: ADR-079
ELDERIMP window.

#### 2.261 Relative Momentum Index (RMI — ADR-079)
...
Source: ADR-079 RMI window.

#### 2.262 Options Expiration Calendar (EXPCAL — ADR-084)
...
strike-oriented OPTIONS chain view (ADR-079) which lists every
...
Source: ADR-084 EXPCAL window (Tier 2 tab).

#### 2.263 Wilder Smoothed MA (SMMA — ADR-079)
...
last_close. Source: ADR-079 SMMA window.

#### 2.264 Bill Williams Alligator (ALLIGATOR — ADR-079)
...
Williams Chaos Theory trio alongside AO (ADR-079) and AC
(ADR-079). Distinct from Fractals (peak/trough markers, separate
surface). Requires n≥23; body reports jaw/teeth/lips (current and
prior-bar), spread_pct, last_close. Source: ADR-079 ALLIGATOR
window.

#### 2.265 Connors RSI (CRSI — ADR-079)
...
crsi_value, crsi_prev, last_close. Source: ADR-079 CRSI window.

#### 2.266 Standard Error Bands (SEB — ADR-079)
...
bandwidth, position_pct, last_close. Source: ADR-079 SEB window.

#### 2.267 Intraday Momentum Index (IMI — ADR-079)
...
sum_losses, imi_value, imi_prev, last_close. Source: ADR-079
IMI window.

#### 2.268 Guppy Multiple Moving Average (GMMA — ADR-079)
...
ADR-079), from every single-MA surface, and from any dual-MA
crossover. Requires n≥62; body reports short_ema_avg, long_ema_avg,
short_min/max, long_min/max, short_compression_pct,
long_compression_pct, group_gap_pct, last_close. Source: ADR-079
GMMA window.

#### 2.269 Moving Average Envelope (MAENV — ADR-079)
...
ADR-079), Keltner (ATR-based, ADR-079), Donchian (rolling high/low,
ADR-079), SEB (regression-residual, ADR-079), and STARC (SMA ± k·ATR):
...
bandwidth_pct (5.0), position_pct, last_close. Source: ADR-079
MAENV window.

#### 2.270 Chaikin Accumulation/Distribution Line (ADL — ADR-079)
...
range-agnostic), CMF (ranged ratio over N bars, ADR-079), KLINGER
(dual-EMA transformation of volume force, ADR-079), PVT (ROC·volume
rather than MFM·volume, ADR-079), and from the Chaikin A/D Oscillator
(CHAIKOSC, difference of EMAs on ADL, ADR-079). Reports ADL,
...
slope_per_bar, last_close, price_delta_pct. Source: ADR-079 ADL
window.

#### 2.271 Vertical Horizontal Filter (VHF — ADR-079)
...
ADR-079), CHOP (log10 of range/sum-of-TR, ADR-079), AROON
(positional HHV/LLV timing, ADR-079), and VI (Vortex, ADR-079).
...
last_close. Source: ADR-079 VHF window.

#### 2.272 Volume Rate of Change (VROC — ADR-079)
...
RelVol (current-vs-long-horizon average, ADR-079), NVol
(current-vs-20-day median, ADR-081), and the price-based ROC
(ADR-079). VROC is the "has volume accelerated?" gauge used in
...
vroc_prev, last_close. Source: ADR-079 VROC window.

#### 2.273 KDJ Chinese Stochastic (KDJ — ADR-079)
...
base as Stochastic (ADR-079) with canonical N=9, but with EMA₁/₃
...
from STOCH (simple MA smoothing, ADR-079), STOCHF (no smoothing),
and STOCHRSI (ADR-079). OVERBOUGHT / BULL / NEUTRAL / BEAR /
...
j_prev, last_close. Source: ADR-079 KDJ window.

#### 2.274 Quantitative Qualitative Estimation (QQE — ADR-079)
...
(ADR-079), STOCHRSI (ADR-079), CRSI (Connors composite, ADR-079),
and RVI (ADR-079). STRONG_BULL / BULL / NEUTRAL / BEAR /
...
last_close. Source: ADR-079 QQE window.

#### 2.275 Price Momentum Oscillator (PMO — ADR-079)
...
close, ADR-079), TRIX (triple-smoothed EMA of close, ADR-079), and
PPO (ADR-079); PMO is the only canonical smoothed-ROC-plus-signal
...
last_close. Source: ADR-079 PMO window.

#### 2.276 Chande Forecast Oscillator (CFO — ADR-079)
...
ADR-079), TSF (projected future value, ADR-079), DPO (ADR-079),
and PPO (non-regression momentum, ADR-079). CFO is the one
...
cfo_prev, last_close. Source: ADR-079 CFO window.

#### 2.277 Twiggs Money Flow (TMF — ADR-079)

Pulled from `research::get_tmf`. Colin Twiggs's smoothed,
volume-weighted variant of Chaikin Money Flow (ADR-079). Replaces
...
denominator (volume). Distinct from CMF (ADR-079), ADL (ADR-079),
KLINGER (ADR-079), PVT (ADR-079), and CHAIKOSC (ADR-079).
...
Source: ADR-079 TMF window.

#### 2.278 Bill Williams Fractals (FRACTALS — ADR-079)
...
(ADR-079). Distinct from ZigZag (percent-move threshold) and Pivot
...
last_close. Source: ADR-079 FRACTALS window.

#### 2.279 Ehlers Inverse Fisher Transform of RSI (IFT_RSI — ADR-079)

Pulled from `research::get_ift_rsi`. Rescales RSI (ADR-079) to
...
alerts. Distinct from raw RSI (ADR-079), STOCHRSI (ADR-079), QQE
(ADR-079), and CRSI (ADR-079). STRONG_BULL / BULL / NEUTRAL /
BEAR / STRONG_BEAR labels. Requires n≥25; body reports
rsi_length (14), wma_length (9), rsi_value, v_value, ift_value,
ift_prev, last_close. Source: ADR-079 IFT_RSI window.

#### 2.280 MESA Adaptive Moving Average (MAMA — ADR-079)
...
adaptive, ADR-079), T3 (Tillson triple-DEMA, ADR-079), VIDYA
(Chande volatility-index DMA, ADR-081), and every fixed-α EMA.
...
Source: ADR-079 MAMA window.

#### 2.281 Ehlers Center of Gravity (COG — ADR-079)
...
PMO, ADR-079), LINREG-based (LINREG/CFO), and simple ROC.
STRONG_BULL / BULL / NEUTRAL / BEAR / STRONG_BEAR labels derived
from COG minus signal. Requires n≥14; body reports length (10),
cog_value, cog_signal (3-bar lag), cog_prev, last_close.
Source: ADR-079 COG window.

#### 2.282 Didi Aguiar Didi Index (DIDI — ADR-079)
...
(12-line fan, ADR-079), and ALLIGATOR (3-line SMMA, ADR-079).
...
last_close. Source: ADR-079 DIDI window.

#### 2.283 Tom DeMark Indicator (DEMARKER — ADR-079)
...
close-to-close gains, ADR-079), STOCH (ADR-079), CCI (ADR-079),
WILLR (ADR-079), and every oscillator that compares closes rather
...
Source: ADR-079 DEMARKER window.

#### 2.284 Bill Williams Gator Oscillator (GATOR — ADR-079)

Pulled from `research::get_gator`. A companion to the ALLIGATOR
(ADR-079) that visualizes how the three shifted SMMAs diverge or
...
last_close. Source: ADR-079 GATOR window.

#### 2.285 Bill Williams Market Facilitation Index (BW_MFI — ADR-079)
...
Distinct from Chaikin's Money Flow Index (ADR-081, based on
...
volume_prev, last_close, bwmfi_color. Source: ADR-079 BW_MFI
window.

#### 2.286 Volume Weighted Moving Average (VWMA — ADR-079)
...
intraday, ADR-079), and from every other fixed-length MA (SMA,
...
Source: ADR-079 VWMA window.

#### 2.287 Rolling Standard Deviation (STDDEV — ADR-079)
...
weighted, ADR-079), REALIZED_VOL (log-return based), and
...
annualized, last_close. Source: ADR-079 STDDEV window.

#### 2.288 Weighted Moving Average (WMA — ADR-079)
...
of HMA (ADR-079), the plain WMA itself was missing from the
shipped list. Distinct from SMA, EMA, HMA, DEMA (ADR-079), TEMA
(ADR-079), T3 (ADR-079), ALMA (ADR-081), KAMA (ADR-079), MAMA
(ADR-079), and every adaptive MA. BULL / WEAK_BULL / NEUTRAL /
...
Source: ADR-079 WMA window.

#### 2.289 Rainbow MA Oscillator (RAINBOW — ADR-079)
...
3/5/8/10/12/15 and 30/35/40/45/50/60, ADR-079) — the
...
last_close. Source: ADR-079 RAINBOW window.

#### 2.290 MESA Sine Wave (MESA_SINE — ADR-079)
...
Distinct from MAMA (phase-adaptive MA, ADR-079) which uses the
same Hilbert discriminator to drive α rather than emit a phase-
based sine; from FISHER (probability Gaussianization, ADR-079);
and from COG (weighted centroid, ADR-079). CYCLE_BUY /
...
Source: ADR-079 MESA_SINE window.

#### 2.291 Fractal Adaptive Moving Average (FRAMA — ADR-079)
...
from KAMA (efficiency-ratio adaptive, ADR-079), VIDYA
(volatility-index adaptive, ADR-081), MAMA (Hilbert-phase
adaptive, ADR-079), and T3 (Tillson triple-DEMA, ADR-079).
...
spread, last_close. Source: ADR-079 FRAMA window.

#### 2.292 Internal Bar Strength (IBS — ADR-079)
...
Distinct from STOCH (%K over N-bar HHV/LLV, ADR-079) which
...
Source: ADR-079 IBS window.

#### 2.293 Ehlers Laguerre RSI (LAGUERRE_RSI — ADR-079)
...
smoothing of gains/losses, ADR-079), STOCHRSI (ADR-079), CRSI
(Connors, ADR-079), QQE (ADR-079), and IFT_RSI (ADR-079).
...
last_close. Source: ADR-079 LAGUERRE_RSI window.

#### 2.294 ZigZag Pattern Detector (ZIGZAG — ADR-079)
...
level. Distinct from FRACTALS (ADR-079, Bill Williams 5-bar
strict peaks) and from PIVOTS (ADR-079, prior-session math),
...
Source: ADR-079 ZIGZAG window.

#### 2.295 Pretty Good Oscillator (PGO — ADR-079)
...
(unscaled price change), PPO (percentage-scaled MACD, ADR-079),
...
pgo_value, pgo_prev, last_close. Source: ADR-079 PGO window.

#### 2.296 Hilbert Instantaneous Trendline (HT_TRENDLINE — ADR-079)

Pulled from `research::get_ht_trendline`. Smoothed trendline
based on the dominant cycle period derived from Ehlers's
Hilbert-transform homodyne discriminator. Unlike MAMA (ADR-079)
...
(adaptive α), FRAMA (ADR-079 fractal-dimension α), and every
...
spread_pct, last_close. Source: ADR-079 HT_TRENDLINE window.

#### 2.297 Midpoint of N (MIDPOINT — ADR-079)
...
recent trading range. Distinct from Donchian channel (ADR-079,
raw HHV/LLV bands), from SMA, and from pivot systems (ADR-079)
...
midpoint_prev, close_position, last_close. Source: ADR-079
MIDPOINT window.

#### 2.298 Dorsey Mass Index (MASSINDEX — ADR-079)
...
ratio, mass_index, mass_index_prev, last_close. Source: ADR-079
MASSINDEX window.

#### 2.299 Normalized ATR (NATR — ADR-079)
...
last_close. Source: ADR-079 NATR window.

#### 2.300 TTM Squeeze (TTM_SQUEEZE — ADR-079)
...
ADR-079 TTM_SQUEEZE window.

#### 2.301 Elder Force Index (FORCE_INDEX — ADR-079)
...
last_close. Source: ADR-079 FORCE_INDEX window.

#### 2.302 True Range raw (TRANGE — ADR-079)

Pulled from `research::get_trange`. TA-Lib's TRANGE function: `tr =
max(H − L, |H − C_prev|, |L − C_prev|)`, the single-bar volatility
measure that underlies Wilder's ATR (ADR-079) but reports the current
...
ADR-079 TRANGE window.

#### 2.303 Linear Regression Slope (LINEARREG_SLOPE — ADR-079)
...
last_close. Source: ADR-079 LINEARREG_SLOPE window.

#### 2.304 Hilbert Dominant Cycle Period (HT_DCPERIOD — ADR-079)
...
Source: ADR-079 HT_DCPERIOD window.

#### 2.305 Hilbert Trend vs Cycle Mode (HT_TRENDMODE — ADR-079)
...
regime), period, last_close. Source: ADR-079 HT_TRENDMODE window.

#### 2.306 Acceleration Bands (ACCBANDS — ADR-079)
...
position, last_close. Source: ADR-079 ACCBANDS window.

#### 2.307 Fast Stochastic (STOCHF — ADR-079)
...
fastk_prev, fastd, fastd_prev, last_close. Source: ADR-079 STOCHF
window.

#### 2.308 Linear Regression Endpoint (LINEARREG — ADR-079)
...
from LINEARREG_SLOPE (ADR-079 raw slope coefficient) and
...
ADR-079 LINEARREG window.

#### 2.309 Linear Regression Angle (LINEARREG_ANGLE — ADR-079)
...
horizontal. Distinct from LINEARREG_SLOPE (ADR-079 raw slope) and
LINEARREG (ADR-079 fitted value) because LINEARREG_ANGLE reports a
...
angle_deg, angle_deg_prev, last_close. Source: ADR-079 LINEARREG_ANGLE
window.

#### 2.310 Hilbert Dominant Cycle Phase (HT_DCPHASE — ADR-079)

Pulled from `research::get_ht_dcphase`. TA-Lib's HT_DCPHASE reuses the
Ehlers homodyne discriminator pipeline (same as HT_DCPERIOD, ADR-079)
...
(ADR-079 sine/leadsine waveforms) because HT_DCPHASE reports the raw
...
phase_delta, period, last_close. Source: ADR-079 HT_DCPHASE window.

#### 2.311 Hilbert Sine Wave (HT_SINE — ADR-079)
...
ADR-079 HT_SINE window.

#### 2.312 Hilbert Phasor Components (HT_PHASOR — ADR-079)
...
phase_deg, last_close. Source: ADR-079 HT_PHASOR window.

#### 2.313 Range Midpoint (MIDPRICE — ADR-079)
...
Distinct from MIDPOINT (ADR-079 close-based midpoint) and from
...
hhv, llv, position, last_close. Source: ADR-079 MIDPRICE window.

#### 2.314 Absolute Price Oscillator (APO — ADR-079)
...
(ADR-079 percentage variant: `(fast − slow) / slow × 100`) and from
...
slow_ema, last_close. Source: ADR-079 APO window.

#### 2.315 Raw Momentum (MOM — ADR-079)
...
mom_pct, last_close. Source: ADR-079 MOM window.

#### 2.316 Extended Parabolic SAR (SAREXT — ADR-079)
...
first two closes). Distinct from PSAR (ADR-079 fixed
...
last_close. Source: ADR-079 SAREXT window.

#### 2.317 ADX Rating (ADXR — ADR-079)

Pulled from `research::get_adxr`. TA-Lib's ADXR function: `(ADX_now
+ ADX[n − period]) / 2` over a 14-bar default lookback. Distinct from
ADX (ADR-079 point-in-time directional movement strength) because
...
Source: ADR-079 ADXR window.

#### 2.318 OHLC Average (AVGPRICE — ADR-079)
...
ADR-079 AVGPRICE window.

#### 2.319 Range Median (MEDPRICE — ADR-079)

Pulled from `research::get_medprice`. TA-Lib's MEDPRICE function:
`(high + low) / 2` — instantaneous range median, zero lookback.
Distinct from MIDPRICE (ADR-079, 14-bar HHV/LLV smoothed midpoint)
...
Source: ADR-079 MEDPRICE window.

#### 2.320 Typical Price (TYPPRICE — ADR-079)
...
typprice, typprice_prev, delta_pct. Source: ADR-079 TYPPRICE window.

#### 2.321 Weighted Close (WCLPRICE — ADR-079)
...
wclprice_prev, delta_pct. Source: ADR-079 WCLPRICE window.

#### 2.322 Close Variance (VARIANCE — ADR-079)
...
last_close. Source: ADR-079 VARIANCE window.

#### 2.323 Prior Ingested Web Research (INGESTED — ADR-080)
...
| Insider flow — Form 4 (ADR-079 INS) | 8 rows | Last quarter-ish of filings covers sentiment |
| Institutional top holders (ADR-079 HDS) | 6 rows | Top-6 concentration coverage |
| Daily bars table (ADR-079 HP) | 10 rows | 2-week audit trail in the packet |
| EPS surprise history (ADR-079 EPS) | 8 rows | 8-quarter beat/miss record |
| World equity indices (ADR-079 WEI) | 12 rows | 12 indices cover Americas/EMEA/Asia regimes |
| Market movers (ADR-079 MOV) | 6 symbols × 3 lists | Leadership/laggards without flooding packet |
| WACC snapshot (ADR-079 WACC) | 3 lines | Derived metric — compact summary is enough |
| FX pairs per region (ADR-079 WCR) | 8 pairs × 3 regions | Majors / crosses / EM without flooding |
| Rolling beta windows (ADR-079 BETA) | typically 3 rows | 1Y / 3Y / 5Y covers both recent and structural |
| Gordon Growth DDM (ADR-079 DDM) | 2-3 lines | Single implied price + input lineage |
| Relative valuation rows (ADR-079 RV) | ≤9 metrics | Matches Fundamentals getters with peer support |
| FIGI identifiers (ADR-079 FIGI) | 3 rows | Most US names have ≤3 share classes |
| HRA rolling windows (ADR-079 HRA) | ≤10 rows | Standard 1D–ITD ladder, one row each |
| DCF projection years (ADR-079 DCF) | 3-15 rows | Matches user-tuned `projection_years` |
| SVM model rows (ADR-079 SVM) | ≤6 rows | WACC / DDM / DCF / 3 peer-multiple rows |
| OMON ATM-zone strikes (ADR-079 OMON) | 11 rows | 5 ITM + ATM + 5 OTM across both sides |
| IVOL history trail (ADR-079 IVOL) | 8 points | Trend into today without dumping full 52w |
| Seasonality months (ADR-079 SEAG) | 12 rows | One per calendar month |
| Seasonality day-of-week (ADR-079 SEAG) | 5 rows | Mon–Fri trading sessions |
| Correlation peer cells (ADR-079 COR) | 10 rows | Sector peers capped at top-10 for readability |
| Total return windows (ADR-079 TRA) | ≤8 rows | Standard 1M–ITD ladder |
| Technical indicators (ADR-079 TECH) | 6 rows | RSI / MACD / BB / ATR / ADX / Stoch |
| Volatility skew points (ADR-079 SKEW) | 9 rows per expiry | ±10% OTM window + ATM coverage |
| Leverage ratios (ADR-079 LEV) | 6 rows | Debt/EBITDA, Net Debt/EBITDA, Debt/Equity, Interest Coverage, Current, Quick |
| Accruals periods (ADR-079 ACRL) | 8 quarters | 2-year cash-conversion trajectory |
| Realized vol windows (ADR-079 RVOL) | 4 rows | 20d / 60d / 120d / 252d cone |
| FCF yield periods (ADR-079 FCFY) | 6 rows | TTM + 5 annual rows for 5Y CAGR |
| Short interest headline fields (ADR-079 SHRT) | 7 k/v rows | Short%, DTC, float, shares out, ADV, short ratio, utilization |
| Altman Z components (ADR-079 ALTZ) | 5 rows | A/B/C/D/E with ratio + coefficient + contribution |
| Piotroski F-score checks (ADR-079 PTFS) | 9 rows | Fixed 9-point quality checklist |
| OHLC volatility estimators (ADR-079 VOLE) | 5 rows | CtC / Parkinson / GK / RS / YZ |
| EPS beat streak fields (ADR-079 EPSB) | 8 k/v rows | Counts, streaks, surprise avg/median/recent, latest |
| Price target dispersion fields (ADR-079 PTD) | 8 k/v rows | Targets, dispersion, spread, implied returns |
| Insider activity fields (ADR-079 MNGR) | 10 k/v rows | Counts, unique insiders, gross/net $, net shares, latest date |
| Dividend growth annual rows (ADR-079 DIVG) | ≤10 rows | Decade of calendar-year buckets |
| Earnings momentum quarters (ADR-079 EARM) | ≤8 rows | 8 quarters covers both recent and prior 4Q comparison windows |
| Sector rotation fields (ADR-079 SECTR) | 10 k/v rows | Sector rank, rel strength, breadth, strongest/weakest |
| Upgrade/downgrade momentum fields (ADR-079 UPDM) | 12 k/v rows | Net 30/90/180, counts per bucket, latest action |
| Momentum 12-1 fields (ADR-079 MOM) | 5 k/v rows | Regime, trend, composite, returns ladder, vol-adj score |
| Liquidity profile fields (ADR-079 LIQ) | 10 k/v rows | Tier, avg/median $, turnover, Amihud, ATR, spread proxy |
| Breakout proximity fields (ADR-079 BREAK) | 10 k/v rows | Label, setup, 20d/60d/52w ranges, position, consolidation |
| Cash conversion cycle periods (ADR-079 CCRL) | ≤8 rows | Latest period + up to 7 prior with DSO/DIO/DPO/CCC |
| Credit score components (ADR-079 CREDIT) | ≤6 rows | ALTZ / PTFS / LEV / ACRL each with value + score + weight |
| GARP composite fields (ADR-079 GROWM) | 5 k/v rows + ≤5 component rows | Label, score, inputs, MOM/EARM/DIVG sub-lines + component contributions |
| Smart-money flow fields (ADR-079 FLOW) | 10 k/v rows | Label, composite, insider+institutional sub-scores, buys/sells/net USD, trades+unique, buyers/sellers/holders, net ratio, share delta |
| Market regime fields (ADR-079 REGIME) | 8 k/v rows | Label, composite, realized vol + source, ADX + trend summary, 1Y return, Sharpe, 3 sub-scores, inputs |
| Relative volume fields (ADR-079 RELVOL) | 6 k/v rows | Activity, direction, current+avg, rel-vol ratios, 5d-vs-20d trend, 60d percentile, bars used |
| Margin trajectory rows (ADR-079 MARGINS) | 3×4 grid + ≤6 period rows | Gross/op/net margin (latest/prior/Δpp/trend) + avg row + per-period history |
| Value-factor composite fields (ADR-079 VAL) | 6 metric pairs + ≤6 component rows | Ratio vs sector median for P/E, Forward P/E, P/B, P/S, EV/EBITDA, FCF Yield |
| Quality-factor composite fields (ADR-079 QUAL) | 8 k/v rows + ≤4 component rows | Piotroski, op margin + trend, cash conversion + trend, leverage summary, debt/EBITDA |
| Risk-factor composite fields (ADR-079 RISK) | 7 k/v rows + ≤5 component rows | Realized vol, beta, liquidity tier, short%float + DTC, Altman Z + zone; higher = riskier |
| Insider streak rows (ADR-079 INSSTRK) | 8 k/v rows + ≤8 per-insider rows | Unique insiders, buy/sell streak counts, longest streaks, net $ totals |
| Coverage breadth fields (ADR-079 COVG) | 12 k/v rows | Num analysts, target mean/low/high, consensus 5-bucket, bull ratio, 90d churn, 3 sub-scores |
| Value rank fields (ADR-079 VRK) | 4 k/v rows | Subject composite + sector median/p25/p75 + percentile + rank position |
| Quality rank fields (ADR-079 QRK) | 4 k/v rows | Subject composite + sector median/p25/p75 + percentile + rank position |
| Risk rank fields (ADR-079 RRK) | 4 k/v rows | Subject composite (higher=riskier) + sector median/p25/p75 + SAFE percentile + rank position |
| Relative EPS growth fields (ADR-079 RELEPSGR) | 4 k/v rows | Latest/earliest EPS, sector median/p25/p75 CAGR, gap-to-median in pp |
| PEAD fields (ADR-079 PEAD) | 6 k/v rows + ≤8 event rows | Avg drift 1d/3d/5d/10d, BEAT/MISS breakouts, latest event, per-event detail table |
| Size factor fields (ADR-079 SIZEF) | 4 k/v rows | Market cap + log(cap) + sector median/p25/p75 caps + percentile + rank position |
| Momentum rank fields (ADR-079 MOMF) | 3 k/v rows | Subject momentum composite + sector median/p25/p75 + percentile + rank position |
| PEAD rank fields (ADR-079 PEADRANK) | 3 k/v rows | Subject avg 5d drift + sector median/p25/p75 + percentile + rank position |
| Fundamental quality meter fields (ADR-079 FQM) | 4 k/v rows | Piotroski, op margin + trend, cash conversion + trend, PTFS/MARGINS/ACRL components |
| Relative revenue growth fields (ADR-079 REVRANK) | 4 k/v rows | Latest/earliest revenue, sector median/p25/p75 CAGR, gap-to-median in pp |
| Leverage rank fields (ADR-079 LEVRANK) | 3 k/v rows | Subject D/E + sector median/p25/p75 D/E + SAFE percentile + rank position (NEGATIVE_EQUITY branch prints raw total_debt/total_equity) |
| Operating quality rank fields (ADR-079 OPERANK) | 3 k/v rows | Subject op margin % + trend + sector median/p25/p75 op margin + percentile + rank position |
| FQM rank fields (ADR-079 FQMRANK) | 3 k/v rows | Subject FQM composite + operator label + sector median/p25/p75 + percentile + rank position |
| Liquidity rank fields (ADR-079 LIQRANK) | 3 k/v rows | Subject ADV$ in $M + tier label + sector median/p25/p75 ADV$ + percentile + rank position |
| Earnings surprise streak fields (ADR-079 SURPSTK) | 4 k/v rows | Events breakdown + beat rate + current/longest streaks + avg surprise + latest event |
| Dividend growth rank fields (ADR-079 DVDRANK) | 4 k/v rows | Subject 3y CAGR + consecutive growth years + trend label + sector median/p25/p75 CAGR + percentile + rank position |
| Earnings momentum rank fields (ADR-079 EARMRANK) | 3 k/v rows | Subject composite score + momentum label + sector median/p25/p75 + percentile + rank position |
| Upgrade/downgrade rank fields (ADR-079 UPDGRANK) | 3 k/v rows | Subject net_90d + bias label + sector median/p25/p75 net + percentile + rank position |
| Gap yearly fields (ADR-079 GY) | 5 k/v rows | 253-bar gap census: 2/5/10% gap bins up+down + largest up/down with date + avg |gap| + label |
| Daily event streak fields (ADR-079 DES) | 4 k/v rows | 253-bar up/down/flat census + longest up/down streaks + current streak + up-day rate + avg up/down move |
| Dividend yield rank fields (ADR-079 DVDYIELDRANK) | 3 k/v rows | Subject yield + sector median/p25/p75 yield + percentile + rank position (non-payers filtered) |
| Short interest rank fields (ADR-079 SHRANK) | 3 k/v rows | Subject short % of float + sector median/p25/p75 short + risk-inverted percentile + rank position |
| Annualized ATR fields (ADR-079 ATRANN) | 4 k/v rows | Latest close + ATR14 price units + ATR14 % + annualized % (×√252) + volatility regime label |
| Drawdown history fields (ADR-079 DDHIST) | 5 k/v rows | Max drawdown % + peak/trough dates + longest drawdown days + 5%/10% correction counts + current drawdown |
| Price performance fields (ADR-079 PRICEPERF) | 4 k/v rows | 1M/3M/6M/YTD/1Y returns + latest close + bars used + trend label (1Y/3M blend) |
| Beta rank fields (ADR-079 BETARANK) | 3 k/v rows | Subject beta + sector median/p25/p75 + risk-inverted percentile + rank position (lower beta = safer) |
| PEG rank fields (ADR-079 PEGRANK) | 3 k/v rows | Subject PEG + sector median/p25/p75 + value-inverted percentile + rank position (lower PEG = better value) |
| 52-week high/low fields (ADR-079 FHIGHLOW) | 5 k/v rows | 52w high/low + dates + days since + pct-from-high/low + range position + proximity label |
| Realized vol cone fields (ADR-079 RVCONE) | 4 k/v rows | RV20/60/120/252 + RV20 rolling min/median/max + RV20 percentile + cone label |
| Calendar period breakdown fields (ADR-079 CALPB) | 5 k/v rows | MTD + QTD + YTD + prior quarter + prior year + current year/quarter + momentum label |
| Return skewness fields (ADR-079 RETSKEW) | 4 k/v rows | Bars used + mean/stdev log return + skewness + positive-day % + largest up/down + skew label |
| Return excess kurtosis fields (ADR-079 RETKURT) | 4 k/v rows | Bars used + stdev + excess kurtosis + 2σ/3σ outlier counts + 2σ outlier pct + kurt label |
| Tail ratio fields (ADR-079 TAILR) | 4 k/v rows | Bars used + P95/P05 + 95/5 tail ratio + P99/P01 + 99/1 tail ratio + bias label |
| Run length fields (ADR-079 RUNLEN) | 4 k/v rows | Bars used + avg up/down runs + run counts + longest up/down + signed current run + trend label |
| Daily range fields (ADR-079 DAYRANGE) | 4 k/v rows | Bars used + avg 60d/252d range % + latest range + compression ratio + widest/narrowest + range label |
| Return autocorrelation fields (ADR-079 AUTOCOR) | 3 k/v rows | Bars used + mean log return + ACF at lags 1/5/10/20 + regime label |
| Hurst exponent fields (ADR-079 HURST) | 2 k/v rows | Bars used + H + scales used + min/max scale + memory label |
| Hit rate fields (ADR-079 HITRATE) | 3 k/v rows | Bars used + up/down/flat days + hitrate 5d/20d/60d/252d + hit label |
| Gain/loss asymmetry fields (ADR-079 GLASYM) | 3 k/v rows | Bars used + up/down day counts + avg/median up & down pct + ratio + asymmetry label |
| Up/down volume ratio fields (ADR-079 VOLRATIO) | 4 k/v rows | Bars used + up/down day counts + avg/median up & down volume + ratio + max up/down volume + flow label |
| Rally history fields (ADR-079 DRAWUP) | 3 k/v rows | Bars used + max drawup % + trough/peak dates + longest rally days + rallies ≥5%/≥10% + current drawup + rally label |
| Overnight gap stat fields (ADR-079 GAPSTATS) | 3 k/v rows | Bars used + gap up/down counts + frequency + avg/up/down gap pct + largest up/down gap + bias label |
| Volatility clustering fields (ADR-079 VOLCLUSTER) | 2 k/v rows | Bars used + |r| ACF lag 1/5/20 + r² ACF lag 1/5/20 + cluster label |
| Close placement fields (ADR-079 CLOSEPLC) | 2 k/v rows | Bars used + avg/median/latest placement + near-high/near-low % + placement label |
| Mean-reversion half-life fields (ADR-079 MRHL) | 2 k/v rows | Bars used + AR(1) β/α + R² + half-life days + regime label |
| Downside deviation / Sortino fields (ADR-079 DOWNVOL) | 2 k/v rows | Bars used + mean log return + downside/upside dev + Sortino raw/ann + downside % of total var + sortino label |
| Sharpe ratio fields (ADR-079 SHARPR) | 2 k/v rows | Bars used + mean/stdev log return + Sharpe raw/ann + mean/stdev ann + sharpe label |
| Kaufman efficiency ratio fields (ADR-079 EFFRATIO) | 2 k/v rows | Bars used + start/end close + net change + Σ |Δclose| + ER + signed ER + efficiency label |
| Wick bias fields (ADR-079 WICKBIAS) | 2 k/v rows | Bars used + avg/median upper/lower wick + body share + bias score + bias label |
| Vol-of-vol fields (ADR-079 VOLOFVOL) | 2 k/v rows | RV points + mean/stdev/min/max/latest RV20 + CV + cv label |
| Calmar ratio fields (ADR-079 CALMAR) | 2 k/v rows | Bars used + total/annualized return + max drawdown + calmar ratio + calmar label |
| Ulcer index fields (ADR-079 ULCER) | 2 k/v rows | Bars used + ulcer index + mean/max drawdown + % in drawdown + ann return + Martin ratio + ulcer label |
| Variance ratio fields (ADR-079 VARRATIO) | 2 k/v rows | Bars used + VR(2/5/10/20) + z-stat(2/5) + rw label |
| Amihud illiquidity fields (ADR-079 AMIHUD) | 2 k/v rows | Bars used + mean/median/90th ILLIQ + avg $ volume + illiq label |
| Jarque-Bera normality fields (ADR-079 JBNORM) | 2 k/v rows | Bars used + skewness + excess kurtosis + JB statistic + p-value + normal label |
| Omega ratio fields (ADR-079 OMEGA) | 2 k/v rows | Bars used + gains/losses Σ + gain/loss days + win rate + omega ratio + omega label |
| Detrended fluctuation fields (ADR-079 DFA) | 2 k/v rows | Bars used + α exponent + num scales + log-log R² + dfa label |
| Burke ratio fields (ADR-079 BURKE) | 2 k/v rows | Bars used + annualized return + dd event count + Σdd² + worst event dd + burke ratio + burke label |
| Monthly seasonality fields (ADR-079 MONTHSEAS) | 3-4 k/v rows | Years covered + best/worst month + full 12-cell month grid (hit % + mean ret %) + season label |
| Roll implicit spread fields (ADR-079 ROLLSPRD) | 2 k/v rows | Bars used + first-lag cov + mean price + implicit spread + implicit spread (bps) + roll label |
| Parkinson vol fields (ADR-079 PARKINSON) | 2 k/v rows | Bars used + daily σ + annualized σ + mean ln(H/L) + vol label |
| Garman-Klass vol fields (ADR-079 GKVOL) | 2 k/v rows | Bars used + daily σ + annualized σ + range/C-O components + vol label |
| Rogers-Satchell vol fields (ADR-079 RSVOL) | 2 k/v rows | Bars used + daily σ + annualized σ (drift-independent) + vol label |
| CVaR / Expected Shortfall fields (ADR-079 CVAR) | 2 k/v rows | Bars used + VaR/ES(5%) + VaR/ES(1%) + tail day counts + cvar label |
| Day-of-week seasonality fields (ADR-079 DOWEFFECT) | 2-3 k/v rows | Bars used + weeks covered + best/worst weekday + full 5-cell weekday grid (hit % + mean ret %) + dow label |
| Sterling ratio fields (ADR-079 STERLING) | 2 k/v rows | Bars used + annualized return + worst_n + dd event count + mean worst-N dd + sterling ratio + sterling label |
| Kelly fraction fields (ADR-079 KELLYF) | 2 k/v rows | Bars used + win/loss rate + avg win/loss % + b ratio + f* + half_kelly + kelly label |
| Ljung-Box fields (ADR-079 LJUNGB) | 2 k/v rows | Bars used + lag h + Q-statistic + p-value + reject white noise + ljungb label |
| Runs test fields (ADR-079 RUNSTEST) | 2 k/v rows | Bars used + positive/negative days + runs observed/expected/std + z-stat + p-value + reject randomness + runs label |
| Zero-return fields (ADR-079 ZERORET) | 2 k/v rows | Bars used + zero day count + zero day % + longest zero streak + epsilon + zero label |
| Probabilistic Sharpe fields (ADR-079 PSR) | 2 k/v rows | Bars used + annualized Sharpe + skewness γ₃ + kurtosis γ₄ + SR benchmark + PSR + psr label |
| Dickey-Fuller fields (ADR-079 ADF) | 2 k/v rows | Bars used + β + SE(β) + t-statistic + crit 1/5/10% + reject unit root + adf label |
| Mann-Kendall fields (ADR-079 MNKENDALL) | 2 k/v rows | Bars used + S-statistic + variance + z-statistic + p-value + Kendall τ + reject no-trend + mk label |
| Bipower fields (ADR-079 BIPOWER) | 2 k/v rows | Bars used + realized variance + bipower variation + continuous/realized ann vol % + jump ratio + jump % + jump label |
| Drawdown duration fields (ADR-079 DDDUR) | 2 k/v rows | Bars used + event count + max/mean/median duration + total bars underwater + % underwater + currently underwater + current duration + dddur label |
| Hill tail-index fields (ADR-079 HILLTAIL) | 2 k/v rows | Returns used + k order stats + threshold |r|(k+1) + α on |r|/left/right + tail label |
| ARCH-LM fields (ADR-079 ARCHLM) | 2 k/v rows | Returns used + q lags + R² + LM=n·R² + p-value + crit χ²(5) 5%/1% + reject homoskedastic + arch label |
| Pain ratio fields (ADR-079 PAINRATIO) | 2 k/v rows | Bars used + pain index (mean\|dd\|) + annualized return + pain ratio + max dd + pain label |
| CUSUM break fields (ADR-079 CUSUM) | 2 k/v rows | Returns used + max\|S_t\| + D=max\|S_t\|/√n + bar at max + direction + crit 10/5/1% + reject stability + cusum label |
| Cornish-Fisher VaR fields (ADR-079 CFVAR) | 2 k/v rows | Returns used + mean/σ returns + skew γ₃ + excess kurt γ₄ + Gauss/CF VaR 5%+1% + adj 5% + skew/kurt term contributions + cfvar label |
| Shannon entropy fields (ADR-079 ENTROPY) | 2 k/v rows | Returns used + bins + H bits + H_max bits + normalised H/H_max + entropy label |
| Rachev ratio fields (ADR-079 RACHEV) | 2 k/v rows | Returns used + ES right/left 5% + Rachev 5% + ES right/left 1% + Rachev 1% + rachev label |
| Gain-to-Pain fields (ADR-079 GPR) | 2 k/v rows | Returns used + Σ all/gains/\|losses\| % + GPR + profit factor + wins/losses + gpr label |
| PACF fields (ADR-079 PACF) | 2 k/v rows | Returns used + PACF lags 1-5 + Bartlett 95% crit + sig lags + max \|PACF\| + max lag + pacf label |
| Approximate entropy fields (ADR-079 APEN) | 2 k/v rows | Returns used + m + r + Φ^m + Φ^(m+1) + ApEn + apen label |
| Upside potential ratio fields (ADR-079 UPR) | 2 k/v rows | Bars used + UPM₁ + LPM₂ + √LPM₂ + UPR + up/down days + upr label |
| Leverage effect fields (ADR-079 LEVEREFF) | 2 k/v rows | Bars used + corr(r,vol²) + down/up vol + asymmetry ratio + pairs used + levereff label |
| Drawdown-at-Risk fields (ADR-079 DRAWDAR) | 2 k/v rows | Bars used + max dd + DaR/CDaR 5% + DaR/CDaR 1% + dd events + drawdar label |
| Volatility half-life fields (ADR-079 VARHALF) | 2 k/v rows | RV points + AR(1) β/α + half-life days + mean/latest RV + varhalf label |
| Return Gini coefficient fields (ADR-079 GINI) | 2 k/v rows | Bars used + Gini + mean/max/min \|r\| + gini label |
| Sample entropy fields (ADR-079 SAMPEN) | 2 k/v rows | Returns used + m + r + A/B counts + SampEn + sampen label |
| Permutation entropy fields (ADR-079 PERMEN) | 2 k/v rows | Returns used + m + patterns obs/possible + H_raw + H_norm + permen label |
| Recovery factor fields (ADR-079 RECFACT) | 2 k/v rows | Bars used + cum return % + max dd % + recovery factor + recfact label |
| KPSS stationarity fields (ADR-079 KPSS) | 2 k/v rows | Returns used + η_μ + lag ℓ + crit 10/5/1% + reject_stationary + kpss label |
| Spectral entropy fields (ADR-079 SPECENT) | 2 k/v rows | Returns used + freq bins + H_raw + H_norm + peak idx + peak share + specent label |
| Robust volatility fields (ADR-079 ROBVOL) | 2 k/v rows | Returns used + classical/MAD/IQR σ (annual) + MAD ratio + IQR ratio + robvol label |
| Rényi entropy fields (ADR-079 RENYIENT) | 2 k/v rows | Returns used + bins + α + H₂ raw + H₂ normalised + collision_prob + renyient label |
| Return quantile profile fields (ADR-079 RETQUANT) | 2 k/v rows | Returns used + P1/P5/P10/P25/P50/P75/P90/P95/P99 + IQR + tail asymmetry + retquant label |
| Multiscale entropy fields (ADR-079 MSENT) | 2 k/v rows | Returns used + m + r + τ_max + SampEn τ=1..5 + complexity index + msent label |
| EWMA volatility fields (ADR-079 EWMAVOL) | 2 k/v rows | Returns used + λ + variance + σ daily/annual + classical σ annual + ewma/classical ratio + ewmavol label |
| KS normality test fields (ADR-079 KSNORM) | 2 k/v rows | Returns used + D statistic + 10%/5%/1% criticals + reject flags + μ + σ + ksnorm label |
| Anderson-Darling fields (ADR-079 ADTEST) | 2 k/v rows | Returns used + A² + A²_adj + p-value approx + 10%/5%/1% criticals + reject flags + adtest label |
| L-moments fields (ADR-079 LMOM) | 2 k/v rows | Returns used + L1/L2/L3/L4 + τ3 skew + τ4 kurt + lmom label |
| Kyle's λ fields (ADR-079 KYLELAM) | 2 k/v rows | Bars used + Kyle λ + mean \|Δp\| + mean V + correlation ρ + R² + kylelam label |
| Peaks-over-threshold fields (ADR-079 PEAKOVER) | 3 k/v rows | Returns used + P95/P99 thresholds + counts + mean/max excesses (both P95 and P99) + peakover label |
| Higuchi fractal dim fields (ADR-079 HIGUCHI) | 2 k/v rows | Returns used + k_max + fractal_dim + R² + log-k points + higuchi label |
| Pickands tail-index fields (ADR-079 PICKANDS) | 2 k/v rows | Returns used + k + γ̂ + tail α + x_k/x_2k/x_4k order-stats + pickands label |
| Kappa-3 ratio fields (ADR-079 KAPPA3) | 2 k/v rows | Returns used + MAR + excess μ + LPM3 + LPM3^(1/3) + κ3 + Sortino reference + kappa3 label |
| Lyapunov exponent fields (ADR-079 LYAPUNOV) | 2 k/v rows | Returns used + m + τ + λ_max + R² + steps used + lyapunov label |
| Spearman rank autocorrelation (ADR-079 RANKAC) | 2 k/v rows | Returns used + ρ(1) + ρ(5) + ρ(10) + mean\|ρ\| + max\|ρ\| + rankac label |
| BNS jump-test Z fields (ADR-079 BNSJUMP) | 2 k/v rows | Returns used + RV + BV + jump ratio + z-statistic + approx p-value + bnsjump label |
| Phillips-Perron fields (ADR-079 PPROOT) | 2 k/v rows | Bars used + ρ̂ + raw t + PP Z(ρ) + PP Z(t) + auto-picked lag truncation q + pproot label |
| Multifractal DFA fields (ADR-079 MFDFA) | 2 k/v rows | Returns used + h(−2) + h(0) + h(+2) + Δh + scales used + mfdfa label |
| Hill-tail KS fields (ADR-079 HILLKS) | 2 k/v rows | Returns used + k (tail size) + Hill α̂ + KS D statistic + KS 5% critical + hillks label |
| True Strength Index fields (ADR-079 TSI) | 2 k/v rows | Bars used + EMA long/short periods + TSI value + signal value + TSI−signal + tsi label |
| GARCH(1,1) fit fields (ADR-079 GARCH11) | 2 k/v rows | Returns used + ω + α + β + persistence + unconditional variance + half-life + log-likelihood + garch11 label |
| Sup-ADF bubble test fields (ADR-079 SADF) | 2 k/v rows | Bars used + min window r0 + full-ADF t + SADF stat + argmax end + critical 5% + reject null + sadf label |
| Correlation dimension fields (ADR-079 CORDIM) | 2 k/v rows | Returns used + embed_dim + radii fitted + D2 + fit R² + cordim label |
| Rolling skewness spectrum fields (ADR-079 SKSPEC) | 2 k/v rows | Returns used + window size + mean/std/min/max/range of rolling skew + skspec label |
| Auto mutual information fields (ADR-079 AUTOMI) | 2 k/v rows | Returns used + num_bins + MI(1/5/10) + H(X) + normalised MI(1)/H(X) + automi label |
| Ingested web articles (ADR-080 INGESTED) | 15 shown / 50 cached | Top 15 newest articles emitted per symbol; FIFO bag holds up to 50 with URL dedup + timestamp-wins replacement |
| Daily bars required for stats | ≥20 | Needed for 20d return and ATR warm-up |

There is no global packet size limit — total size scales with the number of
symbols. A single S&P 500 symbol now produces a packet around **~92-176 KB**
(up from 91-175 KB after ADR-079; ADR-079 adds five optional per-symbol
...
+1.05 KB per symbol; ADR-079 added five optional per-symbol
...
ADR-079 added five optional per-symbol
...
typical +1.45 KB per symbol; ADR-079 added five optional per-symbol
...
+1.45 KB per symbol; ADR-079 added five optional per-symbol
...
labels — distinct from RSI (ADR-079 Wilder), STOCHRSI (ADR-079),
CRSI (ADR-079), QQE (ADR-079), and IFT_RSI (ADR-079);
...
AT_REVERSAL labels — distinct from FRACTALS (ADR-079 Bill Williams
5-bar peaks) and PIVOTS (ADR-079 prior-session math) because it is
...
(unscaled), PPO (ADR-079 percentage MACD), and bounded oscillators
...
from MAMA (ADR-079 adaptive α), FRAMA (ADR-079 fractal α), and
...
LOWER (<0.15) labels — distinct from Donchian (ADR-079 raw
HHV/LLV bands), from SMA, and from pivot systems (ADR-079)
because it uses only the two extremes of the range;
ADR-079 adds five optional per-symbol
...
the width-to-center ratio — distinct from GMMA (ADR-079) which
...
(ADR-079 phase-adaptive MA), FISHER (ADR-079), and COG (ADR-079);
...
magnitude — distinct from KAMA (ADR-079 efficiency-ratio),
VIDYA (ADR-081 volatility-index), MAMA (ADR-079 Hilbert-phase),
and T3 (ADR-079 triple-DEMA);
...
labels — distinct from STOCH (ADR-079) which spans N bars rather
...
GATOR computes Bill Williams's companion to the ALLIGATOR (ADR-079)
...
four-color dots — distinct from Chaikin MFI (ADR-081) which is
...
VWAP (session-anchored, ADR-079) and every other fixed-length MA
...
ADR-079), REALIZED_VOL (log-return based), and Parkinson/Garman-
...
(ADR-079) as the Alligator-system entry trigger;
...
raw RSI, STOCHRSI (ADR-079), QQE (ADR-079), and CRSI (ADR-079);
...
ADR-079), VIDYA (volatility-index adaptive, ADR-081), and T3 (ADR-079);
...
BEAR / BEAR_NEEDLES labels — complements GMMA (ADR-079) and
ALLIGATOR (ADR-079) on the multi-MA axis; ADR-079 added five optional per-symbol
...
smoothing, ADR-079), STOCHF (no smoothing), and STOCHRSI (ADR-079);
...
direction vs prior — distinct from raw RSI (ADR-079), STOCHRSI
(ADR-079), CRSI (ADR-079), and RVI (ADR-079); PMO computes Martin
...
LINREG (ADR-079), TSF (ADR-079), DPO (ADR-079), and PPO; TMF
...
— distinct from CMF (range-based simple sum, ADR-079), ADL
(cumulative total, ADR-079), KLINGER (ADR-079), PVT (ADR-079),
and CHAIKOSC (ADR-079); ADR-079 added five optional per-symbol
...
price-based ROC; ADR-079 adds five optional per-symbol blocks —
...
inter-bar follow-through; ADR-084 adds one optional per-symbol block —
...
research table; ADR-079 added five optional per-symbol
...
trio (Alligator ADR-079 + AO ADR-079 + AC) with the earliest turn
...
range** — distinct from ATR (ADR-079, exponential TR *level*),
BBWIDTH (below, stddev bandwidth), and Volatility Regime (ADR-079,
...
**continuous** bandwidth value underlying BBSQUEEZE (ADR-079, which
...
(ADR-079, the oscillator) as the **regime filter** half of Elder's
...
ADR-079 previously added five optional per-symbol
...
`PVT_t = PVT_{t-1} + volume_t · (close_t − close_{t-1})/close_{t-1}` —
a percent-attribution volume oscillator distinct from OBV (ADR-079,
sign-of-Δclose × full-volume attribution regardless of move size) and
CHAIKOSC (ADR-079, high-low-position-driven A/D Line MACD), with a
21-EMA trend-direction overlay and 8-bar slope for acceleration
detection; ADR-079 previously added five optional per-symbol
...
ADR-079 added five optional per-symbol blocks — DEMA / TEMA / LINREG /
...
TRIX (ADR-079) which is an *oscillator derivative* of the same triple-
...
trend detection as a random-walk null-hypothesis test; ADR-079 adds five
...
OVERBOUGHT&&falling / BULL / NEUTRAL / BEAR / OVERSOLD&&rising labels —
first double-smoothed double-stochastic oscillator, typically leads
other momentum oscillators by 3-7 bars; ADR-079 added five optional
...
(ADR-079) remain unchanged; ADR-079 added five optional per-symbol
...
chosen "where price spent most of the bar"; prior five (ADR-079)
remain unchanged; ADR-079 adds five optional per-symbol
...
prior five (ADR-079) remain unchanged; ADR-079 adds five optional
...
false-positive-ing on noise; prior five (ADR-079) remain unchanged;
ADR-079 adds five optional per-symbol
...
not; prior five (ADR-079) remain unchanged; ADR-079 adds five optional
...
BBSQUEEZE (ADR-079) for the canonical TTM signal; FISHER applies
...
moment a fresh 25-bar high/low prints; prior five (ADR-079) remain
unchanged; ADR-079 adds five optional per-symbol
...
prior five (ADR-079) remain unchanged;
ADR-079 adds five optional per-symbol
...
(ADR-079) remain unchanged;
up from 64-129 KB after ADR-079; ADR-079 adds five optional per-symbol
...
labels driven by the MI(1)/H(X) ratio; prior five (ADR-079) remain
unchanged;
...
ADR-096 adds Return Path auto-ingest at those response-drain sites: if
...
| `self.bg.sec_filings` | `Vec<SecFiling>` | SEC filings window / scraper (ADR-073) |
...
| `news::get_news_by_symbol` | SQLite `research_news` + FTS5 | ADR-078 news pipeline |
| `research::get_dividends` | SQLite `research_dividends` | ADR-079 DVD window |
| `research::get_earnings_estimates` | SQLite `research_earnings_estimates` | ADR-079 EEB window |
| `research::get_rating_changes` | SQLite `research_rating_changes` | ADR-079 UPDG window |
| `research::get_financials` | SQLite `research_financials` | ADR-079 FA window |
| `research::get_executives` | SQLite `research_executives` | ADR-079 MGMT window |
| `research::get_stock_splits` | SQLite `research_stock_splits` | ADR-079 SPLT window |
| `research::get_analyst_recs` | SQLite `research_analyst_recs` | ADR-079 ANR window |
| `research::get_price_target` | SQLite `research_price_target` | ADR-079 ANR window |
| `research::get_esg` | SQLite `research_esg` | ADR-079 ESG window |
| `research::get_insider_trades` | SQLite `research_insider_trades` | ADR-079 INS window |
| `research::get_institutional_holders` | SQLite `research_institutional_holders` | ADR-079 HDS window |
| `research::get_shares_float` | SQLite `research_shares_float` | ADR-079 FLOAT window |
| `research::get_historical_price` | SQLite `research_historical_price` | ADR-079 HP window |
| `research::get_earnings_surprises` | SQLite `research_earnings_surprise` | ADR-079 EPS window |
| `research::get_wacc` | SQLite `research_wacc` | ADR-079 WACC window |
| `research::get_world_indices` | SQLite `research_world_indices` | ADR-079 WEI window |
| `research::get_market_movers` | SQLite `research_market_movers` | ADR-079 MOV window |
| `research::get_sector_performance` | SQLite `research_sector_performance` | ADR-079 INDU window |
| `research::get_currency_rates` | SQLite `research_currency_rates` | ADR-079 WCR window |
| `research::get_beta` | SQLite `research_beta` | ADR-079 BETA window |
| `research::get_ddm` | SQLite `research_ddm` | ADR-079 DDM window |
| `research::get_relative_valuation` | SQLite `research_relative_valuation` | ADR-079 RV window |
| `research::get_figi` | SQLite `research_figi` | ADR-079 FIGI window |
| `research::get_hra` | SQLite `research_hra` | ADR-079 HRA window |
| `research::get_dcf` | SQLite `research_dcf` | ADR-079 DCF window |
| `research::get_svm` | SQLite `research_svm` | ADR-079 SVM window |
| `research::get_options_chain` | SQLite `research_options_chain` | ADR-079 OMON window |
| `research::get_ivol` | SQLite `research_ivol` | ADR-079 IVOL window |
| `research::get_seasonality` | SQLite `research_seasonality` | ADR-079 SEAG window |
| `research::get_correlation` | SQLite `research_correlation` | ADR-079 COR window |
| `research::get_total_return` | SQLite `research_total_return` | ADR-079 TRA window |
| `research::get_technicals` | SQLite `research_technicals` | ADR-079 TECH window |
| `research::get_vol_skew` | SQLite `research_vol_skew` | ADR-079 SKEW window |
| `research::get_leverage` | SQLite `research_leverage` | ADR-079 LEV window |
| `research::get_accruals` | SQLite `research_accruals` | ADR-079 ACRL window |
| `research::get_realized_vol` | SQLite `research_realized_vol` | ADR-079 RVOL window |
| `research::get_fcf_yield` | SQLite `research_fcf_yield` | ADR-079 FCFY window |
| `research::get_short_interest` | SQLite `research_short_interest` | ADR-079 SHRT window |
| `research::get_altman_z` | SQLite `research_altman_z` | ADR-079 ALTZ window |
| `research::get_piotroski` | SQLite `research_piotroski` | ADR-079 PTFS window |
| `research::get_ohlc_vol` | SQLite `research_ohlc_vol` | ADR-079 VOLE window |
| `research::get_eps_beat` | SQLite `research_eps_beat` | ADR-079 EPSB window |
| `research::get_price_target_dispersion` | SQLite `research_price_target_dispersion` | ADR-079 PTD window |
| `research::get_insider_activity` | SQLite `research_insider_activity` | ADR-079 MNGR window |
| `research::get_divg` | SQLite `research_divg` | ADR-079 DIVG window |
| `research::get_earm` | SQLite `research_earm` | ADR-079 EARM window |
| `research::get_sector_rotation` | SQLite `research_sector_rotation` | ADR-079 SECTR window |
| `research::get_updm` | SQLite `research_updm` | ADR-079 UPDM window |
| `research::get_momentum` | SQLite `research_momentum` | ADR-079 MOM window |
| `research::get_liquidity` | SQLite `research_liquidity` | ADR-079 LIQ window |
| `research::get_breakout` | SQLite `research_breakout` | ADR-079 BREAK window |
| `research::get_cash_cycle` | SQLite `research_cash_cycle` | ADR-079 CCRL window |
| `research::get_credit` | SQLite `research_credit` | ADR-079 CREDIT window |
| `research::get_growm` | SQLite `research_growm` | ADR-079 GROWM window (fuses MOM+EARM+DIVG) |
| `research::get_flow` | SQLite `research_flow` | ADR-079 FLOW window (composes cached INS+HDS) |
| `research::get_regime` | SQLite `research_regime` | ADR-079 REGIME window (fuses VOLE+TECH+HRA) |
| `research::get_relvol` | SQLite `research_relvol` | ADR-079 RELVOL window (over cached HP bars) |
| `research::get_margins` | SQLite `research_margins` | ADR-079 MARGINS window (over cached FA statements) |
| `research::get_val` | SQLite `research_val` | ADR-079 VAL window (fuses 6 valuation ratios vs sector-peer medians) |
| `research::get_qual` | SQLite `research_qual` | ADR-079 QUAL window (fuses PTFS+MARGINS+ACRL+LEV) |
| `research::get_risk` | SQLite `research_risk` | ADR-079 RISK window (fuses VOLE+BETA+LIQ+SHRT+ALTZ, higher = riskier) |
| `research::get_insstrk` | SQLite `research_insstrk` | ADR-079 INSSTRK window (groups cached InsiderTrade rows by insider) |
| `research::get_covg` | SQLite `research_covg` | ADR-079 COVG window (fuses PriceTarget+AnalystRecs+UPDM) |
| `research::get_vrk` | SQLite `research_vrk` | ADR-079 VRK window (sector percentile rank of VAL composite) |
| `research::get_qrk` | SQLite `research_qrk` | ADR-079 QRK window (sector percentile rank of QUAL composite) |
| `research::get_rrk` | SQLite `research_rrk` | ADR-079 RRK window (sector percentile rank of RISK composite, inverted) |
| `research::get_relepsgr` | SQLite `research_relepsgr` | ADR-079 RELEPSGR window (3y EPS CAGR vs sector median CAGR) |
| `research::get_pead` | SQLite `research_pead` | ADR-079 PEAD window (joins EarningsSurprise cache with HP bars) |
| `research::get_sizef` | SQLite `research_sizef` | ADR-079 SIZEF window (sector percentile rank of log market cap + absolute tier) |
| `research::get_momf` | SQLite `research_momf` | ADR-079 MOMF window (sector percentile rank of MOMENTUM composite) |
| `research::get_peadrank` | SQLite `research_peadrank` | ADR-079 PEADRANK window (sector percentile rank of PEAD avg 5d drift) |
| `research::get_fqm` | SQLite `research_fqm` | ADR-079 FQM window (fuses PTFS+MARGINS+ACRL, excludes leverage) |
| `research::get_revrank` | SQLite `research_revrank` | ADR-079 REVRANK window (3y revenue CAGR vs sector median CAGR) |
| `research::get_levrank` | SQLite `research_levrank` | ADR-079 LEVRANK window (sector percentile rank of D/E, risk-inverted, NEGATIVE_EQUITY short-circuit) |
| `research::get_operank` | SQLite `research_operank` | ADR-079 OPERANK window (sector percentile rank of operating margin) |
| `research::get_fqmrank` | SQLite `research_fqmrank` | ADR-079 FQMRANK window (sector percentile rank of FQM composite) |
| `research::get_liqrank` | SQLite `research_liqrank` | ADR-079 LIQRANK window (sector percentile rank of ADV$) |
| `research::get_surpstk` | SQLite `research_surpstk` | ADR-079 SURPSTK window (earnings surprise streak stat from cached EarningsSurprise rows) |
| `research::get_dvdrank` | SQLite `research_dvdrank` | ADR-079 DVDRANK window (sector percentile rank of 3y dividend CAGR) |
| `research::get_earmrank` | SQLite `research_earmrank` | ADR-079 EARMRANK window (sector percentile rank of EARM composite score) |
| `research::get_updgrank` | SQLite `research_updgrank` | ADR-079 UPDGRANK window (sector percentile rank of UPDM net_90d) |
| `research::get_gy` | SQLite `research_gy` | ADR-079 GY window (253-bar gap yearly stat from cached HP bars) |
| `research::get_des` | SQLite `research_des` | ADR-079 DES window (253-bar daily event streak stat from cached HP bars) |
| `research::get_dvdyieldrank` | SQLite `research_dvdyieldrank` | ADR-079 DVDYIELDRANK window (sector percentile rank of dividend yield, non-payers filtered) |
| `research::get_shrank` | SQLite `research_shrank` | ADR-079 SHRANK window (risk-inverted sector percentile rank of short_percent_of_float) |
| `research::get_atrann` | SQLite `research_atrann` | ADR-079 ATRANN window (Wilder 14-period ATR annualized via √252 with volatility regime label) |
| `research::get_ddhist` | SQLite `research_ddhist` | ADR-079 DDHIST window (253-bar drawdown history with max dd + longest dd + 5%/10% correction counts) |
| `research::get_priceperf` | SQLite `research_priceperf` | ADR-079 PRICEPERF window (multi-horizon total returns: 1M/3M/6M/YTD/1Y with trend label) |
| `research::get_betarank` | SQLite `research_betarank` | ADR-079 BETARANK window (risk-inverted sector percentile rank of Fundamentals.beta) |
| `research::get_pegrank` | SQLite `research_pegrank` | ADR-079 PEGRANK window (value-inverted sector percentile rank of Fundamentals.peg_ratio) |
| `research::get_fhighlow` | SQLite `research_fhighlow` | ADR-079 FHIGHLOW window (52-week high/low distance + proximity band) |
| `research::get_rvcone` | SQLite `research_rvcone` | ADR-079 RVCONE window (multi-horizon realized volatility cone 20d/60d/120d/252d) |
| `research::get_calpb` | SQLite `research_calpb` | ADR-079 CALPB window (calendar period breakdowns MTD/QTD/YTD + prior quarter/year) |
| `research::get_retskew` | SQLite `research_retskew` | ADR-079 RETSKEW window (Fisher-Pearson skewness of log returns + positive-day share) |
| `research::get_retkurt` | SQLite `research_retkurt` | ADR-079 RETKURT window (excess kurtosis + 2σ/3σ outlier counts) |
| `research::get_tailr` | SQLite `research_tailr` | ADR-079 TAILR window (95/5 and 99/1 quantile tail ratios) |
| `research::get_runlen` | SQLite `research_runlen` | ADR-079 RUNLEN window (up/down run length stats + signed current run) |
| `research::get_dayrange` | SQLite `research_dayrange` | ADR-079 DAYRANGE window (60d vs 252d daily range compression ratio) |
| `research::get_autocor` | SQLite `research_autocor` | ADR-079 AUTOCOR window (lag 1/5/10/20 return autocorrelation + momentum/mean-revert regime label) |
| `research::get_hurst` | SQLite `research_hurst` | ADR-079 HURST window (Hurst exponent via R/S analysis with 5-way persistence label) |
| `research::get_hitrate` | SQLite `research_hitrate` | ADR-079 HITRATE window (multi-horizon hit rate 5d/20d/60d/252d + up/down/flat counts + bias label) |
| `research::get_glasym` | SQLite `research_glasym` | ADR-079 GLASYM window (gain/loss magnitude asymmetry ratio + up/down day medians) |
| `research::get_volratio` | SQLite `research_volratio` | ADR-079 VOLRATIO window (up-day vs down-day volume ratio with accumulation/distribution label) |
| `research::get_drawup` | SQLite `research_drawup` | ADR-079 DRAWUP window (rally history mirror of DDHIST — deepest advance, longest rally, 5%/10% rally counts, current drawup with 5-way rally label) |
| `research::get_gapstats` | SQLite `research_gapstats` | ADR-079 GAPSTATS window (overnight gap frequency + magnitude with up/down bias label; first HP surface to read `bar.open`) |
| `research::get_volcluster` | SQLite `research_volcluster` | ADR-079 VOLCLUSTER window (ACF of r² and |r| at lags 1/5/20 — canonical GARCH-effect volatility clustering test with 5-way label) |
| `research::get_closeplc` | SQLite `research_closeplc` | ADR-079 CLOSEPLC window (average `(close-low)/(high-low)` bar-anatomy stat + near-high/near-low shares with bull/bear label) |
| `research::get_mrhl` | SQLite `research_mrhl` | ADR-079 MRHL window (AR(1) mean-reversion half-life via OLS fit on log returns with fast-revert/persistent label) |
| `research::get_downvol` | SQLite `research_downvol` | ADR-079 DOWNVOL window (downside deviation + Sortino ratio with 5-way risk-quality label) |
| `research::get_sharpr` | SQLite `research_sharpr` | ADR-079 SHARPR window (Sharpe ratio rf=0 raw + annualized with 5-way performance label) |
| `research::get_effratio` | SQLite `research_effratio` | ADR-079 EFFRATIO window (Kaufman efficiency ratio — net/gross price travel with trending/chopping label) |
| `research::get_wickbias` | SQLite `research_wickbias` | ADR-079 WICKBIAS window (upper vs lower wick share asymmetry with buyer/seller rejection label) |
| `research::get_volofvol` | SQLite `research_volofvol` | ADR-079 VOLOFVOL window (CV of rolling 20d realized vol — stable/chaotic vol-regime label) |
| `research::get_calmar` | SQLite `research_calmar` | ADR-079 CALMAR window (Calmar ratio — annualized return / max drawdown with 5-way label) |
| `research::get_ulcer` | SQLite `research_ulcer` | ADR-079 ULCER window (Ulcer index + Martin ratio — continuous drawdown-weighted risk with pain-level label) |
| `research::get_varratio` | SQLite `research_varratio` | ADR-079 VARRATIO window (Lo-MacKinlay variance ratio at horizons 2/5/10/20 — formal random-walk hypothesis test) |
| `research::get_amihud` | SQLite `research_amihud` | ADR-079 AMIHUD window (Amihud illiquidity ratio — |r|/dollar_volume microstructure liquidity scalar) |
| `research::get_jbnorm` | SQLite `research_jbnorm` | ADR-079 JBNORM window (Jarque-Bera normality test — combined skewness+kurtosis χ²(2) test with exact p-value) |
| `research::get_omega` | SQLite `research_omega` | ADR-079 OMEGA window (Omega ratio at threshold 0 — distribution-free gains/losses partition with 5-way label) |
| `research::get_dfa` | SQLite `research_dfa` | ADR-079 DFA window (Detrended Fluctuation Analysis α — Hurst alternative robust to non-stationarity) |
| `research::get_burke` | SQLite `research_burke` | ADR-079 BURKE window (Burke ratio — event-weighted drawdown-adjusted annualized return with 5-way label) |
| `research::get_monthseas` | SQLite `research_monthseas` | ADR-079 MONTHSEAS window (monthly seasonality — 12-month hit rate + mean return grid; full-HP-cache scan, not 253-window) |
| `research::get_rollsprd` | SQLite `research_rollsprd` | ADR-079 ROLLSPRD window (Roll's 1984 implicit bid-ask spread in bps — microstructure companion to AMIHUD) |
| `research::get_parkinson` | SQLite `research_parkinson` | ADR-079 PARKINSON window (Parkinson 1980 H-L range-based vol — 5.2× more efficient than close-to-close; first entry of the OHLC-vol family) |
| `research::get_gkvol` | SQLite `research_gkvol` | ADR-079 GKVOL window (Garman-Klass 1980 OHLC vol — H-L range + C-O drift, 7.4× efficiency; the textbook industrial range-vol estimator) |
| `research::get_rsvol` | SQLite `research_rsvol` | ADR-079 RSVOL window (Rogers-Satchell 1991 drift-independent OHLC vol — unbiased under non-zero drift; completes the OHLC-vol family) |
| `research::get_cvar` | SQLite `research_cvar` | ADR-079 CVAR window (Conditional VaR / Expected Shortfall at 5% and 1% — coherent Basel III downside-risk measure distinct from TAILR shape + DOWNVOL scale) |
| `research::get_doweffect` | SQLite `research_doweffect` | ADR-079 DOWEFFECT window (day-of-week intraday O→C seasonality — weekday calendar companion to MONTHSEAS; full-HP-cache scan) |
| `research::get_sterling` | SQLite `research_sterling` | ADR-079 STERLING window (Sterling ratio — annualized return over mean of N worst distinct drawdown events; middle ground between CALMAR single-worst and BURKE sum-of-squares) |
| `research::get_kellyf` | SQLite `research_kellyf` | ADR-079 KELLYF window (Kelly fraction `(b·p − q)/b` — forward-looking optimal-leverage scalar; first non-realized packet surface) |
| `research::get_ljungb` | SQLite `research_ljungb` | ADR-079 LJUNGB window (Ljung-Box Q at lag 10 — joint autocorrelation / white-noise test with Wilson-Hilferty χ²(10) p-value) |
| `research::get_runstest` | SQLite `research_runstest` | ADR-079 RUNSTEST window (Wald-Wolfowitz runs test — formal inferential randomness test on sign sequence with z-stat + two-sided p-value via A&S 7.1.26 normal CDF) |
| `research::get_zeroret` | SQLite `research_zeroret` | ADR-079 ZERORET window (Lesmond-Ogden-Trzcinka zero-return-day fraction — microstructure liquidity proxy distinct from AMIHUD impact + ROLLSPRD spread) |
| `research::get_psr` | SQLite `research_psr` | ADR-079 PSR window (Probabilistic Sharpe Ratio — Lopez de Prado 2012; corrects SHARPR for skewness/kurtosis and reports a probability the true SR exceeds a benchmark) |
| `research::get_adf` | SQLite `research_adf` | ADR-079 ADF window (Dickey-Fuller unit-root test on log-price with hardcoded MacKinnon 1996 critical values; rejection ⇒ stationary / mean-reverting) |
| `research::get_mnkendall` | SQLite `research_mnkendall` | ADR-079 MNKENDALL window (Mann-Kendall nonparametric trend test — distribution-free z-statistic over all i<j sign comparisons; complements ADF with trend-presence instead of stationarity) |
| `research::get_bipower` | SQLite `research_bipower` | ADR-079 BIPOWER window (Barndorff-Nielsen & Shephard 2004 bipower variation; jump_ratio = 1 − BPV/RV decomposes realized variance into continuous + jump components) |
| `research::get_dddur` | SQLite `research_dddur` | ADR-079 DDDUR window (drawdown duration statistics: event count + max/mean/median bar-duration + % of time underwater — duration-axis companion to CALMAR/BURKE/STERLING magnitude) |
| `research::get_hilltail` | SQLite `research_hilltail` | ADR-079 HILLTAIL window (Hill 1975 tail-index estimator on \|r\| plus separate left/right-tail α — nonparametric power-law exponent well-defined under infinite-variance heavy tails where KURT/JBNORM fail) |
| `research::get_archlm` | SQLite `research_archlm` | ADR-079 ARCHLM window (Engle 1982 ARCH Lagrange-multiplier test at q=5 lags — formal conditional-heteroskedasticity test with χ²(5) critical values; first packet surface on second-moment memory) |
| `research::get_painratio` | SQLite `research_painratio` | ADR-079 PAINRATIO window (Zephyr/FIBA Pain Index = mean\|dd\|, Pain Ratio = ann_return/pain_index — L¹ drawdown norm completing the magnitude-norm sextet with CALMAR/BURKE/STERLING/ULCER/DDDUR) |
| `research::get_cusum` | SQLite `research_cusum` | ADR-079 CUSUM window (Brown-Durbin-Evans 1975 OLS CUSUM structural-break test — first formal mean-stability test in the packet with KS critical values {10%=1.22, 5%=1.36, 1%=1.63}) |
| `research::get_cfvar` | SQLite `research_cfvar` | ADR-079 CFVAR window (Cornish-Fisher 1938 modified VaR — Gauss quantile adjusted for skew γ₃ and excess kurt γ₄ with skew-term vs kurt-term attribution) |
| `research::get_entropy` | SQLite `research_entropy` | ADR-079 ENTROPY window (Shannon entropy H = −Σ pᵢ log₂(pᵢ) over return histogram — first information-theoretic distributional measure with normalised H/H_max ∈ [0,1]) |
| `research::get_rachev` | SQLite `research_rachev` | ADR-079 RACHEV window (Rachev ratio = ES_α(+R)/ES_α(−R) at 5% and 1% — first asymmetric tail comparison ratio; Rachev > 1 ⇒ upside tail outweighs downside) |
| `research::get_gpr` | SQLite `research_gpr` | ADR-079 GPR window (Gain-to-Pain Ratio = Σ rₜ / Σ \|min(rₜ,0)\| + Profit Factor — Schwager's return-per-realized-loss metric, distinct from drawdown-based Pain Ratio) |
| `research::get_pacf` | SQLite `research_pacf` | ADR-079 PACF window (partial autocorrelation at lags 1–5 via Durbin-Levinson with Bartlett 95% band — first lag-specific dependence decomposition of LJUNGB joint autocorrelation) |
| `research::get_apen` | SQLite `research_apen` | ADR-079 APEN window (Pincus 1991 approximate entropy, m=2, r=0.2·σ — first nonlinear predictability measure capturing short-range pattern regularity) |
| `research::get_upr` | SQLite `research_upr` | ADR-079 UPR window (Sortino & van der Meer 1991 Upside Potential Ratio = UPM₁/√LPM₂ — first asymmetric capture-vs-risk ratio) |
| `research::get_levereff` | SQLite `research_levereff` | ADR-079 LEVEREFF window (Black 1976 leverage effect: corr(rₜ, rₜ₊₁²) + asymmetric vol ratio — first return→vol feedback measure) |
| `research::get_drawdar` | SQLite `research_drawdar` | ADR-079 DRAWDAR window (Chekhlov et al. 2005 Drawdown-at-Risk + CDaR at 5%/1% — first quantile-based drawdown risk measure) |
| `research::get_varhalf` | SQLite `research_varhalf` | ADR-079 VARHALF window (AR(1) on rolling 20d RV → half-life = −ln(2)/ln(β) — first vol-regime persistence measure) |
| `research::get_gini` | SQLite `research_gini` | ADR-079 GINI window (Gini coefficient on |log returns| — first return-concentration measure orthogonal to KURT/VOLCLUSTER/BIPOWER) |
| `research::get_sampen` | SQLite `research_sampen` | ADR-079 SAMPEN window (Richman & Moorman 2000 Sample Entropy, m=2, r=0.2·σ, self-match-excluded — modern standard complement to APEN) |
| `research::get_permen` | SQLite `research_permen` | ADR-079 PERMEN window (Bandt & Pompe 2002 Permutation Entropy, m=3 ordinal patterns — temporal ordering structure invisible to ENTROPY/APEN/SAMPEN) |
| `research::get_recfact` | SQLite `research_recfact` | ADR-079 RECFACT window (Recovery Factor = cum return / \|max dd\| — first raw-cumulative recovery metric distinct from annualized ratios) |
| `research::get_kpss` | SQLite `research_kpss` | ADR-079 KPSS window (Kwiatkowski-Phillips-Schmidt-Shin 1992 stationarity test — formal complement to ADF unit-root test) |
| `research::get_specent` | SQLite `research_specent` | ADR-079 SPECENT window (Spectral Entropy via DFT — Shannon entropy of normalised PSD, first frequency-domain periodicity measure) |
| `research::get_robvol` | SQLite `research_robvol` | ADR-079 ROBVOL window (MAD/0.6745 + IQR/1.349 robust σ + classical σ — first outlier-resistant vol surface, exposes classical-σ inflation by extreme days) |
| `research::get_renyient` | SQLite `research_renyient` | ADR-079 RENYIENT window (Rényi entropy at α=2, collision entropy, Σ pᵢ² concentration — first quadratic-order entropy, quadratic probability weighting vs Shannon's log) |
| `research::get_retquant` | SQLite `research_retquant` | ADR-079 RETQUANT window (9-point return quantile profile P1..P99 + IQR + tail asymmetry — first dense non-parametric distribution snapshot) |
| `research::get_msent` | SQLite `research_msent` | ADR-079 MSENT window (Costa-Goldberger-Peng 2005 Multiscale SampEn at τ=1..5 with fixed tolerance — first scale-dependent complexity measure) |
| `research::get_ewmavol` | SQLite `research_ewmavol` | ADR-079 EWMAVOL window (RiskMetrics EWMA variance with λ=0.94 — first adaptive-weighted vol surface with ewma/classical ratio as regime flag) |
| `research::get_ksnorm` | SQLite `research_ksnorm` | ADR-079 KSNORM window (Kolmogorov-Smirnov one-sample normality test against N(μ̂,σ̂²) with three-way 10%/5%/1% rejection flags — first omnibus goodness-of-fit surface) |
| `research::get_adtest` | SQLite `research_adtest` | ADR-079 ADTEST window (Anderson-Darling tail-weighted normality test with Stephens 1986 small-sample-corrected A² and p-value approximation — first tail-focused normality surface) |
| `research::get_lmom` | SQLite `research_lmom` | ADR-079 LMOM window (Hosking 1990 L-moments L1..L4 + L-ratios τ3 τ4 via unbiased probability-weighted moments — first robust-moment surface with bounded L-skew/L-kurt) |
| `research::get_kylelam` | SQLite `research_kylelam` | ADR-079 KYLELAM window (Kyle 1985 price-impact λ = cov(\|Δp\|,V)/var(V) regression slope + correlation + R² — first linear-regression liquidity surface, distinct from AMIHUD ratio) |
| `research::get_peakover` | SQLite `research_peakover` | ADR-079 PEAKOVER window (Peaks-Over-Threshold exceedance counts + mean/max excesses at P95 and P99 thresholds — first EVT/GPD-foundation surface) |
| `research::get_higuchi` | SQLite `research_higuchi` | ADR-079 HIGUCHI window (Higuchi 1988 fractal dimension of cumulative log-return walk, k_max=10, with SMOOTH/PERSISTENT/RANDOM/ROUGH regime labels — first direct geometric FD surface) |
| `research::get_pickands` | SQLite `research_pickands` | ADR-079 PICKANDS window (Pickands 1975 extreme-value γ̂ valid across Fréchet/Gumbel/Weibull domains — first EV-domain-agnostic tail estimator, cross-checks HILLTAIL Hill α) |
...
| `research::get_tsi` | SQLite `research_tsi` | ADR-079 TSI window (Blau 1991 True Strength Index = 100·EMA₁₃(EMA₂₅(ΔP))/EMA₁₃(EMA₂₅(|ΔP|)) — first double-smoothed momentum oscillator) |
| `research::get_garch11` | SQLite `research_garch11` | ADR-079 GARCH11 window (Bollerslev 1986 GARCH(1,1) conditional-variance fit via coordinate-descent grid MLE — first parametric volatility persistence model, shipping α/β/persistence/half-life/unconditional variance) |
| `research::get_sadf` | SQLite `research_sadf` | ADR-079 SADF window (Phillips-Wu-Yu 2011 Sup-ADF explosive-root / bubble test — first asymmetric tail-window stationarity test, complements ADF/KPSS/PPROOT) |
| `research::get_cordim` | SQLite `research_cordim` | ADR-079 CORDIM window (Grassberger-Procaccia 1983 correlation dimension D2 at m=3 embedding — first nonlinear-dynamics dimension surface, distinct from monofractal Hurst/DFA/Higuchi) |
...
- `native/src/app.rs::maybe_queue_ingest_from_ai_response()` — ADR-096 Return Path auto-ingest hook
