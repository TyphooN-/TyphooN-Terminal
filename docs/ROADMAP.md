...
- [x] **M2 implementation gate passed.** The versioned 46-metric registry, sealed reports, hand-computed MAE/MFE/calendar-equity/two-leg goldens, deterministic hybrid replay, repaint evidence, richer-execution kernel, verified native report/replay/comparison/overlay workflows, identity-bound sub-bar inputs, and bounded reference-data ingestion/selection satisfy ADR-135's normative gate. Execution-config v5 additionally binds one corporate-action artifact per symbol and seals an explicit fractional-remainder policy; v3/v4 artifacts retain their original identities. **Availability exclusions remain fail-closed:** no exchange/contracted-vendor calendar or corporate-action feed, entitled historical/live borrow/financing/funding/FX inputs, or immutable versioned tick corpus is present. See ADR-135 §13.1
- [x] **M3 gate passed.** Native general and guided NNFX builders share canonical `StrategyIr`; canonical strategies save/reload through the bounded databank worker with cancellation, failure, and stale-response handling; and the prepared-page browser provides bounded filters, indexed sorting, paging, and comparison without render-thread SQLite. The user-driven flow binds the exact reloaded strategy to selected immutable parent/finer datasets, execution config, seed, and active chart; executes through the existing bounded verified-run worker; appends the sealed report's real metric vector; and reruns it for exact databank verification. Tests cover every profile × entry × direction × rule-toggle combination, each selectable indicator kind in every role slot, and a 100,000-run browser path that installs only a capped worker-prepared page (ADR-135 M3)
- [x] **M4 executable retest checkpoint landed; M4 gate remains open.** A content-bound retest request verifies exact leased bars, assembles a `VerifiedRun`, executes the canonical simulator, seals and re-verifies its report, projects only a defined report metric, runs the declared robustness stages, and records evaluation N. Immutable SQLite evidence and one-way holdout-consumption audit use bounded indexed queries; a bounded off-thread worker exposes backpressure, cancellation, stale-safe request ids, and failure without UI installation. Adaptive Bayesian search, complete OOS/walk-forward execution, the remaining perturbation families, native analysis views, and the literal end-to-end M4 adversarial gate remain open
- [ ] Execute M4–M8 in prerequisite order; later foundations are not milestone completion until each written gate passes
...
- [ ] Multi-symbol clock ordering, warm-up/missing-data semantics and event-based corporate actions are landed in the kernel; action/calendar artifacts materialize from persisted source records with explicit authority and provenance, and one corporate-action artifact per symbol now accumulates in a config. Strategy-level multi-timeframe synchronization and authoritative exchange/vendor source feeds remain open
