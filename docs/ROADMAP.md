...
- [x] **M3 native builder/databank checkpoint (milestone still open).** The native general and NNFX builders share canonical `StrategyIr`; canonical strategies save/reload through the bounded databank worker with cancellation, failure, and stale-response handling; and the prepared-page browser provides bounded filters, sort, paging, and comparison selection without render-thread SQLite. A model-level save-clear-reload test reruns through the existing verified sub-bar simulation/report path and verifies the stored metric vector exactly. The literal user-driven dataset/config/run/append/rerun flow, exhaustive guided-configuration round trips, and explicit render-path proof over the 10⁵-run corpus remain open (ADR-135 M3)
