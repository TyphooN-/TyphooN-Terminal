...
- [ ] **M2 in progress — gate not passed.** Landed: a versioned 46-metric registry with written contracts, typed `Undefined { reason }`, and compact hand-derived golden values/reasons covering every registry id; MAE/MFE and capture efficiency; reconciling daily/weekly ISO/monthly/annual equity; distribution/tail/stability families; diagnostics; and a sealed, byte-bounded, tamper-evident report artifact (`strategy_metrics.rs`, `strategy_report.rs`). The canonical IR lowers fixed, percent-of-entry, and ATR-multiple `trade_management` rules into the NNFX protective lifecycle: independent per-leg stop/target OCO groups, break-even, only-ever-tightening trails, time stops, long/short symmetry, and stale-bracket retirement (`strategy_interpreter.rs`, `strategy_protective.rs`). Verified run assembly binds the exact content-addressed intervention log and now also resolves sealed, bounded repaint QA artifacts in stable indicator order; run-manifest v4 seals artifact identity plus clean/warning acknowledgement into `run_id`, rejects missing/mismatched/duplicate/unexpected or dataset-inconsistent evidence, and propagates that identity through sealed reports (`strategy_run.rs`, `strategy_repaint.rs`, `strategy_report.rs`). Deterministic hybrid replay reproduces the recorded ledger byte for byte (`strategy_intervention.rs`). This closes the metric-golden and repaint-artifact slices, but **does not complete M2**. The protective lifecycle remains implemented pending its final verification command set. Still open for M2 delivery: execution realism §6.6–§6.9, the M1-deferred financing/borrow/funding/currency accrual, and the chart trade overlay — see ADR-135 §13.1
