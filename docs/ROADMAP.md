...
- [ ] **M2 in progress — gate not passed.** Landed: a versioned 43-metric registry with written formulas, units, periodicity, annualization and degenerate-case contracts; typed `Undefined { reason }` in place of every sentinel; MAE/MFE and capture efficiency; real calendar resampling (daily/weekly ISO/monthly/annual) that reconciles exactly with the closed-trade list; distribution/tail and stability families; a ledger cost/rejection diagnostic block; and a sealed, byte-bounded, tamper-evident report artifact bound to the verified run id and metrics version (`strategy_metrics.rs`, `strategy_report.rs`). and the NNFX two-leg order lifecycle — independent per-leg stop/target in their own OCO groups, break-even, only-ever-tightening trails and a time stop — proven by a hand-computed scenario that asserts every fill, fee, realized PnL and equity value exactly (`strategy_metrics.rs`, `strategy_report.rs`, `strategy_protective.rs`). and the §11.5 repainting diagnostic, which names the exact output, bar, event and before/after values of every closed-bar value a later event rewrote, with declared revision windows for honestly provisional output (`strategy_repaint.rs`). and deterministic hybrid replay — a content-addressed operator intervention log anchored to decision ordinals, whose replay reproduces a recorded session's ledger byte for byte (`strategy_intervention.rs`). Still open for the M2 gate: compiling IR `trade_management` into a protective plan so canonical strategies can request the two-leg lifecycle. Still open for M2 delivery: execution realism §6.6–§6.9, the M1-deferred financing/borrow/funding/currency accrual, and the chart trade overlay — see ADR-135 §13.1
