...
- [ ] **M2 in progress — gate not passed.** Landed: a versioned 43-metric registry with written formulas, units, periodicity, annualization and degenerate-case contracts; typed `Undefined { reason }` in place of every sentinel; MAE/MFE and capture efficiency; real calendar resampling (daily/weekly ISO/monthly/annual) that reconciles exactly with the closed-trade list; distribution/tail and stability families; a ledger cost/rejection diagnostic block; and a sealed, byte-bounded, tamper-evident report artifact bound to the verified run id and metrics version (`strategy_metrics.rs`, `strategy_report.rs`). The canonical IR now lowers fixed, percent-of-entry, and ATR-multiple `trade_management` rules into the NNFX protective lifecycle: independent per-leg stop/target OCO groups, break-even, only-ever-tightening trails, time stops, long/short symmetry, and stale-bracket retirement (`strategy_interpreter.rs`, `strategy_protective.rs`). Verified run assembly now binds the exact content-addressed intervention log named by the manifest and rejects missing, mismatched, or unexpected logs (`strategy_run.rs`, `strategy_intervention.rs`). These two source/test slices are implemented **pending final verification**; this does not complete M2. Also landed: the §11.5 repainting diagnostic with exact output/bar/event/before-after evidence and declared revision windows (`strategy_repaint.rs`), plus deterministic hybrid replay whose recorded operator actions reproduce the ledger byte for byte (`strategy_intervention.rs`). Still open for the M2 gate: a hand-verified golden assertion for every registered metric. Still open for M2 delivery: execution realism §6.6–§6.9, the M1-deferred financing/borrow/funding/currency accrual, and the chart trade overlay — see ADR-135 §13.1
