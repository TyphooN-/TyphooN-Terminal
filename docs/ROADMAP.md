...
- [ ] **M2 in progress — gate not passed.** Proven foundations now include the versioned 46-metric registry and sealed report artifact; hand-computed MAE/MFE, calendar-equity and protective two-leg gates; deterministic hybrid replay and sealed repaint evidence; and the richer-execution kernel (`strategy_metrics.rs`, `strategy_report.rs`, `strategy_protective.rs`, `strategy_intervention.rs`, `strategy_repaint.rs`, `strategy_simulator.rs`). Execution-config v3 seals a shared bar-volume participation cap, per-instrument ticks/calendars/currencies/financing, closed-session queue/reject policy, corporate-action schedule, conversion table and declared sub-bar timeframe. Run-manifest v5 now identity-binds finer immutable datasets and verified assembly rejects missing, unexpected, tampered, symbol/policy/timeframe-incompatible, gapped or overlapping sub-bar inputs before bounded causal simulation. The native Backtest Engine now also selects bounded parent/finer Dataset Inspector records plus existing sealed strategy/config/manifest artifacts, runs the verified sub-bar worker off-thread, rejects stale chart generations, and installs prepared overlays/reports. The verified-pair viewer provides summary/metric/trade/equity/drawdown sections, timeline-bounded replay, identity-preserving report/simulation export, future-data-safe overlays and trade inspection, plus bounded manual-intervention authoring/loading/sealing/export at revealed causal decisions. A sealed or loaded intervention remains a **candidate log** with verified content identity, not a replay-verified ledger: native manifest binding and a successful `run_verified_simulation_with_intervention` rerun are still required. **Still open:** that native manifest-bound rerun/promotion flow; comparison/distribution and remaining report workflows; authoritative calendar/corporate-action ingestion; and entitled historical/live borrow, financing, funding and currency inputs. Unavailable paid/live rate inputs fail closed rather than becoming zero. True tick fidelity is unavailable until TyphooN retains immutable versioned tick history. **M2 is not complete** — see ADR-135 §13.1
