...
- [ ] **M2 in progress — gate not passed.** Proven foundations now include the versioned 46-metric registry and sealed report artifact; hand-computed MAE/MFE, calendar-equity and protective two-leg gates; deterministic hybrid replay and sealed repaint evidence; and the richer-execution kernel (`strategy_metrics.rs`, `strategy_report.rs`, `strategy_protective.rs`, `strategy_intervention.rs`, `strategy_repaint.rs`, `strategy_simulator.rs`). Execution-config v3 seals a shared bar-volume participation cap, per-instrument ticks/calendars/currencies/financing, closed-session queue/reject policy, corporate-action schedule, conversion table and declared sub-bar timeframe. Run-manifest v5 now identity-binds finer immutable datasets and verified assembly rejects missing, unexpected, tampered, symbol/policy/timeframe-incompatible, gapped or overlapping sub-bar inputs before bounded causal simulation. The native Backtest Engine selects bounded parent/finer Dataset Inspector records plus existing sealed strategy/config/manifest artifacts, runs the verified sub-bar worker off-thread, rejects stale chart generations, and installs prepared overlays/reports. It now also selects every manifest-bound parent dataset and sealed candidate InterventionLog, performs bounded worker-side artifact/store loading, requires exact manifest/log identity, assembles and executes the intervention-aware verified-run APIs, seals and verifies the replay report, and only then promotes it; stale, cancelled and failed results preserve the existing report while full run/log/report identities remain visible. The verified-pair viewer provides summary/metric/trade/equity/drawdown sections, timeline-bounded replay, identity-preserving report/simulation export, future-data-safe overlays and trade inspection, plus bounded manual-intervention authoring/loading/sealing/export at revealed causal decisions. **Still open:** comparison/distribution and remaining report workflows; authoritative calendar/corporate-action ingestion; and entitled historical/live borrow, financing, funding and currency inputs. Unavailable paid/live rate inputs fail closed rather than silently becoming zero. True tick fidelity remains externally blocked until TyphooN retains an immutable, versioned tick-history corpus (ADR-135 §13.1)
...
- [ ] Multi-symbol clock ordering, warm-up/missing-data semantics and event-based corporate actions are landed in the kernel; strategy-level multi-timeframe synchronization, authoritative action/calendar ingestion and native presentation remain open
