...
ADR-074 documented a comprehensive performance audit but deferred 7 items as
...
This ADR closes the loop on the deferred items from ADR-074. The principle
...
- Alpaca rate-limit header test uses `#[tokio::test]` instead of constructing a
  private runtime.
- ADR-089's macOS AC-power item is no longer deferred: the auto-compaction gate
