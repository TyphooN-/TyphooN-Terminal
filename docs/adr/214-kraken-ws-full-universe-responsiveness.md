# ADR-214: Kraken WS Full-Universe Streaming Under egui Responsiveness Budget

**Status:** Accepted
**Date:** 2026-05-24
**Related:** ADR-205 (zstd compression policy), ADR-052 (performance architecture), ADR-210 (Kraken async bar sync), ADR-213 (per-frame O(1) discipline), `native/src/app/kraken_ohlc_ws.rs`, `native/src/app/app_runtime.rs`, `engine/src/broker/kraken/ohlc_ws.rs`, `engine/src/core/cache.rs::merge_bars_fast`

## Context

ADR-210 covers REST-side Kraken sync pacing. This ADR covers the public
...
~1 req/sec global counter (see ADR-210/-211). At ≈1500 Spot pairs × 8
timeframes that is unreachable through REST alone for the low TFs. WS push
exists exactly to close that gap. ADR-210's REST-side pacing is unchanged;
...
them at zstd-22. ADR-205 §5 captures the formal carve-out. Operators do
...
- Replacing REST. WS is forward streaming; REST still handles cold-start
  deep history. ADR-210 covers REST; this ADR is strictly the WS side.
