...
- `strategy_ir.rs`: canonical strategy IR, semantic/type/resource validation, content-addressed
  ...
  artifact ids the settings were materialized from. Schema v5 binds one corporate-action artifact
  per symbol and seals the fractional-remainder policy. Both bumps are compatible extensions:
  v3 and v4 remain loadable with their sealed ids byte for byte, while content introduced by a
  later schema is refused under an earlier one. True tick fidelity is deliberately unrepresentable
  while TyphooN retains no versioned tick history.
...
**M2 — implementation gate passed 2026-07-29; externally sourced fidelity remains
availability-gated.** Every normative gate clause below now has test evidence. Exchange/vendor
calendar and corporate-action feeds, entitled historical/live rates, and retained tick history
remain unavailable capabilities; the implementation refuses them rather than weakening the gate:
...
| §6.8 corporate actions and adjustment consistency | **Met at the implementation boundary; automated source fetchers remain availability-gated** | `split_then_dividend_adjusts_live_position_and_cash_in_canonical_order` pins effective-time split-before-dividend units, basis and cash. `reverse_split_cash_in_lieu_settles_the_fraction_at_the_declared_post_split_mark` pins whole-unit retention, exact settlement cash, basis and equity; `reverse_split_refuses_an_unpriced_fraction_instead_of_guessing` proves the fail-closed alternative. `verified_run_assembly_refuses_corporate_actions_already_baked_into_prices` proves the identity-bearing run boundary rejects double-counting against adjusted prices. Materialization tests pin canonical source/action ordering, timestamp strictness, duplicate/conflict handling and raw-byte identity. Fetchers that persist authoritative source records remain open because no such feed is currently integrated |
| Reference-data identity, restart recovery and schema compatibility | **Met for the boundary that exists** | `declaration_order_is_canonical_and_retrieval_time_is_audit_only` proves two declaration orders and two retrieval times seal one id while an as-of change does not; bounded codec tests pin bounded-before-decode, unknown-field, non-canonical-encoding and tampered-id refusals. `restart_identity_and_current_execution_binding_round_trip` stores both artifact families, reloads them from ids alone and re-derives the sealed config id. v3 and v4 identity vectors remain loadable and unchanged; older schemas cannot smuggle newer fields. Multi-symbol tests prove one corporate-action artifact per symbol accumulates canonically, duplicate/replacement ambiguity is refused, and every binding contributes a symbol's events |
| Native reference-data inspection, materialization and selection | **Landed for the local-snapshot path** | `strategy_reference_data_worker/tests.rs::inspecting_a_snapshot_reports_authority_coverage_and_never_promotes` proves inspection runs off the submitting thread, reports authority/coverage/completeness, and leaves the store empty; `rule_only_keyless_and_unreachable_sources_are_blocked_with_a_reason` proves each non-authoritative class is reported blocked *with the refusal* and then actually fails to materialize; `materializing_then_selecting_seals_a_config_and_labels_its_authority` seals both artifacts, lists them, accumulates the action artifact with prior symbol bindings, and rebuilds the same current-schema config id while labelling the pair non-authoritative because one source is a cache of a keyless feed; `the_job_queue_is_bounded_and_reports_backpressure` pins non-blocking backpressure, the per-poll ceiling and a terminating drop of a backlogged worker; `a_listing_bounded_below_the_store_reports_what_it_omitted` proves a truncated listing counts what it hid; `unreadable_snapshots_and_unknown_artifact_ids_fail_without_stopping_the_worker` covers missing, garbage, directory and path-as-id inputs. The panel view model is pure and separately tested in `typhoon-native/src/app/reference_data_model/tests.rs`: no defaults, promotion gated on the worker's own dry run, superseded replies dropped, slot changes discarding a stale preparation, vanished artifacts clearing their slot, omissions surfaced, and backpressure freeing the pending slot. Sealing a source below exchange/vendor authority additionally requires an explicit operator acknowledgement that starts false and is cleared by every new inspection (`a_non_authoritative_source_needs_an_explicit_acknowledgement`), so a durable artifact from a rule-derived or keyless feed is a stated decision rather than a default. Snapshot selection uses the platform file dialog, which yields a path only — the file is opened, bounded and decoded on the worker |
...
M2's normative implementation gate is complete. The remaining items are external capability
availability, not permission to substitute weaker data: **no exchange or contracted-vendor feed
is integrated**, so in practice only `UnverifiedPublic` and `DerivedRule` source records exist to
seal — and neither may back a run that requires authority. Entitled historical/live borrow,
financing, funding and currency inputs are likewise unavailable. Automated fetchers can only land
once an authoritative source contract exists; until then the snapshot ingestion boundary is the
honest seam. True tick fidelity remains unavailable until TyphooN retains an immutable, versioned
...
### M2 — Metrics, reporting, and richer execution — **implementation gate passed 2026-07-29**
**Prereqs:** M1.
