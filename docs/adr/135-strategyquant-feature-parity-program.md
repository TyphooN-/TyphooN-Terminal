...
| Native report/results and chart-overlay slice | **Partial; verified viewer, candidate authoring and export landed** | The Backtest Engine graphically selects one sealed report artifact plus its paired `SimulationReport` JSON and performs bounded reads, decoding, digest verification and presentation indexing on a worker. It now adds summary/metric/trade/equity/drawdown sections; identity-preserving report/simulation export; and bounded authoring, loading, sealing and export of intervention logs at actually revealed causal decision indices. Sealing is explicitly a **candidate-log** state: loading verifies content identity only, and neither path claims ledger replay without the manifest-bound run inputs. Worker-side export re-verifies intervention bytes; replay hides completed-run summaries/metrics/curves/trade lists, future open-trade outcomes and future chart overlays. Native manifest binding plus execution of `run_verified_simulation_with_intervention`, comparison/distribution workflows, and native sub-bar assembly remain open |
| Replaying a recorded hybrid run reproduces its ledger bit-for-bit | **Met at the engine boundary; native candidate rerun wiring open** | `strategy_intervention/tests.rs::replaying_a_recorded_hybrid_run_reproduces_its_ledger_bit_for_bit` records a session where an automated strategy and an operator both act under costs, intrabar resolution and seeded latency, then replays the sealed log and compares the serialized ledgers byte for byte. `verified_hybrid_execution_is_identity_bound_exact_and_fail_closed` additionally requires the supplied log id to match the verified run manifest, proves exact repeated output and actual application, and rejects missing, unexpected, mismatched and trailing interventions; malformed loaded actions/content identities fail before assembly/replay. `a_hybrid_run_replays_identically_from_a_round_tripped_log` proves persistence, and `the_operator_actions_are_what_make_the_two_ledgers_match` is the non-vacuous control |
...
Remaining M2 packet: wire a native candidate log into a newly manifest-bound verified run and only
promote it after `run_verified_simulation_with_intervention` succeeds; complete comparison,
distribution and remaining results/report workflows; select and assemble the now identity-bound
sub-bar datasets through the native path; add exchange/vendor ingestion for historical corporate
