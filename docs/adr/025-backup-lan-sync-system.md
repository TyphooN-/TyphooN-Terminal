# ADR-025: Backup + LAN Sync System

**Status:** Implemented
**Date:** 2026-03-23

> **Note:** Extends the portable backup and SQLite cache work that predated the retained ADR set. Later LAN security/deployment updates are tracked by [ADR-045](045-lan-sync-tls-encryption.md), [ADR-046](046-lan-sync-remote-request-forwarding.md), [ADR-058](058-lan-sync-bandwidth-optimization.md), [ADR-090](090-headless-lan-server-deployment.md), and [ADR-093](093-lan-observability-and-kafka-deployment.md).
...
- Optional password protection: `--cache-backup-passphrase` uses the ADR-091
...
WebSocket messages over TLS-encrypted TCP (wss://), authenticated with PBKDF2-derived shared secret + HMAC-SHA256 challenge-response. See [ADR-045](045-lan-sync-tls-encryption.md) for TLS implementation details (ephemeral self-signed certs, no pinning).
...
- No certificate pinning (ephemeral certs regenerated on every server restart; see ADR-045)
