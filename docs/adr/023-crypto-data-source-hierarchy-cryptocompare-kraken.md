...
CryptoCompare handles deep history (2000 bars/request, back to 2010). Kraken handles recent public OHLCV and gap-fill without requiring credentials, with Spot public pacing/cooldown per ADR-095. Rate limit retry with exponential backoff (2s->16s) remains specific to CryptoCompare.
