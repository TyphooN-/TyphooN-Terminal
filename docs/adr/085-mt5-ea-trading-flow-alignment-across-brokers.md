# ADR-085: MT5 EA Trading-Flow Alignment Across Brokers

**Date:** 2026-04-24
**Status:** Accepted
**Related:** `engine/src/broker/alpaca.rs`, `engine/src/broker/tastytrade.rs`, `engine/src/broker/kraken_broker.rs`, `native/src/app.rs`, ADR-009 (multi-broker), ADR-018 (tastytrade), ADR-051 (kraken)
...
`BrokerCmd` / `BrokerMsg` channel architecture from ADR-009. Each broker has
...
- **Future brokers must implement the same surface.** Any new broker added
  under ADR-009 has to expose `get_position_summaries`,
