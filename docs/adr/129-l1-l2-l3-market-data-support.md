...
- half-open watchdog (code): the Kraken ticker/book/trade lanes previously relied on the app-level ping's *write* failing to notice a half-open socket (server silently stops sending while TCP stays up) — on Linux a stuck write can sit in retransmit for **15-30 min**, freezing L1/L2 with no reconnect. They now carry a stale-frame watchdog (`KRAKEN_WS_V2_STALE_AFTER` = 60 s, `ws_v2_connection_is_stale`): any received frame — data **or** Kraken's ~1 s `heartbeat` — refreshes it, and a lapse past the window forces a reconnect, bounding half-open detection to ~60-90 s. Safe against false trips because an alive Kraken v2 connection is never silent (heartbeats). Mirrors Alpaca's `STALE_AFTER`. The level3 real-feed loop shared the same weakness *and* silently swallowed read errors / stream-close on the auth path (the `_` arm did nothing when a token was present, so the outer reconnect never fired) — it now surfaces those as `Err` and applies the same stale watchdog, so **all four Kraken WS lanes self-heal** on error, close, or silence. (Real L3 stays entitlement-gated; the sim/demo path is unchanged.)
---
...
Future broker modules, if deliberately approved, must plug into the same capability model instead of forking UI semantics. No additional broker is currently scheduled on `master`. Each broker advertises supported L1/L2/L3 capabilities, entitlement constraints, freshness semantics, and snapshot-vs-stream behavior; UI surfaces consume normalized capabilities and degrade cleanly when a broker lacks a level.
...
- [x] Reconnect parity (robustness): the Kraken ticker/book/trade/level3 WS lanes share one self-healing reconnect (capped exponential backoff via `compute_*_reconnect_backoff`, terminal only when the consumer drops the channel — never on a transient failure burst). The public-trade tape (executed markers + M1/M5 forming-bar freshness) no longer dies for the session after 5 consecutive failures; the broker-runtime now drains and surfaces its lifecycle events as `KrakenWsStatus` like the ticker/book/L3 lanes. The ticker/book/trade lanes also carry a heartbeat-based stale-frame watchdog (`KRAKEN_WS_V2_STALE_AFTER` = 60 s) so a half-open socket is recycled in ~60-90 s instead of waiting on a TCP write to fail (potentially 15-30 min).
...
- Additional completeness / future TODOs: 
  - [x] Broker-modular capability discipline documented for the current Alpaca/Kraken scope and any deliberately approved future adapter.
