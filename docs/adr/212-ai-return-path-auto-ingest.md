# ADR-212: AI Return Path Auto-Ingest

**Status:** Accepted
**Date:** 2026-05-05
**Related:** ADR-130 (web research ingestion + packet viewer), ADR-157
(AI session persistence), ADR-162 (AI response cache),
`native/src/app.rs`, `native/src/app/ai.rs`

## Context

ADR-130 shipped the `===TYPHOON_INGEST===` Return Path format plus the
...
The earlier ADR-130 alternative called this "scrape subprocess stdout".
...
- The existing ADR-130 broker handler still owns parsing, URL dedupe,
