# ADR-065: UX Pass: Calendar UI, Staleness, Alerts, Help, Order Entry, Function Renames
...
Follow-up to ADR-064. Remaining items from the UX audit and perf audit after the first wave landed. User asked to tackle the rest.

## Decisions

### 1. Economic Calendar UI parity with ForexFactory

The `econ_calendar.rs` scraper from ADR-064 returned all the data ForexFactory has (impact, forecast, previous, currency). The UI was rendering it in the old collapsed 5-tuple schema. This pass:
...
- `broker_scope` in EV viewer and EVSCRAPE: closed by ADR-047. HV Cone was rejected there as chart-scoped, not fundamentals-scoped.
- `econ_filter_*` persistence: closed by ADR-047.
- Alert attention: closed by ADR-047 with `ViewportCommand::RequestUserAttention(Critical)` instead of adding an audio dependency.
- Help command list auto-generation: closed by ADR-047; the help window now iterates the `COMMANDS` registry directly.
...
- ADR-064 — Broker scope filter + ForexFactory calendar + prior perf/UX pass
