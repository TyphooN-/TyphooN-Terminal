# ADR-215 — News Article Rendering: DOM-aware extractor, hero images, CommonMark viewer; NO HTML/JS renderer

**Status:** Accepted (2026-05-26)
**Supersedes / Builds on:** ADR-107 (news ingest pipeline), ADR-130 (web ingest + packet viewer), ADR-212 (AI return-path auto-ingest)
...
- **Schema**: no migrations. `image_url` column already existed (ADR-107) — the extractor just starts populating it for sources that previously left it blank.
