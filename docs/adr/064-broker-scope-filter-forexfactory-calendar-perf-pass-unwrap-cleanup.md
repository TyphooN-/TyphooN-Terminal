# ADR-064: Broker Scope Filter, ForexFactory Calendar, Perf Pass, Unwrap Cleanup
...
**Darwinex symbol normalization:** Darwinex MT5 symbols use suffixes (`.US`, `.UK`, `.DE`, `.JP`, `.HK`). The scope helper strips everything after the first `.` to get the bare ticker before comparing to the fundamentals set. This is the same pattern used by the Event Calendar in ADR-063.
...
- **Unwrap replacements don't cover 100% of the codebase.** The audit flagged ~8 production unwraps; this pass hit all 8. Test code remains untouched per ADR-061 scope.
...
- Persistent scope indicator: closed by ADR-047.
