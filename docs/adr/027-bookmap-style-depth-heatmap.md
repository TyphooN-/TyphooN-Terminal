...
highlight/ring, header selected side/price/quantity + clear action, scroll list with
order_id/price/qty/age, click-to-copy/select interactions, real/Demo L3 start auto-open, and L3-aware window badges), live depth profile
...
source/depth shape explicitly as Kraken WS L2, Kraken WS L3, Kraken L3 demo,
Kraken snapshot, or Alpaca snapshot.
...
Bookmap's symbol-scoped L3 badge and the Orderbook DOM share the same L3 detector:
explicit `is_l3` metadata or per-order `order_id` fields on either bid or ask side,
so real L3 payloads still render as L3 if the source omits the convenience flag.
...
(chart auto-L2 top-of-book, toolbar L2, Start Stream, Apply to Stream, Bookmap Stream Depth, and L3 start), so equity
