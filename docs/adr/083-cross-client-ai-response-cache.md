# ADR-083: Cross-Client AI Response Cache
...
1. **AI session persistence** (ADR-082) — transcripts are stored in
...
- Accuracy update 2026-05-05: the implemented cache-aside path is the
  ...
  ADR-082, but `claude --print`, Antigravity `--prompt`, and `codex exec`
