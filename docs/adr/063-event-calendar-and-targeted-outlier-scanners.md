...
1. **Extended-hours candle regression.** The magenta pre/post-market candle added in ADR-057 never appeared on charts. Two bugs combined to suppress it silently.
...
- ~~**Fetch events from an external calendar API**~~ **Implemented in ADR-064.** Finnhub economic calendar with ForexFactory XML fallback, impact/currency filters.
- ~~**Include economic calendar events** (FOMC, CPI, NFP)~~ **Implemented in ADR-064.** Separate Economic Calendar window with country/impact/date columns.
