# ADR-052: WASM Web Client for Phone Access
...
- **Order entry from phone:** implemented in ADR-066 with broker selection, two-step confirmation, close/cancel actions.
- **Indicators on phone:** implemented in ADR-069 through server-computed `GetIndicators` / `IndicatorData`.
- **DARWIN analytics on phone:** implemented in ADR-070 through `GetDarwinWeb` / `DarwinWebUpdate`.
- **Push-style updates:** implemented in ADR-069 via `BarUpdate`, `PositionUpdate`, and `AccountUpdate` messages.
