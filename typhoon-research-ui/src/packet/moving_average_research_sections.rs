...
pub fn write_symbol_moving_average_research_sections(
    ctx: &SymbolResearchContext,
    p: &mut String,
    sym_upper: &str,
) {
    // ── research emitters ──
    if let Ok(Some(dm)) = rx::get_dema(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(tm)) = rx::get_tema(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(lr)) = rx::get_linreg(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(pv)) = rx::get_pivots(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(hk)) = rx::get_heikin(ctx.conn, &sym_upper) {
    ...
    // ── research emitters ──
    if let Ok(Some(al)) = rx::get_alma(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(zl)) = rx::get_zlema(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(er)) = rx::get_elderray(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ts)) = rx::get_tsf(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(rv)) = rx::get_rvi(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(tm)) = rx::get_trima(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(t3)) = rx::get_t3(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(vd)) = rx::get_vidya(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(sm)) = rx::get_smi(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(pv)) = rx::get_pvt(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ac)) = rx::get_ac(ctx.conn, &sym_upper) {
        if ac.ac_label != "INSUFFICIENT_DATA" && !ac.ac_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                ac.bars_used, ac.ac_value, ac.ac_prev, ac.ao_value, ac.ao_sma5, ac.last_close
            );
            if !ac.note.is_empty() {
    ...
    if let Ok(Some(cv)) = rx::get_chvol(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(bw)) = rx::get_bbwidth(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ei)) = rx::get_elderimp(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(rm)) = rx::get_rmi(ctx.conn, &sym_upper) {
