...
pub fn write_technical_indicator_squeeze_breakouts(
    ctx: &SymbolResearchContext,
    p: &mut String,
    sym_upper: &str,
) {
    // ── Research section ──
    if let Ok(Some(sq)) = rx::get_squeeze(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(sr)) = rx::get_squeezerank(ctx.conn, &sym_upper) {
        if sr.squeezerank_label != "INSUFFICIENT_DATA" && !sr.squeezerank_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(bb)) = rx::get_bbsqueeze(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(dn)) = rx::get_donchian(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(km)) = rx::get_kama(ctx.conn, &sym_upper) {
