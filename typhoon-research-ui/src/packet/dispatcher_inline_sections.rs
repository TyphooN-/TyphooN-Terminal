    ...
    if let Ok(Some(se)) = rx::get_symbol_expirations(ctx.conn, &sym_upper) {
        if !se.expirations.is_empty() {
            ...
            if !se.next_triple_witching.is_empty() {
                let _ = writeln!(p, "- Next triple witching: **{}**", se.next_triple_witching);
            }
...
pub fn write_candlestick_and_stats(ctx: &SymbolResearchContext, p: &mut String, sym_upper: &str) {
    ...
    if let Ok(Some(ch)) = rx::get_cdl_hammer(ctx.conn, &sym_upper) {
        if ch.cdl_hammer_label != "INSUFFICIENT_DATA" && !ch.cdl_hammer_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(cs)) = rx::get_cdl_shooting_star(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ce)) = rx::get_cdl_engulfing(ctx.conn, &sym_upper) {
        if ce.cdl_engulfing_label != "INSUFFICIENT_DATA" && !ce.cdl_engulfing_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(cr)) = rx::get_cdl_harami(ctx.conn, &sym_upper) {
        if cr.cdl_harami_label != "INSUFFICIENT_DATA" && !cr.cdl_harami_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(cm)) = rx::get_cdl_morning_star(ctx.conn, &sym_upper) {
        if cm.cdl_morning_star_label != "INSUFFICIENT_DATA" && !cm.cdl_morning_star_label.is_empty()
        {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(cv)) = rx::get_cdl_evening_star(ctx.conn, &sym_upper) {
        if cv.cdl_evening_star_label != "INSUFFICIENT_DATA" && !cv.cdl_evening_star_label.is_empty()
        {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(cb)) = rx::get_cdl_three_black_crows(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cw)) = rx::get_cdl_three_white_soldiers(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cd)) = rx::get_cdl_dark_cloud_cover(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cp)) = rx::get_cdl_piercing(ctx.conn, &sym_upper) {
        if cp.cdl_piercing_label != "INSUFFICIENT_DATA" && !cp.cdl_piercing_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(cdd)) = rx::get_cdl_dragonfly_doji(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cgd)) = rx::get_cdl_gravestone_doji(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(chm)) = rx::get_cdl_hanging_man(ctx.conn, &sym_upper) {
        if chm.cdl_hanging_man_label != "INSUFFICIENT_DATA" && !chm.cdl_hanging_man_label.is_empty()
        {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(cih)) = rx::get_cdl_inverted_hammer(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(chc)) = rx::get_cdl_harami_cross(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(clld)) = rx::get_cdl_long_legged_doji(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cm)) = rx::get_cdl_marubozu(ctx.conn, &sym_upper) {
        if cm.cdl_marubozu_label != "INSUFFICIENT_DATA" && !cm.cdl_marubozu_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(cst)) = rx::get_cdl_spinning_top(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cts)) = rx::get_cdl_tristar(ctx.conn, &sym_upper) {
        if cts.cdl_tristar_label != "INSUFFICIENT_DATA" && !cts.cdl_tristar_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    // ── Research section ──
    if let Ok(Some(cds)) = rx::get_cdl_doji_star(ctx.conn, &sym_upper) {
        if cds.cdl_doji_star_label != "INSUFFICIENT_DATA" && !cds.cdl_doji_star_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(cmds)) = rx::get_cdl_morning_doji_star(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ceds)) = rx::get_cdl_evening_doji_star(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cab)) = rx::get_cdl_abandoned_baby(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(c3i)) = rx::get_cdl_three_inside(ctx.conn, &sym_upper) {
    ...
    // ── Research section ──
    if let Ok(Some(cbh)) = rx::get_cdl_belt_hold(ctx.conn, &sym_upper) {
        if cbh.cdl_belt_hold_label != "INSUFFICIENT_DATA" && !cbh.cdl_belt_hold_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(ccm)) = rx::get_cdl_closing_marubozu(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(chw)) = rx::get_cdl_high_wave(ctx.conn, &sym_upper) {
        if chw.cdl_high_wave_label != "INSUFFICIENT_DATA" && !chw.cdl_high_wave_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(cll)) = rx::get_cdl_long_line(ctx.conn, &sym_upper) {
        if cll.cdl_long_line_label != "INSUFFICIENT_DATA" && !cll.cdl_long_line_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(csl)) = rx::get_cdl_short_line(ctx.conn, &sym_upper) {
        if csl.cdl_short_line_label != "INSUFFICIENT_DATA" && !csl.cdl_short_line_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    // ── Research section ──
    if let Ok(Some(cca)) = rx::get_cdl_counterattack(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(chp)) = rx::get_cdl_homing_pigeon(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cin)) = rx::get_cdl_in_neck(ctx.conn, &sym_upper) {
        if cin.cdl_in_neck_label != "INSUFFICIENT_DATA" && !cin.cdl_in_neck_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(con)) = rx::get_cdl_on_neck(ctx.conn, &sym_upper) {
        if con.cdl_on_neck_label != "INSUFFICIENT_DATA" && !con.cdl_on_neck_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(cth)) = rx::get_cdl_thrusting(ctx.conn, &sym_upper) {
        if cth.cdl_thrusting_label != "INSUFFICIENT_DATA" && !cth.cdl_thrusting_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(c2)) = rx::get_cdl_two_crows(ctx.conn, &sym_upper) {
        if c2.cdl_two_crows_label != "INSUFFICIENT_DATA" && !c2.cdl_two_crows_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(c3ls)) = rx::get_cdl_three_line_strike(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(c3o)) = rx::get_cdl_three_outside(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cml)) = rx::get_cdl_matching_low(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(csl)) = rx::get_cdl_separating_lines(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(css)) = rx::get_cdl_stick_sandwich(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(crm)) = rx::get_cdl_rickshaw_man(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ctk)) = rx::get_cdl_takuri(ctx.conn, &sym_upper) {
        if ctk.cdl_takuri_label != "INSUFFICIENT_DATA" && !ctk.cdl_takuri_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(c3sis)) = rx::get_cdl_three_stars_in_south(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(citc)) = rx::get_cdl_identical_three_crows(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ckk)) = rx::get_cdl_kicking(ctx.conn, &sym_upper) {
        if ckk.cdl_kicking_label != "INSUFFICIENT_DATA" && !ckk.cdl_kicking_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(ckbl)) = rx::get_cdl_kicking_by_length(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(clb)) = rx::get_cdl_ladder_bottom(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cutr)) = rx::get_cdl_unique_three_river(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cab)) = rx::get_cdl_advance_block(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cbr)) = rx::get_cdl_breakaway(ctx.conn, &sym_upper) {
        if cbr.cdl_breakaway_label != "INSUFFICIENT_DATA" && !cbr.cdl_breakaway_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(cgssw)) = rx::get_cdl_gap_side_side_white(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cug2c)) = rx::get_cdl_upside_gap_two_crows(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cxgm)) = rx::get_cdl_xside_gap_three_methods(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ccbs)) = rx::get_cdl_conceal_baby_swallow(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(chk)) = rx::get_cdl_hikkake(ctx.conn, &sym_upper) {
        if chk.cdl_hikkake_label != "INSUFFICIENT_DATA" && !chk.cdl_hikkake_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(chkm)) = rx::get_cdl_hikkake_mod(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cmh)) = rx::get_cdl_mat_hold(ctx.conn, &sym_upper) {
        if cmh.cdl_mat_hold_label != "INSUFFICIENT_DATA" && !cmh.cdl_mat_hold_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(crf3m)) = rx::get_cdl_rise_fall_three_methods(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(csp)) = rx::get_cdl_stalled_pattern(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ctg)) = rx::get_cdl_tasuki_gap(ctx.conn, &sym_upper) {
        if ctg.cdl_tasuki_gap_label != "INSUFFICIENT_DATA" && !ctg.cdl_tasuki_gap_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    // ── Research section ──
    if let Ok(Some(ms)) = rx::get_modsharpe(ctx.conn, &sym_upper) {
        if ms.modsharpe_label != "INSUFFICIENT_DATA" && !ms.modsharpe_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(ht)) = rx::get_hsiehtest(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cb)) = rx::get_chowbreak(ctx.conn, &sym_upper) {
        if cb.chowbreak_label != "INSUFFICIENT_DATA" && !cb.chowbreak_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(db)) = rx::get_driftburst(ctx.conn, &sym_upper) {
        if db.driftburst_label != "INSUFFICIENT_DATA" && !db.driftburst_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(hc)) = rx::get_hlvclust(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(yz)) = rx::get_yangzhang(ctx.conn, &sym_upper) {
        if yz.yangzhang_label != "INSUFFICIENT_DATA" && !yz.yangzhang_label.is_empty() {
            let _ = writeln!(
                p,
            ...
            let _ = writeln!(
                ...
                yz.bars_used, yz.overnight_var, yz.open_to_close_var, yz.rs_component, yz.k_weight
            );
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(kp)) = rx::get_kuiper(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(dg)) = rx::get_dagostino(ctx.conn, &sym_upper) {
        if dg.dagostino_label != "INSUFFICIENT_DATA" && !dg.dagostino_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(bp)) = rx::get_baiperron(ctx.conn, &sym_upper) {
        if bp.baiperron_label != "INSUFFICIENT_DATA" && !bp.baiperron_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    if let Ok(Some(kp2)) = rx::get_kupiecpof(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ing)) = rx::get_ingested_articles(ctx.conn, &sym_upper) {
...
pub fn write_quarterly_and_holders(cache: &SqliteCache, p: &mut String, sym_upper: &str) {
...
pub fn write_sec_filings(p: &mut String, sym_upper: &str, sec_filings: &[SecFiling]) {
...
pub fn write_insider_activity(p: &mut String, trades: Option<&[InsiderTrade]>) {
...
pub fn write_price_volatility(cache: &SqliteCache, p: &mut String, sym_upper: &str) {
    let keys = [
...
pub fn write_recent_news(cache: &SqliteCache, p: &mut String, sym_upper: &str) {
    let Some(conn) = cache.try_connection() else {
        return;
    };
