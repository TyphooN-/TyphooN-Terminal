...
pub fn write_symbol_momentum_volume_indicator_sections(
    ctx: &SymbolResearchContext,
    p: &mut String,
    sym_upper: &str,
) {
    // ── SMMA / ALLIGATOR / CRSI / SEB / IMI ──
    if let Ok(Some(sm)) = rx::get_smma(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(al)) = rx::get_alligator(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cr)) = rx::get_crsi(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(sb)) = rx::get_seb(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(im)) = rx::get_imi(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(gm)) = rx::get_gmma(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(me)) = rx::get_maenv(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ad)) = rx::get_adl(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(vh)) = rx::get_vhf(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(vr)) = rx::get_vroc(ctx.conn, &sym_upper) {
    ...
    // ── KDJ / QQE / PMO / CFO / TMF ──
    if let Ok(Some(kj)) = rx::get_kdj(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(qq)) = rx::get_qqe(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(pm)) = rx::get_pmo(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cf)) = rx::get_cfo(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(tm)) = rx::get_tmf(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(fr)) = rx::get_fractals(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ir)) = rx::get_ift_rsi(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ma)) = rx::get_mama(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cg)) = rx::get_cog(ctx.conn, &sym_upper) {
        if cg.cog_label != "INSUFFICIENT_DATA" && !cg.cog_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                cg.bars_used, cg.length, cg.cog_value, cg.cog_prev, cg.cog_signal, cg.last_close
            );
            if !cg.note.is_empty() {
    ...
    if let Ok(Some(dd)) = rx::get_didi(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(dm)) = rx::get_demarker(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(gt)) = rx::get_gator(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(bw)) = rx::get_bw_mfi(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(vw)) = rx::get_vwma(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(sd)) = rx::get_stddev(ctx.conn, &sym_upper) {
