...
pub fn write_symbol_fractal_tail_stationarity_sections(
    ctx: &SymbolResearchContext,
    p: &mut String,
    sym_upper: &str,
) {
    // ── Research section ──
    if let Ok(Some(hi)) = rx::get_higuchi(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(pk)) = rx::get_pickands(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(k3)) = rx::get_kappa3(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ly)) = rx::get_lyapunov(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ra)) = rx::get_rankac(ctx.conn, &sym_upper) {
    ...
    // ── Research section ──
    if let Ok(Some(bj)) = rx::get_bnsjump(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(pp)) = rx::get_pproot(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(mf)) = rx::get_mfdfa(ctx.conn, &sym_upper) {
        if mf.mfdfa_label != "INSUFFICIENT_DATA" && !mf.mfdfa_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                mf.bars_used, mf.h_q_neg2, mf.h_q_zero, mf.h_q_pos2, mf.delta_h, mf.scales_used
            );
            if !mf.note.is_empty() {
    ...
    if let Ok(Some(hk)) = rx::get_hillks(ctx.conn, &sym_upper) {
        if hk.hillks_label != "INSUFFICIENT_DATA" && !hk.hillks_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                hk.bars_used, hk.k_order, hk.alpha_hat, hk.ks_statistic, hk.ks_critical_5pct
            );
            if !hk.note.is_empty() {
    ...
    if let Ok(Some(tsi)) = rx::get_tsi(ctx.conn, &sym_upper) {
    ...
    // ── Research section ──
    if let Ok(Some(g11)) = rx::get_garch11(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(sa)) = rx::get_sadf(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cd)) = rx::get_cordim(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(sk)) = rx::get_skspec(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(am)) = rx::get_automi(ctx.conn, &sym_upper) {
    ...
    // ── Research section ──
    if let Ok(Some(dw)) = rx::get_durbinwatson(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(bd)) = rx::get_bdstest(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(bp)) = rx::get_breuschpagan(ctx.conn, &sym_upper) {
        if bp.bp_label != "INSUFFICIENT_DATA" && !bp.bp_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                bp.bars_used, bp.lm_stat, bp.r_squared, bp.df, bp.critical_95, bp.reject_null
            );
            if !bp.note.is_empty() {
    ...
    if let Ok(Some(tp)) = rx::get_turnpts(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(pg)) = rx::get_periodogram(ctx.conn, &sym_upper) {
        if pg.periodogram_label != "INSUFFICIENT_DATA" && !pg.periodogram_label.is_empty() {
            let _ = writeln!(
                p,
    ...
    // ── Research section ──
    if let Ok(Some(ml)) = rx::get_mcleodli(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ou)) = rx::get_oufit(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(gp)) = rx::get_gph(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(bs)) = rx::get_burgspec(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(kt)) = rx::get_kendalltau(ctx.conn, &sym_upper) {
