...
| **Multi-Account** | 4 uniform Alpaca slots; successfully connected accounts round-robin historical requests/batches into one shared cache namespace (up to ~4× aggregate key budget, independent of Primary) + Kraken trading identities, explicit Broker / Account order targeting (default Primary), account-primary cycling, and opt-in TradeCopy (`TRADECOPY`) |
