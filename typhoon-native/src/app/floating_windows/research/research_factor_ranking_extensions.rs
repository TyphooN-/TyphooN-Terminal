            ...
            egui::Window::new("PEADRANK — PEAD Drift Rank vs Sector Peers")
                ...
                .show(ctx, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(egui::RichText::new("Symbol:").color(AXIS_TEXT));
                        ui.add(
                            egui::TextEdit::singleline(&mut self.peadrank_symbol)
                                .desired_width(100.0),
                        );
                        if ui.button("Use Chart").clicked() {
                            self.peadrank_symbol = chart_sym_research.clone();
                        ...
                        if ui.button("Load Cached").clicked() {
                            if let Some(ref cache) = self.cache {
                                if let Ok(conn) = cache.connection() {
                                    ...
                                    if let Ok(Some(snap)) =
                                        typhoon_engine::core::research::get_peadrank(&conn, &sym_u)
                                    {
                        ...
                        if ui.add(egui::Button::new("Compute").fill(BTN_MG)).clicked() {
                            ...
                            let _ = self
                                .broker_tx
                                .send(BrokerCmd::ComputePeadrankSnapshot { symbol: sym });
                        }
                        if self.peadrank_loading {
                            ui.label(egui::RichText::new("Loading…").color(AXIS_TEXT).small());
                        }
                    });
                    super::render::render_peadrank_snapshot(ui, &self.peadrank_snapshot);
                });
            ...
            egui::Window::new("REVRANK — Relative 3y Revenue CAGR vs Sector")
                ...
                .show(ctx, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(egui::RichText::new("Symbol:").color(AXIS_TEXT));
                        ui.add(
                            egui::TextEdit::singleline(&mut self.revrank_symbol)
                                .desired_width(100.0),
                        );
                        if ui.button("Use Chart").clicked() {
                            self.revrank_symbol = chart_sym_research.clone();
                        ...
                        if ui.button("Load Cached").clicked() {
                            if let Some(ref cache) = self.cache {
                                if let Ok(conn) = cache.connection() {
                                    ...
                                    if let Ok(Some(snap)) =
                                        typhoon_engine::core::research::get_revrank(&conn, &sym_u)
                                    {
                        ...
                        if ui.add(egui::Button::new("Compute").fill(BTN_MG)).clicked() {
                            ...
                            let _ = self
                                .broker_tx
                                .send(BrokerCmd::ComputeRevrankSnapshot { symbol: sym });
                        }
                        if self.revrank_loading {
                            ui.label(egui::RichText::new("Loading…").color(AXIS_TEXT).small());
                        }
                    });
                    super::render::render_revrank_snapshot(ui, &self.revrank_snapshot);
                });
            ...
            egui::Window::new("LEVRANK — Leverage Rank vs Sector")
                ...
                .show(ctx, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(egui::RichText::new("Symbol:").color(AXIS_TEXT));
                        ui.add(
                            egui::TextEdit::singleline(&mut self.levrank_symbol)
                                .desired_width(100.0),
                        );
                        if ui.button("Use Chart").clicked() {
                            self.levrank_symbol = chart_sym_research.clone();
                        ...
                        if ui.button("Load Cached").clicked() {
                            if let Some(ref cache) = self.cache {
                                if let Ok(conn) = cache.connection() {
                                    ...
                                    if let Ok(Some(snap)) =
                                        typhoon_engine::core::research::get_levrank(&conn, &sym_u)
                                    {
                        ...
                        if ui.add(egui::Button::new("Compute").fill(BTN_MG)).clicked() {
                            ...
                            let _ = self
                                .broker_tx
                                .send(BrokerCmd::ComputeLevrankSnapshot { symbol: sym });
                        }
                        if self.levrank_loading {
                            ui.label(egui::RichText::new("Loading…").color(AXIS_TEXT).small());
                        }
                    });
                    super::render::render_levrank_snapshot(ui, &self.levrank_snapshot);
                });
            ...
            egui::Window::new("OPERANK — Operating Quality Rank vs Sector")
                ...
                .show(ctx, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(egui::RichText::new("Symbol:").color(AXIS_TEXT));
                        ui.add(
                            egui::TextEdit::singleline(&mut self.operank_symbol)
                                .desired_width(100.0),
                        );
                        if ui.button("Use Chart").clicked() {
                            self.operank_symbol = chart_sym_research.clone();
                        ...
                        if ui.button("Load Cached").clicked() {
                            if let Some(ref cache) = self.cache {
                                if let Ok(conn) = cache.connection() {
                                    ...
                                    if let Ok(Some(snap)) =
                                        typhoon_engine::core::research::get_operank(&conn, &sym_u)
                                    {
                        ...
                        if ui.add(egui::Button::new("Compute").fill(BTN_MG)).clicked() {
                            ...
                            let _ = self
                                .broker_tx
                                .send(BrokerCmd::ComputeOperankSnapshot { symbol: sym });
                        }
                        if self.operank_loading {
                            ui.label(egui::RichText::new("Loading…").color(AXIS_TEXT).small());
                        }
                    });
                    super::render::render_operank_snapshot(ui, &self.operank_snapshot);
                });
            ...
            egui::Window::new("FQMRANK — Fundamental Quality Rank vs Sector")
                ...
                .show(ctx, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(egui::RichText::new("Symbol:").color(AXIS_TEXT));
                        ui.add(
                            egui::TextEdit::singleline(&mut self.fqmrank_symbol)
                                .desired_width(100.0),
                        );
                        if ui.button("Use Chart").clicked() {
                            self.fqmrank_symbol = chart_sym_research.clone();
                        ...
                        if ui.button("Load Cached").clicked() {
                            if let Some(ref cache) = self.cache {
                                if let Ok(conn) = cache.connection() {
                                    ...
                                    if let Ok(Some(snap)) =
                                        typhoon_engine::core::research::get_fqmrank(&conn, &sym_u)
                                    {
                        ...
                        if ui.add(egui::Button::new("Compute").fill(BTN_MG)).clicked() {
                            ...
                            let _ = self
                                .broker_tx
                                .send(BrokerCmd::ComputeFqmrankSnapshot { symbol: sym });
                        }
                        if self.fqmrank_loading {
                            ui.label(egui::RichText::new("Loading…").color(AXIS_TEXT).small());
                        }
                    });
                    super::render::render_fqmrank_snapshot(ui, &self.fqmrank_snapshot);
                });
            ...
            egui::Window::new("LIQRANK — Liquidity Rank vs Sector")
                ...
                .show(ctx, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(egui::RichText::new("Symbol:").color(AXIS_TEXT));
                        ui.add(
                            egui::TextEdit::singleline(&mut self.liqrank_symbol)
                                .desired_width(100.0),
                        );
                        if ui.button("Use Chart").clicked() {
                            self.liqrank_symbol = chart_sym_research.clone();
                        ...
                        if ui.button("Load Cached").clicked() {
                            if let Some(ref cache) = self.cache {
                                if let Ok(conn) = cache.connection() {
                                    ...
                                    if let Ok(Some(snap)) =
                                        typhoon_engine::core::research::get_liqrank(&conn, &sym_u)
                                    {
                        ...
                        if ui.add(egui::Button::new("Compute").fill(BTN_MG)).clicked() {
                            ...
                            let _ = self
                                .broker_tx
                                .send(BrokerCmd::ComputeLiqrankSnapshot { symbol: sym });
                        }
                        if self.liqrank_loading {
                            ui.label(egui::RichText::new("Loading…").color(AXIS_TEXT).small());
                        }
                    });
                    super::render::render_liqrank_snapshot(ui, &self.liqrank_snapshot);
                });
        ...
        if self.show_tlrank {
            ...
            egui::Window::new("TLRANK — 30-Day Liquidity Rank")
                ...
                .show(ctx, |ui| {
                    ...
                    super::render::render_tlrank_snapshot(ui, &self.tlrank_snapshot);
                });
