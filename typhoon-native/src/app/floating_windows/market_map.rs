...
fn heat_color(change_pct: f64) -> egui::Color32 {
    ...
    if t >= 0.0 {
        ...
        egui::Color32::from_rgb((70.0 - 130.0 * t) as u8, (20.0 + 10.0 * (1.0 + t)) as u8, 30)
    }
}
        ...
        egui::Window::new("Market Map — sectors × market cap")
            ...
            .show(ctx, |ui| {
                ...
                egui::CollapsingHeader::new("Sector groups (cap-weighted performance)")
                    .default_open(false)
                    .show(ui, |ui| {
                        egui::Grid::new("sector_groups").striped(true).show(ui, |ui| {
                ...
                for band in &bands {
                    let band_h =
                        (band.total_cap / grand_total.max(1.0)) as f32 * rect.height();
                    if band_h < 3.0 {
