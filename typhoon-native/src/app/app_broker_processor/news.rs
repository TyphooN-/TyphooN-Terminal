                ...
                .spawn(move || {
                    ...
                    rt.block_on(async {
                        let Some(cache) = shared_cache_broker.read().ok().and_then(|g| g.clone())
                            ...
                            let _ = msg_tx
                                .send(BrokerMsg::Error("NewsScrapeAll: cache not ready".into()));
                            return;
                        };
                        ...
                        if use_kraken {
                            if let Ok(conn) = cache.connection() {
                                match typhoon_engine::core::market_data_symbols::extract_news_symbols_from_market_data_cache(
                                    ...
                                    Ok(kraken_tickers) => {
                                        let _ =
                                            msg_tx.send(BrokerMsg::FundamentalsProgress(format!(
                                                "News scrape: {} Kraken market-data symbols",
                                                kraken_tickers.len()
                                            )));
                        ...
                        tickers.sort();
                        let _ = msg_tx.send(BrokerMsg::FundamentalsProgress(format!(
                            "News scrape: starting for {} symbols",
                            tickers.len()
                        )));
                        let fresh_tickers = cache
                            .connection()
                        ...
                        for (i, ticker) in tickers.iter().enumerate() {
                            // Deduplicate crypto fetches by base asset (e.g. ETH/USD and ETH/EUR both fetch ETH)
                            let fetch_key = if news::is_crypto_symbol(ticker) {
                                news::crypto_base_for_symbol(ticker)
                        ...
                        let _ = msg_tx.send(BrokerMsg::FundamentalsProgress(format!(
                            "News scrape complete: {} OK, {} failed of {}",
                            ok,
                            fail,
                            tickers.len()
                        )));
