...
pub(super) fn write_symbol_market_behavior_sections(
    ctx: &SymbolResearchContext,
    p: &mut String,
    sym_upper: &str,
) {
    // SEAG seasonality (monthly + day-of-week)
    if let Ok(Some(sg)) = rx::get_seasonality(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cm)) = rx::get_correlation(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(tr)) = rx::get_total_return(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ti)) = rx::get_technicals(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(sk)) = rx::get_vol_skew(ctx.conn, &sym_upper) {
        if !sk.expiries.is_empty() {
            ...
            if let Some(exp) = sk.expiries.first() {
                let _ = writeln!(
                    ...
                    exp.expiration, exp.days_to_expiry, exp.atm_iv_pct, exp.put_call_skew_25d_pct
                );
