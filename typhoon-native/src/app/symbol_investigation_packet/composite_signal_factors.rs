...
pub(super) fn write_composite_signal_factors(
    ctx: &SymbolResearchContext,
    p: &mut String,
    sym_upper: &str,
) {
    // ── Research section ──
    if let Ok(Some(v)) = rx::get_val(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(q)) = rx::get_qual(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(r)) = rx::get_risk(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ins)) = rx::get_insstrk(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cv)) = rx::get_covg(ctx.conn, &sym_upper) {
