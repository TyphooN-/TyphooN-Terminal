...
pub(super) fn write_price_behavior_seasonality_vol(
    ctx: &SymbolResearchContext,
    p: &mut String,
    sym_upper: &str,
) {
    if let Ok(Some(om)) = rx::get_omega(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(df)) = rx::get_dfa(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(bk)) = rx::get_burke(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ms)) = rx::get_monthseas(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(rs)) = rx::get_rollsprd(ctx.conn, &sym_upper) {
