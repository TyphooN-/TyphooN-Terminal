...
pub(super) fn write_talib_momentum_range(
    ...
    if let Ok(Some(rc)) = rx::get_roc(ctx.conn, &sym_upper) {
        if rc.roc_label != "INSUFFICIENT_DATA" && !rc.roc_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                rc.bars_used, rc.period, rc.roc, rc.roc_prev, rc.close_now, rc.close_lag
            );
            if !rc.note.is_empty() {
    ...
    if let Ok(Some(rcp)) = rx::get_rocp(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(rcr)) = rx::get_rocr(ctx.conn, &sym_upper) {
        if rcr.rocr_label != "INSUFFICIENT_DATA" && !rcr.rocr_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                rcr.bars_used, rcr.period, rcr.rocr, rcr.rocr_prev, rcr.close_now, rcr.close_lag
            );
    ...
    if let Ok(Some(rc1)) = rx::get_rocr100(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cr)) = rx::get_correl(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(mn)) = rx::get_min(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(mx)) = rx::get_max(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(mm)) = rx::get_minmax(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(mi)) = rx::get_minindex(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(mxi)) = rx::get_maxindex(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(bb)) = rx::get_bbands(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ad)) = rx::get_ad(ctx.conn, &sym_upper) {
        if ad.ad_label != "INSUFFICIENT_DATA" && !ad.ad_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                ad.bars_used, ad.ad, ad.ad_prev, ad.ad_delta, ad.ad_slope, ad.last_close
            );
            if !ad.note.is_empty() {
    ...
    if let Ok(Some(ao)) = rx::get_adosc(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(su)) = rx::get_sum(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(li)) = rx::get_linreg_intercept(ctx.conn, &sym_upper) {
        if li.linreg_intercept_label != "INSUFFICIENT_DATA" && !li.linreg_intercept_label.is_empty()
        {
            let _ = writeln!(
                p,
