...
pub(super) fn write_rank_drift_cone_corrs(
    ...
    if let Ok(Some(rvc)) = rx::get_rvcone(ctx.conn, &sym_upper) {
        if rvc.cone_label != "INSUFFICIENT_DATA" && !rvc.cone_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                rvc.rv20_min_pct, rvc.rv20_median_pct, rvc.rv20_max_pct, rvc.rv20_percentile
            );
    ...
    if let Ok(Some(cpb)) = rx::get_calpb(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cs)) = rx::get_corrstk(ctx.conn, &sym_upper) {
        if cs.correlation_label != "INSUFFICIENT_DATA" && !cs.correlation_label.is_empty() {
            let _ = writeln!(
                p,
            ...
            let _ = writeln!(
                ...
                cs.dominant_benchmark, cs.corr_spy_20d, cs.corr_spy_60d, cs.corr_spy_252d
            );
            let _ = writeln!(
                p,
