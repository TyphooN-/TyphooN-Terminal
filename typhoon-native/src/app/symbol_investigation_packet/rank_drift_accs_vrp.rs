...
pub(super) fn write_rank_drift_accs_vrp(
    ...
    if let Ok(Some(da)) = rx::get_divacc(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ea)) = rx::get_epsacc(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(vrp)) = rx::get_vrp(ctx.conn, &sym_upper) {
