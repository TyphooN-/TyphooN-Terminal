...
pub(super) fn write_rank_drift_vol_perf(
    ...
    if let Ok(Some(at)) = rx::get_atrann(ctx.conn, &sym_upper) {
        if at.regime_label != "INSUFFICIENT_DATA" && !at.regime_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                at.bars_used, at.latest_close, at.atr14, at.atr14_pct, at.atr_annualized_pct
            );
            if !at.note.is_empty() {
    ...
    if let Ok(Some(dh)) = rx::get_ddhist(ctx.conn, &sym_upper) {
        if dh.regime_label != "INSUFFICIENT_DATA" && !dh.regime_label.is_empty() {
            ...
            if !dh.max_drawdown_peak_date.is_empty() && !dh.max_drawdown_trough_date.is_empty() {
                let _ = writeln!(
                    p,
    ...
    if let Ok(Some(pp)) = rx::get_priceperf(ctx.conn, &sym_upper) {
        if pp.trend_label != "INSUFFICIENT_DATA" && !pp.trend_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                pp.bars_used, pp.latest_close, pp.ret_1m_pct, pp.ret_3m_pct, pp.ret_6m_pct
            );
            let _ = writeln!(
                p,
                "- YTD {:+.2}% · 1Y {:+.2}%",
    ...
    if let Ok(Some(mrm)) = rx::get_momrank_multi(ctx.conn, &sym_upper) {
    ...
    // ── Research section ──
    if let Ok(Some(br)) = rx::get_betarank(ctx.conn, &sym_upper) {
            ...
            && !br.rank_label.is_empty()
        {
            ...
            let _ = writeln!(
                ...
                br.sector, br.sector_median_beta, br.sector_p25_beta, br.sector_p75_beta
            );
            if !br.note.is_empty() {
    ...
    if let Ok(Some(pgr)) = rx::get_pegrank(ctx.conn, &sym_upper) {
            ...
            && !pgr.rank_label.is_empty()
        {
            ...
            let _ = writeln!(
                ...
                pgr.sector, pgr.sector_median_peg, pgr.sector_p25_peg, pgr.sector_p75_peg
            );
    ...
    if let Ok(Some(fhl)) = rx::get_fhighlow(ctx.conn, &sym_upper) {
