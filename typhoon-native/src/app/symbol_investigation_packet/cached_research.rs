...
/// Cached research surfaces (DVD/EEB/UPDG/FA/MGMT/SPLT/ANR/ESG). ADR-125 Phase 1
/// step 3 — a free function over the research context using `ctx.conn`.
pub(super) fn write_symbol_cached_research_surfaces(
    ...
    if let Ok(Some(divs)) = rx::get_dividends(ctx.conn, sym_upper) {
        if !divs.is_empty() {
    ...
    if let Ok(Some(est)) = rx::get_earnings_estimates(ctx.conn, sym_upper) {
        if !est.is_empty() {
    ...
    if let Ok(Some(rc)) = rx::get_rating_changes(ctx.conn, sym_upper) {
        if !rc.is_empty() {
    ...
    if let Ok(Some(fa)) = rx::get_financials(ctx.conn, sym_upper) {
    ...
    if let Ok(Some(execs)) = rx::get_executives(ctx.conn, sym_upper) {
        if !execs.is_empty() {
    ...
    if let Ok(Some(splits)) = rx::get_stock_splits(ctx.conn, sym_upper) {
        if !splits.is_empty() {
    ...
    let pt = rx::get_price_target(ctx.conn, sym_upper).ok().flatten();
    let recs = rx::get_analyst_recs(ctx.conn, sym_upper).ok().flatten();
    ...
    if let Ok(Some(esg)) = rx::get_esg(ctx.conn, sym_upper) {
