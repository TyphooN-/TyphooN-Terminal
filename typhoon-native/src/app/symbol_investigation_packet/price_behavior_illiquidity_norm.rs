...
pub(super) fn write_price_behavior_illiquidity_norm(
    ctx: &SymbolResearchContext,
    p: &mut String,
    sym_upper: &str,
) {
    if let Ok(Some(vr)) = rx::get_varratio(ctx.conn, &sym_upper) {
        if vr.rw_label != "INSUFFICIENT_DATA" && !vr.rw_label.is_empty() {
            ...
            let _ = writeln!(p, "- z(2) {:+.2} · z(5) {:+.2}", vr.z_stat_2, vr.z_stat_5);
            if !vr.note.is_empty() {
    ...
    if let Ok(Some(am)) = rx::get_amihud(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(jb)) = rx::get_jbnorm(ctx.conn, &sym_upper) {
