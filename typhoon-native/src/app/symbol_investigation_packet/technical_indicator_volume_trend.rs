...
pub(super) fn write_technical_indicator_volume_trend(
    ctx: &SymbolResearchContext,
    p: &mut String,
    sym_upper: &str,
) {
    if let Ok(Some(vx)) = rx::get_vortex(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ch)) = rx::get_chop(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ob)) = rx::get_obv(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(tr)) = rx::get_trix(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(hm)) = rx::get_hma(ctx.conn, &sym_upper) {
