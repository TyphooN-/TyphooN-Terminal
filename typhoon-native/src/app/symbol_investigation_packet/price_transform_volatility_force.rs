...
pub(super) fn write_price_transform_volatility_force(
    ctx: &SymbolResearchContext,
    p: &mut String,
    sym_upper: &str,
) {
    // ── MASSINDEX / NATR / TTM_SQUEEZE / FORCE_INDEX / TRANGE ──
    if let Ok(Some(mi)) = rx::get_mass_index(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(na)) = rx::get_natr(ctx.conn, &sym_upper) {
        if na.natr_label != "INSUFFICIENT_DATA" && !na.natr_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                na.bars_used, na.length, na.atr_value, na.natr_value, na.natr_prev, na.last_close
            );
            if !na.note.is_empty() {
    ...
    if let Ok(Some(tq)) = rx::get_ttm_squeeze(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(fi)) = rx::get_force_index(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(tr)) = rx::get_trange(ctx.conn, &sym_upper) {
