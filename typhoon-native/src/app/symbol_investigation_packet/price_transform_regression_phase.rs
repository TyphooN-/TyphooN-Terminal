...
pub(super) fn write_price_transform_regression_phase(
    ctx: &SymbolResearchContext,
    p: &mut String,
    sym_upper: &str,
) {
    // ── Research section ──
    if let Ok(Some(lr)) = rx::get_linearreg(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(la)) = rx::get_linearreg_angle(ctx.conn, &sym_upper) {
        if la.angle_label != "INSUFFICIENT_DATA" && !la.angle_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                la.bars_used, la.length, la.slope, la.angle_deg, la.angle_deg_prev, la.last_close
            );
            if !la.note.is_empty() {
    ...
    if let Ok(Some(dp)) = rx::get_ht_dcphase(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(hs)) = rx::get_ht_sine(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(hp)) = rx::get_ht_phasor(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(mp)) = rx::get_midprice(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ap)) = rx::get_apo(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(mm)) = rx::get_mom(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(sx)) = rx::get_sarext(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ar)) = rx::get_adxr(ctx.conn, &sym_upper) {
