...
pub(super) fn write_technical_indicator_final_osc(
    ctx: &SymbolResearchContext,
    p: &mut String,
    sym_upper: &str,
) {
    if let Ok(Some(pp)) = rx::get_ppo(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(dp)) = rx::get_dpo(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(kt)) = rx::get_kst(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(uo)) = rx::get_ultosc(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(wr)) = rx::get_willr(ctx.conn, &sym_upper) {
    ...
    // ── Research section ──
    if let Ok(Some(ms)) = rx::get_mass(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(co)) = rx::get_chaikosc(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(kv)) = rx::get_klinger(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(sr)) = rx::get_stochrsi(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ao)) = rx::get_awesome(ctx.conn, &sym_upper) {
    ...
    // ── Research section ──
    if let Ok(Some(ef)) = rx::get_efi(ctx.conn, &sym_upper) {
        if ef.efi_label != "INSUFFICIENT_DATA" && !ef.efi_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                ef.bars_used, ef.ema_period, ef.raw_efi, ef.efi_value, ef.efi_prev, ef.last_close
            );
            if !ef.note.is_empty() {
    ...
    if let Ok(Some(em)) = rx::get_emv(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(nv)) = rx::get_nvi(ctx.conn, &sym_upper) {
        if nv.nvi_label != "INSUFFICIENT_DATA" && !nv.nvi_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                nv.bars_used, nv.signal_period, nv.nvi_value, nv.signal_value, nv.last_close
            );
            if !nv.note.is_empty() {
    ...
    if let Ok(Some(pv)) = rx::get_pvi(ctx.conn, &sym_upper) {
        if pv.pvi_label != "INSUFFICIENT_DATA" && !pv.pvi_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                pv.bars_used, pv.signal_period, pv.pvi_value, pv.signal_value, pv.last_close
            );
            if !pv.note.is_empty() {
    ...
    if let Ok(Some(cp)) = rx::get_coppock(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(cm)) = rx::get_cmo(ctx.conn, &sym_upper) {
        if cm.cmo_label != "INSUFFICIENT_DATA" && !cm.cmo_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                cm.bars_used, cm.period, cm.sum_up, cm.sum_dn, cm.cmo_value, cm.last_close
            );
            if !cm.note.is_empty() {
    ...
    if let Ok(Some(qs)) = rx::get_qstick(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(ds)) = rx::get_disparity(ctx.conn, &sym_upper) {
        if ds.disparity_label != "INSUFFICIENT_DATA" && !ds.disparity_label.is_empty() {
            ...
            let _ = writeln!(
                ...
                ds.bars_used, ds.period, ds.sma_value, ds.disparity_value, ds.last_close
            );
            if !ds.note.is_empty() {
    ...
    if let Ok(Some(bp)) = rx::get_bop(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(st)) = rx::get_schaff(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(so)) = rx::get_stoch(ctx.conn, &sym_upper) {
    ...
    if let Ok(Some(mc)) = rx::get_macd(ctx.conn, &sym_upper) {
