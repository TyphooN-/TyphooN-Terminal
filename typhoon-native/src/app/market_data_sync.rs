...
impl TyphooNApp {
    ...
    pub(super) fn run_broad_dispatch_slice(&mut self, now_instant: std::time::Instant) {
        ...
        let (ran, new_cursor) = broad_dispatch_rotate(
            ...
            lane_budget,
            |lane| match lane {
                0 => self.broad_dispatch_kraken_universe_lane(
                    now_instant,
                    interval,
                    refill_requested,
                ),
                1 => {
                    self.broad_dispatch_kraken_futures_lane(now_instant, interval, refill_requested)
                }
    ...
    pub(super) fn build_source_cache_state_map(
        ...
        self.bg.source_sync_state.remove(prefix).unwrap_or_default()
    }
    ...
    pub(super) fn kraken_sync_symbol_sectors(&self) -> Vec<Vec<String>> {
        ...
        if self.cached_kraken_sync_sectors_key == Some(active_key)
            && !self.cached_kraken_sync_sectors.is_empty()
        {
    ...
    pub(super) fn schedule_alpaca_pairs(&mut self, symbols: &[String]) -> usize {
        ...
        if low_tf_reserve > 0 {
            ...
            candidates.extend(
                reserve
                    .into_iter()
                    .filter(|c| !just_chosen.contains(&alpaca_fetch_key(&c.symbol, &c.timeframe))),
            );
        }
        drop_background_candidates_when_paused(&mut candidates);
