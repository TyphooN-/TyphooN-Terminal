//! Indicator math (SMA/EMA/RSI/MACD/… + harmonic patterns), pure over `Bar` (ADR-125
//! Target 2, slice 2). No egui, no engine, no `TyphooNApp`.
use crate::types::Bar;

pub fn compute_sma(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
...
pub fn compute_ema(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if n < period {
        return out;
...
pub fn compute_regression_channel(
    bars: &[Bar],
    period: usize,
) -> (Vec<Option<f64>>, Vec<Option<f64>>, Vec<Option<f64>>) {
    let n = bars.len();
...
pub fn compute_squeeze_momentum(
    bb_upper: &[Option<f64>],
    bb_lower: &[Option<f64>],
    kc_upper: &[Option<f64>],
    kc_lower: &[Option<f64>],
...
pub fn compute_rsi(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if n <= period {
        return out;
...
pub fn compute_fisher(bars: &[Bar], period: usize) -> (Vec<Option<f64>>, Vec<Option<f64>>) {
    let n = bars.len();
...
pub fn true_range(bar: &Bar, prev: &Bar) -> f64 {
...
pub fn renko_bricks(bars: &[Bar]) -> Vec<Bar> {
    if bars.len() < 15 {
        return bars.to_vec();
    }
...
pub struct HarmonicPattern {
    pub name: &'static str,
    pub x: (usize, f64), // bar index, price
    pub a: (usize, f64),
    pub b: (usize, f64),
    pub c: (usize, f64),
    pub d: (usize, f64), // completion / entry point
    pub tp1: f64,        // target 1 (0.382 AD)
    pub tp2: f64,        // target 2 (0.618 AD)
    pub sl: f64,         // stop loss (beyond X)
    pub bullish: bool,
}
...
pub fn compute_fractals_up(bars: &[Bar]) -> Vec<bool> {
    let n = bars.len();
...
pub fn compute_fractals_down(bars: &[Bar]) -> Vec<bool> {
    let n = bars.len();
    let mut out = vec![false; n];
    if n < 5 {
        return out;
...
/// Group bars into consecutive periods keyed by `key(ts_ms)` and return the
/// previous (last closed) and current (forming) period high/low —
/// `((prev_hi, prev_lo), (cur_hi, cur_lo))`.
fn group_levels_by(bars: &[Bar], key: impl Fn(i64) -> i64) -> (HiLo, HiLo) {
...
// Period keys aligned to the app's own candle boundaries (chart_merge_bucket_ts):
// UTC-midnight days, Monday-start weeks, calendar months. The old weekly key
// (ts / 7 days) bucketed from the epoch, a Thursday, so weekly levels disagreed
// with the chart's own W1 candles by several days.
fn hour_key(ts_ms: i64) -> i64 {
    ts_ms.div_euclid(3_600_000)
}
fn four_hour_key(ts_ms: i64) -> i64 {
    ts_ms.div_euclid(14_400_000)
}
fn day_key(ts_ms: i64) -> i64 {
    ts_ms.div_euclid(86_400_000)
}
fn week_key(ts_ms: i64) -> i64 {
    // Epoch day 0 (1970-01-01) is a Thursday — 3 days after Monday — so shifting
    // by 3 puts week boundaries on Monday 00:00 UTC.
    (ts_ms.div_euclid(86_400_000) + 3).div_euclid(7)
}
fn month_key(ts_ms: i64) -> i64 {
    use chrono::Datelike;
    chrono::DateTime::from_timestamp(ts_ms / 1000, 0)
        .map_or(0, |dt| dt.year() as i64 * 100 + dt.month() as i64)
}
...
pub fn compute_prev_candle_levels(bars: &[Bar]) -> (HiLo, HiLo, HiLo, HiLo, HiLo) {
    ...
    let d1 = group_levels_by(bars, day_key).0;
    let w1 = group_levels_by(bars, week_key).0;
    let mn1 = group_levels_by(bars, month_key).0;
    (h1, h4, d1, w1, mn1)
}

/// Current ("Judas") candle levels — the *forming* D1/W1/MN1 period high/low.
/// PreviousCandleLevels.mqh draws these alongside the previous levels.
/// Returns (D1, W1, MN1) current candle high/low.
#[allow(clippy::type_complexity)]
pub fn compute_current_candle_levels(bars: &[Bar]) -> (HiLo, HiLo, HiLo) {
    if bars.is_empty() {
        return ((None, None), (None, None), (None, None));
    }
    let d1 = group_levels_by(bars, day_key).1;
    let w1 = group_levels_by(bars, week_key).1;
    let mn1 = group_levels_by(bars, month_key).1;
    (d1, w1, mn1)
}
...
pub fn sma_of_option(data: &[Option<f64>], period: usize) -> Vec<Option<f64>> {
    let n = data.len();
    let mut out = vec![None; n];
...
pub fn compute_wma(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if n < period {
        return out;
...
pub fn compute_hma(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
...
pub fn compute_cci(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if n < period {
        return out;
...
pub fn compute_williams_r(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if n < period {
        return out;
...
pub fn compute_obv(bars: &[Bar]) -> Vec<Option<f64>> {
    let n = bars.len();
    if n == 0 {
        return vec![];
    }
...
pub fn compute_momentum(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    for i in period..n {
...
pub fn compute_cmo(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
...
pub fn compute_qstick(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if period == 0 || n < period {
        return out;
...
pub fn compute_disparity(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if period == 0 || n < period {
        return out;
...
pub fn compute_bop(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if period == 0 || n < period {
        return out;
...
pub fn compute_stddev(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
...
pub fn ema_of_values(values: &[f64], period: usize) -> Vec<Option<f64>> {
    let n = values.len();
    let mut out = vec![None; n];
    if period == 0 || n < period {
        return out;
...
pub fn ema_of_option_series(values: &[Option<f64>], period: usize) -> Vec<Option<f64>> {
    let n = values.len();
    let mut out = vec![None; n];
    if period == 0 {
        return out;
...
pub fn compute_mfi(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if period == 0 || n <= period {
        return out;
...
pub fn compute_ultosc(bars: &[Bar]) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if n <= 28 {
        return out;
...
pub fn compute_var_oscillator(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
...
pub fn compute_parabolic_sar(bars: &[Bar], af_step: f64, af_max: f64) -> Vec<Option<f64>> {
    let n = bars.len();
    if n < 2 {
        return vec![None; n];
    }
...
pub fn compute_atr_projection_levels(
    bars: &[Bar],
...
pub fn aggregate_bars_to_htf(bars: &[Bar], htf_minutes: u32) -> Vec<Bar> {
    if bars.is_empty() {
        return Vec::new();
    }
...
pub fn compute_better_volume(bars: &[Bar]) -> Vec<u8> {
    let n = bars.len();
...
pub fn compute_supply_demand_zones_from_gpu(
    gpu_data: &[f32],
    bars: &[Bar],
...
pub fn compute_supply_demand_zones(
    bars: &[Bar],
) -> (Vec<(usize, f64, f64, u8)>, Vec<(usize, f64, f64, u8)>) {
    pub(crate) const FRACTAL_LOOKBACK: usize = 5;
    pub(crate) const BACK_LIMIT: usize = 1000;
...
pub fn ehlers_super_smoother(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if n < 3 {
        return out;
...
pub fn ehlers_decycler(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
...
pub fn ehlers_instantaneous_trendline(bars: &[Bar]) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if n < 7 {
        return out;
...
pub fn ehlers_even_better_sinewave(bars: &[Bar], duration: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if n < 5 {
        return out;
...
pub fn ehlers_cyber_cycle(bars: &[Bar]) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if n < 5 {
        return out;
...
pub fn ehlers_cg_oscillator(bars: &[Bar], period: usize) -> Vec<Option<f64>> {
    let n = bars.len();
    let mut out = vec![None; n];
    if n < period {
        return out;
