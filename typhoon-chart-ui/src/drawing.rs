//! Chart drawing tools — trendlines, fibs, patterns, anchored VWAP, trade overlays
//! (ADR-125 Target 2, slice 4). egui-only value types (no engine, no `TyphooNApp`),
//! the "drawing tools" the ADR assigns to the chart-UI crate. `ChartState` holds
//! `Drawing` / `TradeOverlay` fields, so these move ahead of it.

// ─── drawing tools ───────────────────────────────────────────────────────────

pub const TRENDLINE_COL: egui::Color32 = egui::Color32::from_rgb(100, 200, 255);
pub const FIBO_COL: egui::Color32 = egui::Color32::from_rgb(200, 160, 100);
...
pub struct TradeMarker {
    pub bar_idx: usize, // index into bars array
    pub price: f64,
    pub volume: f64,    // aggregated total lots
    pub is_buy: bool,   // true=buy, false=sell
    pub count: u32,     // number of individual fills aggregated
    pub ticker: String, // source label (e.g., "Alpaca", "Kraken")
}
