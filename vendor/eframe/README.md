...
> **TyphooN-Terminal local patch** (vendored copy of eframe 0.35.0, wired via
> `[patch.crates-io]` in the workspace `Cargo.toml`):
>
> **Redraw-starvation watchdog** — Wayland compositors withhold frame
> callbacks from windows on inactive workspaces, so winit never delivers
> `RedrawRequested` while `window.is_visible()` still reports true. Upstream
> eframe then parks forever: no frame runs, and `App::logic` — documented to
> run "also when the UI is hidden" — never fires, freezing any in-frame data
> pump until the window is refocused. The patch tracks `request_redraw()`
> calls that go undelivered for ≥1s (`REDRAW_STARVATION_THRESHOLD` in
> `src/native/run.rs`) and, while starved, drives throttled (≥250ms)
> background ticks: `run_ui_and_paint(.., background_tick = true)` runs
> `App::logic` but treats the viewport as invisible, so `App::ui`, painting
> and presentation are all skipped (an occluded Wayland swapchain present
> could otherwise block the event loop). The first genuinely delivered
> `RedrawRequested` clears the state and normal rendering resumes.
>
> Files touched: `src/native/run.rs`, `src/native/winit_integration.rs`,
> `src/native/wgpu_integration.rs`, `src/native/glow_integration.rs` — all
> marked with "TyphooN-Terminal local patch". When bumping the egui/eframe
> version, re-vendor the new eframe and re-apply these marked blocks.
