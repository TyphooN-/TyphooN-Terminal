...
impl WinitApp for WgpuWinitApp<'_> {
    ...
    fn run_ui_and_paint(
        ...
        event_loop: &ActiveEventLoop,
        window_id: WindowId,
        background_tick: bool,
    ) -> Result<EventResult> {
        self.initialized_all_windows(event_loop);

        if let Some(running) = &mut self.running {
            running.run_ui_and_paint(window_id, event_loop, background_tick)
        } else {
            Ok(EventResult::Wait)
        }
    }
...
impl WgpuWinitRunning<'_> {
    ...
    fn run_ui_and_paint(
        ...
        let (viewport_ui_cb, raw_input, is_visible, run_ui) = {
            ...
            // TyphooN-Terminal local patch: a background tick must never
            // paint/present — on Wayland the swapchain of an occluded window
            // can block the event-loop thread in acquire. Treating the
            // viewport as invisible reuses the existing logic-only path.
            let is_visible = viewport.info.visible().unwrap_or(true) && !background_tick;
