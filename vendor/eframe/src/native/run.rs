...
// ---------------------------------------------------------------------------
// TyphooN-Terminal local patch: redraw-starvation watchdog.
//
// Wayland compositors withhold frame callbacks from surfaces that are not
// visible (window on another workspace, fully occluded, …). winit gates
// `RedrawRequested` delivery on those callbacks, so `window.request_redraw()`
// can starve indefinitely while `window.is_visible()` still reports true —
// neither the invisible-window direct-paint path (#5229) nor anything else
// ever runs the frame, and `App::logic` stops ticking until the window is
// focused again. The watchdog detects a `request_redraw()` that has gone
// undelivered for `REDRAW_STARVATION_THRESHOLD` and then drives throttled
// background ticks (`run_ui_and_paint` with `background_tick = true`):
// `App::logic` runs, `App::ui` and painting are skipped, so no swapchain
// present can block on the withheld frame callbacks. The first genuinely
// delivered `RedrawRequested` clears the starvation state and normal
// painting resumes.
// ---------------------------------------------------------------------------

/// How long a `request_redraw()` may go undelivered before we consider the
/// window redraw-starved. Generous against slow-compositor false positives: a
/// spurious background tick is harmless (it never paints).
const REDRAW_STARVATION_THRESHOLD: Duration = Duration::from_secs(1);

/// Minimum interval between background ticks of a redraw-starved window.
/// Bounds worst-case hidden CPU use no matter how aggressively the app
/// requests repaints (e.g. catch-up draining of a message backlog).
const STARVED_WINDOW_TICK_INTERVAL: Duration = Duration::from_millis(250);
...
struct WinitAppWrapper<T: WinitApp> {
    ...
    /// TyphooN-Terminal local patch: per window, when the oldest still-undelivered
    /// `request_redraw()` was issued. Entry removed once `RedrawRequested` is
    /// actually delivered. Feeds the redraw-starvation watchdog.
    windows_undelivered_redraw_since: HashMap<WindowId, Instant>,
    /// TyphooN-Terminal local patch: last background tick per starved window,
    /// enforcing `STARVED_WINDOW_TICK_INTERVAL` at tick time (robust even if
    /// repaint deadlines get pulled earlier by app repaint requests).
    windows_last_background_tick: HashMap<WindowId, Instant>,
    winit_app: T,
...
impl<T: WinitApp> WinitAppWrapper<T> {
    fn new(winit_app: T, run_and_return: bool) -> Self {
        Self {
            ...
            windows_undelivered_redraw_since: HashMap::default(),
            windows_last_background_tick: HashMap::default(),
            winit_app,
            return_result: Ok(()),
            run_and_return,
        }
    ...
    fn handle_event_result(
            ...
            && let Ok(EventResult::RepaintNow(window_id)) = event_result
        {
            ...
            event_result = self.winit_app.run_ui_and_paint(event_loop, window_id, false);
        }
    ...
    fn check_redraw_requests(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let mut starved_window_ids = Vec::new();
            ...
            .retain(|window_id, repaint_time| {
                ...
                if let Some(window) = self.winit_app.window(*window_id) {
                    ...
                    if is_invisible_or_minimized(&window) {
                        ...
                        .windows_undelivered_redraw_since
                        .get(window_id)
                        .is_some_and(|since| {
                            now.duration_since(*since) >= REDRAW_STARVATION_THRESHOLD
                        })
                    {
                        // TyphooN-Terminal local patch: redraw delivery is starved
                        // (Wayland window on an inactive workspace). Tick app logic
                        // directly, throttled, without painting. Keep one redraw
                        // request outstanding so normal painting resumes the moment
                        // the compositor delivers again, and keep this entry armed
                        // so the watchdog stays alive even if the app stops
                        // requesting repaints.
                        if self
                            .windows_last_background_tick
                            .get(window_id)
                            .is_none_or(|last| {
                                now.duration_since(*last) >= STARVED_WINDOW_TICK_INTERVAL
                            })
                        {
                            starved_window_ids.push(*window_id);
                        }
                        window.request_redraw();
                        *repaint_time = now + STARVED_WINDOW_TICK_INTERVAL;
                        true
                    } else {
                        log::trace!("request_redraw for {window_id:?}");
                        self.windows_undelivered_redraw_since
                            .entry(*window_id)
                            .or_insert(now);
                        event_loop.set_control_flow(ControlFlow::Poll);
                        window.request_redraw();
                        // TyphooN-Terminal local patch: instead of dropping the
                        // entry, re-arm a checkup so starvation is detected even
                        // when no further repaint requests arrive. Harmless when
                        // delivery is healthy: the app's own repaint requests
                        // min-merge below this deadline.
                        *repaint_time = now + REDRAW_STARVATION_THRESHOLD;
                        true
                    }
                } else {
                    log::trace!("No window found for {window_id:?}");
                    self.windows_undelivered_redraw_since.remove(window_id);
                    self.windows_last_background_tick.remove(window_id);
                    false
                }
            });
        ...
        for window_id in &invisible_window_ids {
            let event_result = self.winit_app.run_ui_and_paint(event_loop, *window_id, false);
            self.handle_event_result(event_loop, event_result);
        }

        // TyphooN-Terminal local patch: background ticks for redraw-starved
        // windows — App::logic only, no ui, no paint.
        for window_id in &starved_window_ids {
            self.windows_last_background_tick
                .insert(*window_id, Instant::now());
            let event_result = self
                .winit_app
                .run_ui_and_paint(event_loop, *window_id, true);
            self.handle_event_result(event_loop, event_result);
        }
        ...
        event_loop_context::with_event_loop_context(event_loop, move || {
            let event_result = match event {
                winit::event::WindowEvent::RedrawRequested => {
                    // TyphooN-Terminal local patch: an actually-delivered redraw
                    // proves the compositor is serving this window again — clear
                    // the starvation watchdog state.
                    self.windows_undelivered_redraw_since.remove(&window_id);
                    self.windows_last_background_tick.remove(&window_id);
                    self.winit_app.run_ui_and_paint(event_loop, window_id, false)
                }
