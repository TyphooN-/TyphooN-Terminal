...
impl WinitApp for GlowWinitApp<'_> {
    ...
    fn run_ui_and_paint(
        ...
        if let Some(running) = &mut self.running {
            running.run_ui_and_paint(event_loop, window_id, background_tick)
        } else {
            Ok(EventResult::Wait)
        }
    }
...
impl GlowWinitRunning<'_> {
    fn run_ui_and_paint(
        ...
        let (raw_input, viewport_ui_cb, is_visible, run_ui) = {
            ...
            // TyphooN-Terminal local patch: background ticks run logic only —
            // treat the viewport as invisible so nothing is painted/swapped.
