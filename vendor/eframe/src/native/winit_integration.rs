...
pub trait WinitApp {
    ...
    /// TyphooN-Terminal local patch: when `background_tick` is true, run
    /// [`crate::App::logic`] without running [`crate::App::ui`] or painting.
    /// Used for windows whose compositor is withholding `RedrawRequested`
    /// (e.g. a Wayland window on an inactive workspace), so app logic keeps
    /// ticking while nothing can be presented.
    fn run_ui_and_paint(
        &mut self,
        event_loop: &ActiveEventLoop,
        window_id: WindowId,
