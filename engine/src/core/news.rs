...
pub fn upsert_news(conn: &Connection, article: &NewsArticle) -> Result<(), String> {
    ...
    if a.url_hash.is_empty() { return Err("url_hash empty after hash".into()); }
    ...
    let _ = conn.execute("DELETE FROM research_news_fts WHERE url_hash = ?1", params![a.url_hash]);
    let _ = conn.execute(
...
pub fn get_news_by_symbol(conn: &Connection, symbol: &str, limit: usize) -> Result<Vec<NewsArticle>, String> {
    let _ = create_news_tables(conn);
    ...
    let mut stmt = conn.prepare(sql).map_err(|e| format!("prepare get_news: {e}"))?;
    let lim = limit as i64;
    let mut rows = if sym.is_empty() {
        stmt.query(params![lim]).map_err(|e| format!("query get_news: {e}"))?
    } else {
        let like = format!("%\"{}\"%", sym);
        stmt.query(params![lim, sym, like]).map_err(|e| format!("query get_news: {e}"))?
    };
    let mut out = Vec::new();
...
pub fn search_news(conn: &Connection, query: &str, limit: usize) -> Result<Vec<NewsArticle>, String> {
    let _ = create_news_tables(conn);
...
pub fn purge_older_than(conn: &Connection, cutoff_ts: i64) -> Result<usize, String> {
    ...
    let hashes: Vec<String> = {
        let mut stmt = conn.prepare("SELECT url_hash FROM research_news WHERE published_at < ?1")
            .map_err(|e| format!("prepare purge select: {e}"))?;
        let mut rows = stmt.query(params![cutoff_ts]).map_err(|e| format!("query purge: {e}"))?;
        let mut v = Vec::new();
    ...
    for h in &hashes {
        ...
        let _ = conn.execute("DELETE FROM research_news_fts WHERE url_hash = ?1", params![h]);
    }
    Ok(hashes.len())
}
...
pub async fn fetch_gdelt_news(
    ...
    let v: serde_json::Value = serde_json::from_str(&text)
        .map_err(|e| format!("GDELT parse: {e}; body: {}", &text.chars().take(120).collect::<String>()))?;
    let mut out = Vec::new();
...
pub async fn fetch_yahoo_rss(
    ...
    let url = format!("https://feeds.finance.yahoo.com/rss/2.0/headline?s={}&region=US&lang=en-US", clean);
    let resp = client.get(&url)
    ...
    let body = resp.text().await.map_err(|e| format!("Yahoo RSS read: {e}"))?;
...
pub async fn fetch_sec_edgar_rss(
    ...
    let body = resp.text().await.map_err(|e| format!("EDGAR RSS read: {e}"))?;
...
pub async fn fetch_marketaux_news(
    ...
    if token.is_empty() { return Err("Marketaux API key required".into()); }
    let sym = symbol.replace("/", "").to_uppercase();
    let resp = client.get("https://api.marketaux.com/v1/news/all")
        .query(&[
            ("api_token", token),
            ("symbols", sym.as_str()),
    ...
    let v: serde_json::Value = resp.json().await.map_err(|e| format!("Marketaux parse: {e}"))?;
    let mut out = Vec::new();
    if let Some(arr) = v["data"].as_array() {
        for e in arr {
            ...
            if let Some(ents) = e["entities"].as_array() {
                for en in ents {
                    if let Some(sym) = en["symbol"].as_str() {
                        if !sym.is_empty() { tickers.push(sym.to_uppercase()); }
                    }
                    if sent_score == 0.0 {
            ...
            let sentiment = if sent_score > 0.15 { "bullish" }
                else if sent_score < -0.15 { "bearish" }
                else { "neutral" };
            let art = NewsArticle {
                symbol: sym.clone(),
...
pub async fn fetch_alpha_vantage_news(
    ...
    if token.is_empty() { return Err("Alpha Vantage API key required".into()); }
    let sym = symbol.replace("/", "").to_uppercase();
    let resp = client.get("https://www.alphavantage.co/query")
        .query(&[
    ...
    let v: serde_json::Value = resp.json().await.map_err(|e| format!("Alpha Vantage parse: {e}"))?;
                ...
                for t in ts_arr {
                    if let Some(tsym) = t["ticker"].as_str() {
                        ...
                        if tsym.to_uppercase() == sym {
                            rel_sent = t["ticker_sentiment_score"].as_str()
...
pub async fn fetch_fmp_news(
    ...
    if token.is_empty() { return Err("FMP API key required".into()); }
    let sym = symbol.replace("/", "").to_uppercase();
    let url = format!(
...
fn strip_cdata(s: &str) -> String {
    let s = s.trim();
    if let Some(inner) = s.strip_prefix("<![CDATA[").and_then(|x| x.strip_suffix("]]>")) {
        inner.to_string()
    } else {
        s.to_string()
    }
...
fn extract_all_tag_bodies<'a>(body: &'a str, tag: &str) -> Vec<&'a str> {
    ...
    while let Some(rel_start) = body[cursor..].find(&open) {
        ...
        let Some(content_offset) = after_open.find('>') else { break };
        let content_start_abs = abs_start + content_offset + 1;
        let rest = &body[content_start_abs..];
        let Some(end_rel) = rest.find(&close) else { break };
...
pub async fn fetch_all_sources_for_symbol(
    ...
    cb(&format!("news/{}: {} articles fetched", sym, all_articles.len()));
    Ok(all_articles)
}
...
mod tests {
    ...
    fn upsert_batch_counts_rows() {
        let conn = mem_conn();
        let articles: Vec<NewsArticle> = (0..5).map(|i| NewsArticle {
            symbol: "X".into(),
