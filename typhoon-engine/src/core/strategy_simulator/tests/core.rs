...
fn event_log_is_strictly_ordered_by_time_priority_sequence() {
    ...
    for record in &report.events {
        let key = (record.time_ns, record.priority, record.sequence);
...
struct CancelOnSecondDecision {
    decisions: usize,
    target: Option<ClientOrderId>,
}

impl ReferenceStrategy for CancelOnSecondDecision {
    fn on_bar_close(
        &mut self,
        ...
        if self.decisions == 0 {
            self.target = Some(orders.submit(OrderRequest::limit(
                context.symbol(),
                OrderSide::Buy,
                1.0,
                50.0,
            ))?);
        } else if self.decisions == 1 {
            orders.cancel(self.target.expect("the first decision submitted an order"))?;
        }
        self.decisions += 1;
        Ok(())
    }
...
#[test]
fn a_request_created_by_a_decision_never_time_travels_before_that_decision() {
    let report = run(
        free_settings(),
        &[ramp("aaa", 3)],
        &mut CancelOnSecondDecision {
            decisions: 0,
            target: None,
        },
    )
    ...
    for pair in report.events.windows(2) {
        assert!(
            (pair[0].time_ns, pair[0].priority, pair[0].sequence)
                < (pair[1].time_ns, pair[1].priority, pair[1].sequence),
            "event order inverted: {:?} then {:?}",
            pair[0],
            pair[1]
        );
    }
...
fn reference_ledger_v1_golden_digest_is_stable() {
    ...
    assert_eq!(
        digest,
        "7080ebf3909607eb14892ffd43977a8e3a6d09b8c344ddf0d04a240c033e16dd"
    );
}
...
#[test]
fn explicit_legacy_compatibility_reproduces_same_close_execution() {
    let settings = ExecutionSettings {
        compatibility: crate::core::strategy_ir::ExecutionCompatibility::LegacySameBarClose,
        ..free_settings()
    };
    ...
    .expect("legacy compatibility run succeeds");

    assert_eq!(
        report.fills.len(),
        2,
        "legacy compatibility enters at the decision close and liquidates at the final close"
    );
    let fill = &report.fills[0];
    ...
    assert_close(
        fill.fill_price,
        100.5,
        "legacy fill uses the decision close",
    );
    assert_eq!(
        report.fills[1].time_ns,
        3 * MINUTE_NS,
        "forced liquidation is a causal event immediately after the final close"
    );
...
fn invalid_order_quantities_are_rejected() {
    ...
    impl ReferenceStrategy for BadQuantity {
        fn on_bar_close(
            ...
            orders
                .market(ctx.symbol(), OrderSide::Buy, self.0)
                .map(|_| ())
        }
    }
    let streams = [ramp("aaa", 3)];
...
fn an_unknown_symbol_id_is_rejected() {
    ...
    impl ReferenceStrategy for WrongSymbol {
        fn on_bar_close(
            ...
            orders.market(SymbolId(9), OrderSide::Buy, 1.0).map(|_| ())
        }
    }
    let streams = [ramp("aaa", 3)];
    assert_eq!(
